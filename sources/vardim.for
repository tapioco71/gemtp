...
!  lunit(2) = 7
  ...
  mulvar(3)  =  2
  mulvar(4)  =  1
  open (newunit = lunit(2), status = 'scratch', iostat = ios, form = 'formatted', action = 'write')
  if (ios .eq. 0) then
     inquire (unit = lunit(2), name = filename)
     if (filename .ne. '') write (lunit(6), 2) lunit(2), trim(filename)
2    format ('Scratch file opened on unit ', i4, ' with name ', a32)
     indexm = 0
     mextra = 0
