...
subroutine over2
  ...
  integer(4) :: i, iaddrs, ibf, ibr2, ibr3, ibrnam, ibrter, icas, ichar, ichtr2, icrit(1)
  ...
  integer(4) :: itemq, itranm, itrans, itym1, ityold
  ...
  integer(4) :: n, n5, n6, n8, n9, n9sq, n14, n16, n24, ncoil, ncount, nextsq(1), nfir, nfscan, nkq
...
2000 ityold = 0
  ...
  nycmp = 0
  isecti = 400
1000 if (iprsup .ge. 1) write (unit = lunit6, fmt = 4567)
...
25970 if (iprsup .lt. 1) go to 5971
  n1 = iaddrs
  n2 = n1 + n9sq -1
  write (unit = lunit6, fmt = 15968) n1, n2, (qfd(i), i = n1, n2), (sfd(i), i = n1, n2)
15968 format (' qfd(i) and sfd(i) for i =', i6, ' to ', i6, ' are', /, (1x, 8e15.7))
  write (unit = lunit6, fmt = 25968) ibr1, iaddrs, litype(ibr1)
...
4011 vzero(inonl) = tr(it)
  ...
  nltype(inonl) = -98
  curr(inonl) = 1.0d0
  if (iprsup .ge. 2) write (unit = lunit6, fmt = 4014) ksat, it, ichar, ntot, it3, inonl
...
64041 format (/, ' Switch arrays with xformer info.', /, 11x, 'modswt', 4x, 'kmswit', 4x, 'isourc', 4x, 'nextsw', 5x, 'icrit', 6x, 'kpos', 9x, 'tclose', 9x, ' topen', /, (7x, 6i10, 2e15.5))
  write (unit = lunit6, fmt = 74041) (i, tr(i), tx(i), emtpc(i), i = 1, it)
...
64033 modswt(kswtch) = ibr + 1
  kodebr(ibr) = 1
  if (iprsup .ge. 1) write (unit = lunit6, fmt = 54033) yzero, kswtch, ibr, it, inonl, ichar, kph, ksat, ideal, keepb, keept, ntot, nodtop, n1, n2
...
44035 iref = iref + 1
  n5 = iabs (length(iref))
  if (iprsup .gt. 3) write (unit = lunit6, fmt = 74035) ibr, it, ktref, iref, n5, kbus(ibr), mbus(ibr)
74035 format (/, ' Ref. comp. copy of r1-l1 and mag. branch, at 74035', /, 1x, 8i15)
  ...
  go to 44035
4044 continue
  read (unit = abuff, fmt = 4046) tr(it), tx(it), emtpc(it), iout
4046 format (26x, 3e6.2, 35x, i1)
  if (noutpr .eq. 0) write (unit = kunit6, fmt = 54047) itype, tr(it), tx(it), emtpc(it)
54047 format ('+Winding', i2, '. ',  3e12.4)
  i = it + 1
  call move0 (tr(i :), ll2)
  call move0 (tx(i :), ll2)
  call move0 (emtpc(i :), ll2)
  if (emtpc(it) .gt. 0.0d0) go to 4047
  kill = 52
  lstat(19) = 4047
  lstat(16) = 3
  go to 9200
...
4054 ncoil = -nonle(inonl) - it3
  ...
  ichar = it3 - 1
  inonl = inonl - 1
  if (iprsup .ge. 2) write (unit = lunit6, fmt = 4053) it, ibr, ichar, inonl, kswtch, ncoil, ksat, kph
...
4068 if (itype .lt. icrit(ktref)) go to 100
  ktref = 0
  go to 100
4070 d1 = emtpc(it) / turn1
  if (ksat .gt. 3) go to 4073
  topen(kswtch) = emtpc(it)
...
4071 emtpc(it) = 0.0
  ...
  tx(it) = cut3
  call ibrinc
  if (iprsup .ge. 2) write (unit = lunit6, fmt = 4076) it, d1, cut1, cut2, tr(it), tx(it)
4076 format (/, ' Row 1 of 2x2 at 4076', i10, 5e15.4, /, 1x)
  ...
  tr(it) = d1 * cut2
  tx(it) = cut3
  if (iprsup .ge. 2) write (unit = lunit6, fmt = 4084) it
...
54205 if (iprsup .ge. 2) write (unit = lunit6, fmt = 54207) npower, maxpe, koutvp(npower), koutvp(mpower)
...
171 if (noutpr .ne. 0) go to 5411
  ...
  write (unit = kunit6, fmt = 54777) tr(it), tx(it), emtpc(it)
...
8331 format (26x, 3e16.0)
     ...
     write (unit = kunit6, fmt = 8335) itype, tr(it32), tx(it32), emtpc(it32)
8335 format ('+phase', i3, 3e13.4)
     go to 8346
8338 write (unit = kunit6, fmt = 8341) tr(it32), tx(it32), emtpc(it32)
...
47634 if (itrans .gt. 0) go to 21693
  it3 = it + 1
  if (noutpr .eq. 0) write (unit = kunit6, fmt = 54104) tr(it), tx(it), emtpc(it), tr(it3), tx(it3)
54104 format ('+', 5e10.3)
  go to 21694
...
4264 i = it3
  if (it32 .gt. i) i = i + 1
  n3 = i
  if (noutpr .eq. 0) write (unit = kunit6, fmt = 54104) tr(it3), tx(it3), emtpc(it3), tr(n3), tx(n3)
...
170 nr(ibr) = -it
  ...
  read (unit = abuff, fmt = 8331) tr(it), tx(it), emtpc(it)
8371 if (noutpr .eq. 0) write (unit = kunit6, fmt = 54107) tr(it), tx(it), emtpc(it)
...
8680 inonam = inonam + 1
  ...
  namenl(inonam) = n24
  go to 8680
8681 if (iprsup .ge. 1) write (unit = lunit6, fmt = 1644) ntot, ibr, it, inonl, mxphas
1644 format (' Bottom of "over2".    ntot     ibr      it   inonl  mxphas ', /, 19x, 10i8)
  if (iprsup .le. 2) go to 9900
  write (unit = lunit6, fmt = 208) (k, kbus(k), mbus(k), nr(k), kodebr(k), length(k), kodsem(k), indhst(k), litype(k), imodel(k), namebr(k), k = 1, ibr)
208 format (/, " Branch-table integer vectors at end  'over2' .", /, '     row    kbus    mbus      nr  kodebr  length  kodsem indhst  litype  imodel  namebr', /, (11i8))
  write (unit = lunit6, fmt = 209) (k, bus(k), ci(k), ck(k), cik(k), cki(k), ckkjm(k), k = 1, ibr)
...
9200 nchain = 51
  lstat(18) = 2
9900 if (iprsup .ge. 1) write (unit = lunit6, fmt = 9906) ibr, inonl, ntot, kill
...
subroutine fddata (ikf, isfd, ibf)
  ...
  integer(4) :: i, ibk, idk, ifk, igk, ikk, isk
  integer(4) :: ka, kb
...
12 continue
  ...
  isk = isfd + 1
  write (unit = lunit6, fmt = 13) isk, ikk, (rmfd(ka), ka = isk, ikk)
...
4221 iprint = 9
  ...
  read (unit = abuff, fmt = 5432) tr(it), tx(it), emtpc(it), tr(it + 1)
...
73418 if (itype .ne. 96) go to 4741
  ...
  num99 = num99 + 1
  vecnl1(inonl) = emtpc(it)
  if (n3 * n4 .eq. 1) go to 4736
  ichar = ichar + 6
  nonle(inonl) = -ichar
  go to 182
4736 if (noutpr .eq. 0) write (unit = kunit6, fmt = 4738) tr(it), tx(it), emtpc(it)
...
54113 format ('+N.l. inductance, type 93.', 2x, 2e11.3)
  ...
  if (noutpr .eq. 0) write (unit = kunit6, fmt = 54114) tr(it), emtpc(it)
54114 format ('+Time-varying R, type 91.', 3x, 2e11.3)
  if (emtpc(it) .eq. 3333.0d0) go  to 12
  kill = 5
  lstat(19) = 20
  go to 9999
12 nltype(inonl) = 923
...
19 if (itype .ne. 92) go to 80
  if (noutpr .eq. 0) write (unit = kunit6, fmt = 54115)  tr(it), tx(it), emtpc(it)
54115 format ('+Gap, type 92.', 1x, 3e11.3)
  if (tr(it) .ne. 5555.0d0) go to 20
  kill = 28
  lstat(19) = 20
  go to 9999
20 if (emtpc(it) .eq. 4444.0d0 .or. emtpc(it) .eq. 5555.0d0) go to 21
  kill = 5
  lstat(19) = 20
  go to 9999
21 iprint = ichar + 1
...
23 ck1 = -fltinf
  a2 = 0.0d0
  vzero(inonl) = d4
  if (emtpc(it) .eq. 4444.0d0) go to 40
  ! enter zno data code**********************************************
...
73441 if (itype .ne. 96) go to 73442
  ...
  c1 = d2
  ck1 = xlong
  if (iprsup .ge. 4) write (unit = lunit6, fmt = 4232) ichar, inonl, nonlad(inonl), d2, xlong
...
189 if (itype .ne. 96) go to 4758
  ...
  n16 = ichar + n13 + 1
  if (iprsup .ge. 2) write (unit = lunit6, fmt = 4234) inonl, ichar, n11, n13, tr(it), vecnl1(inonl), anonl(inonl)
...
5323 n17 = n16 + n13 + 1
  ...
  if (iprsup .ge. 2) write (unit = lunit6, fmt = 5327) it, n16, n17, d7, d8, anonl(inonl), flzero
...
5353 n17 = n16 + n13 + 1
  ...
  if (iprsup  .ge.  2) write (unit = lunit6, fmt = 5355) n12, n14, n16, n17, d9, anonl(inonl), vzero(inonl), gslope(n16), gslope(n17)
...
269 nonlad(inonl) = nonlad(i)
  ...
  if (emtpc(it) .ne. 3333.) lstat(19) = 269
  go to 73
72 if (itype .ne. 92) go to 100
73 if (emtpc(it) .ne. 4444. .and. emtpc(it) .ne. 5555.) lstat( 19 ) = 269
...
271 iaw = ichar + 1
  ...
  if (emtpc(it) .eq. 0.0) emtpc(it) = 1.0
  vchar(ichar + 11) = emtpc(it)
  if (n3 .ne. 1) go to 4271
  if (n4 .eq. 1) go to 3271
4271 ichar = ichar + 11
  go to 182
3271 nonlad(inonl) = -iaw
  if (noutpr .eq. 0) write (unit = kunit6, fmt = 2271) tr(it), tx(it), emtpc(it)
...
2272 format ('+consts. ', i2, '-', i2, '.', 3e11.3)
  do j = 5, 17, 3
     ...
     n7 = ii + ichar
     read (unit = abuff, fmt = 1272) (cchar(i), i = n6, n7)
274  if (noutpr .eq. 0) write (unit = kunit6, fmt = 2272)  j, ii, (cchar(i), i = n6, n7)
  end do
  ichar = ichar + 18
100 go to 9999
9000 lstat(16) = iprint
  kill = 1
9999 if (iprsup .ge. 3 .or. kill .gt. 0) write (unit = lunit6, fmt = 9998)  kill, inonl, num99, ichar, itype
...
4567 format ('  "Begin module distr2."')
  ...
  lstat(18) = 0
  pdt0 = 0
  if (iprsup .ge. 2) write (unit = lunit6, fmt = 3213) icheck, moldat, ibr, itype
...
6753 n12 = n11 + 2
  ...
  call cimage
  read (unit = abuff, fmt = 6741) (voltbc(ii), ii = n11, n12)
  if (noutpr .eq. 0 ) write (unit = lunit6, fmt = 6755) n11, n12, (voltbc(ii), ii = n11, n12)
...
40004 sconst(nn11) = ysum * xsum
  sconst(jj) = ysum
  i = n12 - jj + 1
  if (ii .eq. 0 .and. noutpr .eq. 0) write (unit = lunit6, fmt = 40005) i, sconst(nn11), sconst(jj)
40005 format (' Z0 equivalence.  I, R * P, P =', i5, 2e20.13)
  if (ii .ne. 0 .and. noutpr .eq. 0) write (unit = lunit6, fmt = 40006) i, sconst(nn11), sconst(jj)
...
6756 ckkjm(ibr) = npa
  ...
  if (noutpr .eq. 0) write (unit = lunit6, fmt = 6759) npa, tauo
...
40022 noutpr = aa
  ...
  if (ifsem .gt. lfsem)  go to 8090
  if (iprsup .ge. 1) write (unit = lunit6, fmt = 4834) ifx, npz, npa, ibr, ifsem
4834 format (' Done Marti cards.  ifx, npz, npa, ibr,ifsem =', 5i6)
21105 icheck = 3
  length(ibr1)=kcount
  if (kcount .gt. mxphas) mxphas = kcount
  if (iprsup .ge. 1) write (unit = lunit6, fmt = 6178) itype, n13, model, ipunch
6178 format (' Below 21105. itype, n13, model, ipunch =', 4i6)
  ...
  litype(ibr) = iaddrs
  if (iprsup .ge. 1) write (unit = lunit6, fmt = 62078) model, iaddrs, ibr1
...
7653 n12 = n11 + 2
  ...
  call cimage
  read (unit = abuff, fmt = 7641) (voltbc(ii), ii = n11, n12)
  if (noutpr .eq. 0) write (unit = lunit6, fmt = 7655) n11, n12, (voltbc(ii), ii = n11, n12)
...
50004 sconst(nn11) = ysum * xsum
  sconst(jj) = ysum
  i = n12 - jj + 1
  if (ii .eq. 0 .and. noutpr .eq. 0) write (unit = lunit6, fmt = 50005) i, sconst(nn11), sconst(jj)
50005 format (' Z0 equivalence.  I, R * P, P =', i5, 2e20.13)
  if (ii .ne. 0 .and. noutpr .eq. 0) write(unit = lunit6, fmt = 50006) i, sconst(nn11), sconst(jj)
...
55000 call cimage
  ...
  go to 9999
7656 ckkjm(ibr) = npa
  if (noutpr .eq. 0) write (unit = lunit6, fmt = 7659) npa, tauo
...
50022 noutpr = aa
  ...
  if (ifsem .gt. lfsem)  go to 8090
  if (iprsup  .ge. 1) write (unit = lunit6, fmt = 8434) ifx, npz, npa, ibr, ifsem
8434 format (' Done Marti cards.  ifx, npz, npa, ibr,ifsem =', 5i6)
20105 icheck = 3
  length(ibr1) = kcount
  if (kcount .gt. mxphas) mxphas = kcount
  if (iprsup .ge. 1) write (unit = lunit6, fmt = 6078) itype, n13, model, ipunch
6078 format (' Below 20105. itype, n13, model, ipunch =', 4i6)
  ...
  model = n13
  litype(ibr) = iaddrs
  if (iprsup .ge. 1) write (unit = lunit6, fmt = 26078) model, iaddrs, ibr1
...
4104 format ('+', 5e10.3)
  if (iprsup .ge. 4) write (unit = lunit6, fmt = 5487) ipunch, kcount, model, n13
...
10010 iadd = iaddrs + 2 * model * model - 1
  ...
  if (iprsup .ge. 1) write (unit = lunit6, fmt = 27693)
27693 format (' at s.n. 10010, ready to read [ti]')
  ...
  lstat(19) = 10010
  go to 9000
9175 write (unit = lunit6, fmt = 9176)
...
2736 format (a4, i2, ',', i2, ')...', 6f6.3)
  ...
  n9 = itranm * itranm
  write (unit = lunit6, fmt = 10003) n13, model, iadd, n9, (qfd(i), i = iaddrs, iadd)
...
9000 lstat(16) = iprint
  kill = 1
9999 if (iprsup .ge. 3) write (unit = lunit6, fmt = 9998) kill, nchain, lstat(18), ibr, ifsem
...
76503 npos2 = 2 * nphcas
  ...
  do i = locz11, locznn
     ...
     emtpc(i) = freqcs * emtpc(i) / (2.0 * freqc)
  end do
...
76711   npos = locatn(mapcas(i), mapcas(j)) + locz11 - 1
        ...
        caslnx(nnpos) = tx(npos) / dsectj + emtpc(npos) * dsectj
...
76433   nnpos = locatn(i + nphcas, j + nphcas)
        caslnr(nnpos) = tr(npos) / dsectj
76432   caslnx(nnpos) = tx(npos) / dsectj + emtpc(npos) * dsectj
     end do
  end do
...
76513 if (msect .le. 0) go to 76505
  ...
  do i = 1, nphcas
     ...
     !     read input card using cimage.
     call cimage
     read (unit = abuff, fmt = 76518) itype, (tr(j), tx(j), emtpc(j), j = istart, iend)
76518 format (i2, 24x, 9e6.2)
     if (i .gt. 1) go to 5264
     if (noutpr .eq. 0) write (unit = kunit6, fmt = 54777) tr(istart), tx(istart), emtpc(istart)
54777 format ('+1st of pi-ckt.', 1x, 3e11.3)
     go to 5271
5264 idumy = istart + 1
     if (noutpr .eq. 0) write (unit = kunit6, fmt = 54104) tr(idumy), tx(idumy), emtpc(idumy), tr(idumy + 1), tx(idumy + 1)
54104 format ('+', 5e10.3)
5271 istart = iend + 1
     idumy = i
76520 idumy = idumy - 3
     ...
     !     read input card using cimage.
     call cimage
     read (unit = abuff, fmt = 76519) (tr(j), tx(j), emtpc(j), j = istart, iend)
76519 format (26x, 9e6.2)
     iendd = iend
     if (idumy .gt. 3) iendd = istart + 1
     if (noutpr .eq. 0) write (unit = kunit6, fmt = 54104) tr(istart), tx(istart), emtpc(istart), tr(istart + 1), tx(istart + 1)
     istart = iend + 1
     go to 76520
76517 end do
  do i = locz11, locznn
...
76633 idumy = idumy - 9
     ...
     write (unit = lunit6, fmt = 76604) (caslnr(ii), ii = istart, iend)
...
6718 if (cshun(jbr) .eq. 0.0) go to 76452
     ...
     if (iprsup .ge. 6) write (unit = lunit6, fmt = 76721) jbr, mbr1, node1(jbr), node2(jbr), trshun(jbr), txshun(jbr), cshun(jbr), freqx, freqc, freqcs, ymag2, yshunr, yshunx
...
76465   caslnx(nnpos) = tx(npos) / dsectj + emtpc(npos) * dsectj + caslnx(nnpos)
     end do
  end do
  if (iprsup .lt. 9) go to 76641
  write (unit = lunit6, fmt = 76605)
  write (unit = lunit6, fmt = 76635)
...
subroutine over4
  ...
  use tracom
  implicit none
  integer(4) :: i, iaddrs, ibf, ibr2, ibr3, iend, ik, ikp, iofa1p, ioftm1, ioftm2
  integer(4) :: j, jfdep2
  integer(4) :: kfdep2, kfin, kflop
  integer(4) :: locmax, locmin
  integer(4) :: mode
  integer(4) :: n, n1, n2, n3, n4, n5, n6, n7, n8, n9, n10, n12, nii, npoint
...
5741 n12 = ntime2 - n5
  ...
  do i = 1, ntime1
     n6 = ioftm1 + i
6121 tim1(n6) = tim1(n6) / 1000000.0d0
  end do
  do i = 1, ntime2
     n7 = ioftm2 + i
6122 tim2(n7) = tim2(n7) / 1000000.0d0
  end do
  d3 = 0.0d0
  do i = 2, ntime1
     n2 = ioftm1 + i
...
6200 a1 = ap(i)
  ...
  kflop = 1
  mode  = 0
  if (iprsup .ge. 1) write (unit = lunit6, fmt = 6213) ifdep, ifdep2, jst, i, ioftm2, iend, dzero, deltat
...
6240 if (i .lt. iend) go to 6250
  ...
  write (unit = lunit6, fmt = 6243) i, iend, jst, mode, kflop, lwt, kfin, n, ifdep, ifdep2, ioftm2, locmin, locmax
6243 format (/, ' End raw data points.       i    iend     jst    mode   kflop     lwt,     kfin       n   ifdep  ifdep2  ioftm2  locmin  locmax  ', /, 21x, 13i8)
  write (unit = lunit6, fmt = 6244)  t1, a1, t, a, d5, d6, tend, d3
...
6260 aditn = aditn + d18
  ...
  d6 = d6 / deltat
  if (iprsup .ge. 4) write (unit = lunit6, fmt = 6316) i, jst, mode, n, t, a, d5, d6
...
6320 d1 = d6 - n * d5
  ...
  if (n .le. 5 .and. iprsup .ge. 2) write (unit = lunit6, fmt = 6323) d1, d2, d3, weight(jst)
...
6465 d1 = 1.0d0 / (d6 / d5 - tails)
  d2 = d1 * d5
  if (iprsup .ge. 2) write (unit = lunit6, fmt = 6466) ifdep2, d2, d1, tails, d5, d6
6466 format (/, ' Tail integral complete.  ifdep2', 14x, 'd2', 14x, 'd1', 11x, 'tails', 14x, 'd5', 14x, 'd6', /, 24x, i8, 5e16.7)
  ...
  if (iprsup .ge. 1) write (unit = lunit6, fmt = 7812) d3, aamp, aditn, con1(ifdep2), d1, tails, ifdep2
...
7910 d1 = d12
  ...
  fder = 1.0d0 / d1 + tails
  d18 = fstar / fder
  if (iprsup .ge. 1) write (unit = lunit6, fmt = 7912) ik, fstar, fder, d18, d1, bct
...
7905 con1(jfdep2) = d1
  ...
  if (iprsup .ge. 1) write (unit = lunit6, fmt = 7913) ifdep2, con1(ifdep2), con1(jfdep2)
...
6600 zinf(ifdep) = z1
  ...
  if (iprsup .ge. 1) write (unit = lunit6, fmt = 6601) ibr, jst, npoint, iskip(ifdep), nr(ibr), nhist(ifdep)
