...
c     Module is called only by installation-dependent "erexit".
 ...
 5244 if ( llbuff .ne. -3333   .and.   ! 2nd or later pass, skip
     1 file6(numcrd+1)(1:4) .ne. 'eof ' ) go to 1708  ! keyboard
 ...
 1324 format (  ' emtp begins.  send (spy, $attach, debug,',
     1          ' help, module, junk, stop) :'  )
...
51329 if ( buff77(1:7) .eq. '$attach' ) go to 1347  ! batch mode
      ...
      write (lunit6, 1330)
 1330 format ( '   send root word to over-ride "junk" for',
     1         ' spy and plot windows :' )
...
41332 if ( buff77(1:6) .eq. 'module' )  go to 2613
      ...
      write (munit6, 1333)
 1333 format ( '    greetings, greetings.  welcome to the',
     1         ' wonderful new world of interactive'     )
      call window         ! output of character variable munit6
      write (munit6, 1334)
 1334 format ( '    emtp execution, observation, and',
     1         ' control.  after sending  "spy",  send'   )
      call window         ! output of character variable munit6
      write (munit6, 1335)
 1335 format ( '    "help",  and then  "all"  to receive',
     1         ' some 500 lines of instruction.  also,'  )
      call window         ! output of character variable munit6
      write (munit6, 1336)
 1336 format ( '    see section 9 of the rule book dated',
     1         ' june, 1984.  also see  "apollo".'       )
      call window         ! output of character variable munit6
      go to 1311
...
c     begin interactive control sequence, leading to "emtspy":
...
c     begin non-interactive variable initialization:
...
c       "$attach,filename,5" usage requires extraction of name:
      lunt13 = 13  ! we will internally connect data to unit 13
      n16 = 0  ! column which begins file name is not yet known
      do 1732  j=9, 40  ! search for 2nd comma in these columns
      if ( buff77(j:j) .ne. ' '  .and.     ! if nonblank column,
     1     n16 .eq. 0 )  n16 = j       ! and 1st, save it in n16
 ...
 1753 continue
      krdoff = numcrd
      krdcom = 0
      do 1758  j=1, limcrd  ! read until an end-of-file detected
...
c     if all emtp data (e.g., "kill codes" use) comes via key
c     board, it is ended with "eof"; when solved, more keyboard.
 ...
 1761 format ( '  & & & & &   input buffer overflow.  limit =',
     1   i6,   '.   reject this data, and reprompt ....'  )
      call window         ! output of character variable munit6
      if ( lunt13 .eq. 13 )  ! if this was $attach usage, then
     1 go to 1311    ! loop back to start anew the data input
      call stoptp   ! installation-dependent program stop card
 1766 numcrd = numcrd + 1
      write (munit6, 1767)  numcrd
 1767 format ( ' done reading disk file into emtp cache.',
     1         '   numcrd =',  i5,  '  cards.'  )
      call window         ! output of character variable munit6
      if ( lunt13 .ne. 5 )  ! if internally-connected file, then
     1 close (unit=lunt13, status='keep')        ! disconnect it
 1774 n22 = 1    ! initialize pass number of $include processing
      n13 = 1   ! 1st $include might be 1st data card (do 1786)
c     begin loop to replace next presently-visible $include :
 1776 n17 = 0     ! switch set so do 1816 loop executed 1st time
      do 1786  j=n13, numcrd  ! search suspect data for $include
...
41777 if ( buff77(1:8) .eq. '$include' ) go to 1787
      if ( buff77(1:7) .ne. '$prefix'   .and.   ! only processes
     1     buff77(1:7) .ne. '$suffix'   .and.   ! these limited
     2     buff77(1:6) .ne. '$level'    .and.   ! dollar cards
     3     buff77(1:6) .ne. '$dummy' )  go to 1786
      k = 8     ! begin searching for file pre/suffix in col. 8
 1778 if ( buff77(k:k) .ne. ' '  .and.    ! if not blank and not
     1     buff77(k:k) .ne. ',' )  go to  1779     ! comma, exit
 ...
 1779 if ( buff77(1:7) .ne. '$prefix'   .and.
     1     buff77(1:7) .ne. '$suffix' )  go to  1783
      l = index ( buff77(k:), ' ' )  ! one col. past end of name
      m = index ( buff77(k:), ',' )      ! col. of comma, if any
      if ( m .ne. 0  .and.   ! if there is a trailing comma, and
     1     m .lt. l )        ! if this precedes the blank, then
     2 l = m       ! it bounds user-supplied name;  bound is "l"
...
31784 format ( 3i1 )   ! 3-digit decimal serialization for dummy
 ...
 1801 if ( buff77(k:k) .eq. ','  .or.  ! if comma or blank, exit
     1     buff77(k:k) .eq. ' ' )  go to 1804   ! with file name
 ...
 1804 if ( nchpre .gt. 0 )    ! if non-blank prefix exists, then
     1 answ80(1:nchpre) = prefix(1:nchpre)  ! prefix begins name
      ...
      if ( nchsuf .gt. 0 )    ! if non-blank suffix exists, then
     1 answ80(n7+1:n8) = suffix(1:nchsuf) ! suffix finishes name
 ...
 1811 if ( n17 .eq. 1 ) go to 1819  ! 2nd or later pass, so skip
      ...
      do 1816  m=j+1, numcrd   ! loop over all data below $incl.
 ...
 4208 n16 = n16 + 1       ! another (the n16-th) argument begins
     ...
     1 call stoptp   ! installation-dependent program stop card
 ...
 4228 read (lunt13, 4232) ( kbeg(l), l=1, 25 )  ! read next card
      do 4229  l=1, 25  ! search i3 replacement fields for blank
...
c     overflow.  199 is max number of replacements, temporarily
      call stoptp   ! installation-dependent program stop card
 ...
 4239 n20 = 0        ! no records read from $include file so far
      ...
      do 4273  k=n18, limcrd   ! read $include records until eof
      ...
      do 4241  l=1, 10       ! see if col. 2 is one of 10 digits
 ...
 4247 j = j + 1       ! accept this data card; do not discard it
      ...
      if ( buff77(1:1) .eq. 'c' )    ! comment cards can have no
     1 go to 4273     ! arguments, so bypass the check for usage
...
34250 if ( n4 .gt. 0 )  go to 4252    ! use argument of $include
 ...
 4251 format ( i3 )
      ...
      if ( n2-n1 .ne. 5 )         ! if name is not 6 characters,
     1 call stoptp   ! installation-dependent program stop card
 ...
 4252 if ( n2-n1 .ne. n3-1   .and.   ! if arg. length mismatch,
     1     ktex(n24) .eq. 1 )        ! & if this is text, then
     2 go to 4253                    ! jump to correction chance
 ...
 4260 format ( a20 )
      if ( arginc(n4)(1:5) .eq. 'stop ' )  ! if user surrenders,
     1 call stoptp   ! installation-dependent program stop card
...
24261 format ( ' argument now processed, arginc(n4) =',  a20 )
      call window         ! output of character variable munit6
34261 do 4263  ip=1, 20  ! process all 20 characters of argument
 ...
 4262 format ( '  next non-blank digit.  ip, n, digit =',
     1   2i6,  '   "',  a1,  1h"  )
      call window         ! output of character variable munit6
54262 if ( char1 .eq. '#' )   ! pounds reserves blank space, so
     1 char1 = ' '            ! it is now blanked as it is used
 ...
 4263 continue           ! end  do 4263  loop over arginc(20:1)
      if ( n+1 .gt. n1  .and.  ! if blank space remains on left,
     1     ktex(n24) .eq. 1 )        ! & if this is text, then
     2 go to 4253   ! allow user correction of erroneous string
      if ( n .ge. n1 )  ! if data field still has space on left,
     1 file6(j)(n1:n) = ' '     ! then blank this remainder out
 ...
 1833 if ( j .gt. n19 )  go to 1760       ! overflow error stop
      do 1835  m=n19, limcrd   ! copy lower cards back as k+1,..
      j = j + 1   ! destination index for next data card copied
      file6(j) = file6(m)
      if ( file6(j)(1:8) .eq. '$include' )  go to 1841
 1835 continue
...
c     following code processes any $spy usage:
 2320 j = 1
      n7 = 0
      n17 = 0    ! so far, no data-sorting requests ("/") found
 2347 if ( j .gt. numcrd )   go to 2415   ! done with $spy cards
      if ( file6(j)(1:8) .ne. '$spy    ' )  go to 2378
      n7 = n7 + 1
      if ( n7 .gt. 9 )        ! if over 9 such $spy usages, then
     1 call stoptp   ! installation-dependent program stop card
 ...
 2353 open (unit=lunt13, status='new', file=ansi32)
      do 2361  k=j+1, numcrd
      if ( file6(k)(1:7) .ne. '$spyend' )  go to 2361
      close (unit=lunt13)
      n24 = k - j
      do 2355  l=k+1, numcrd
 2355 file6(l-n24) = file6(l)
      numcrd = numcrd - n24
...
c     begin code to sort emtp data according to class :
      ...
      if ( iprsup .ge. 9 )
     1 write (lunit6, 2416)  ( j, file6(j), j=1, numcrd )
 2416 format ( ' entire input file as we start sorting ...'
     1    ,/,  ( i5, a80 ) )
 ...
 2418 if ( file6(j)(1:1) .ne. '/' ) go to 2431 ! skip non-"/" card
      ...
      if ( kpsour(n12-1) .eq. 0 )     ! if previous "/" usage is
     1 kpsour(n12-1) = j-1  ! unbounded, then remember last card
 2420 do 2421  k=1, numtyp     ! check for each possible data class name
      if ( file6(j)(2:13) .eq. typdat(k) )  go to 2428   ! yes, found it
 2421 continue           ! end  do 2421  loop to identify name after "/"
      if ( file6(j)(2:10)  .ne.  'tacs data' )  go to 2424
      k = 6         ! set 'tacs data' to 'tacs initial' if
      ntacs = 1     ! new tacs data format used.
      go to 2428    !  ntacs set to 1 if tacs with new format
 2424 if ( munit4 .ne. 1 )    ! if execution is not interactive,
     1 call stoptp   ! installation-dependent program stop card
      write (munit6, 1737)    ! tell user to send corrected name
      call window         ! output of character variable munit6
      read (munit5, 1329)  file6(j)    ! read corrected "/" card
 ...
 2431 if ( n12 .ge. 1  .and.    ! if 1 or more "/" found so far,
     1     kpsour(n12) .eq. 0 .and.  ! & last "/" not yet ended,
     2     file6(j)(1:6) .eq. 'blank ' )    ! and card is blank,
     3     kpsour(n12) = j - 1       ! then bound last "/" usage
 2436 continue      ! end  do 2436  loop which sets up summary "/" table
 ...
 2442 format ( 4i8 )
      call window         ! output of character variable munit6
 2443 continue   ! end  do 2443  loop over
...
12448 if ( kode(l) .gt. 3 )  go to 12458
      do 12456  ll = nn1, nn2    ! for tacs sources, convert
      if ( file6(ll)(1:1) .eq. '$' .or. file6(ll)(1:1) .eq. '9'
     1    .or.  file6(ll)(1:1) .eq. 'c' )  go to 12456
...
12480 continue                ! initial condition cards
 ...
 2445 if ( file6(j+1)(1:16) .ne. 'begin new data c' ) go to 2446
      n24 = n24 + 1    ! increment destination address past this
 ...
 2464 format (  ' blank card recognized.  j =',  i6 )
      call window         ! output of character variable munit6
 2465 do 2466  k=1, numtyp    ! search class table to identify data type
      n8 = lentyp(k)   ! number of characters in k-th key word
      n14 = index ( file6(j), typdat(k)(1:n8) ) ! search for key word k
...
c     convert end of class blank card in old tacs data to comment card
      if ( k .eq. 2  .or. k .eq. 3  .or. k .eq. 4  .or.
     1     k .eq. 5  )    file6(j)(1:2) = 'c '
 ...
 2474 format ( ' data class needs consideration.  n8 =',  i6 )
      call window         ! output of character variable munit6
 2475 do 2484  k=1, n12   ! search rows of "/" table for class number n8
      ...
      do 2477  l=n5, n6   ! loop over each active card of k-th "/" usage
 ...
 2484 continue    ! end  do 2484  loop over all entries "k" in "/" table
      if ( n8 .eq. 10  .or.  ! if just done with "/load flow" or
     1     n8 .eq. 11 ) go to 2456  ! "/initial", special return
 ...
 2500 j = 0         ! initialize card destination index for upward shift
      do 2505  k=numcrd+1, n24    ! consider each lower-assembled card k
 ...
 2508 format (  ' done with all "/" processing.  numcrd =', i8 )
      call window         ! output of character variable munit6
 2509 if ( m4plot .eq. 1 )    ! ?????  if interactive use,
     1 call spying    !  ????????   temporary diagnostic  ??????
      go to 9800         ! exit module after possible diagnostic
c     begin code to process "module" of user.  convert user data
c     from  "arg", "dum", and "num" character declarations to
c     the vectors kard/karg/kbeg/kend/ktex as needed by $include
 ...
 2628 n13 = n13 + 1         ! next input card (natural order) considered
      ...
      if ( buff77(1:3) .ne. 'arg'  .and.            ! skip over any card
     1     buff77(1:3) .ne. 'dum'  .and.            ! not "arg" or "dum"
     2     buff77(1:3) .ne. 'num' )  go to 2621     ! or "num"
      k = 4
      n8 = n8 + 1       ! one more declaration pushed to top of vector
      file6(n8) = buff77      ! store declaration after last such card
 2637 if ( buff77(k:k) .ne. ' '  .and.       ! start of next argument is
     1     buff77(k:k) .ne. ',' )  go to 2648        ! non-"," & non-" "
 ...
 2656 if ( buff77(l:l) .eq. ' '  .or.           ! blank or comma bounds
     1     buff77(l:l) .eq. ',' )  go to 2664   ! right edge of argument
 ...
 2664 l = l - 1    ! right edge of argument is one byte to left of bound
      ...
      do 2672  m=1, numarg  ! search existing argument list for this one
 ...
 2679 format (  '   ???  illegal "num" declaration.  unrecognized',
     1          ' name = ',  a )
      call window         ! output of character variable munit6
      call stoptp   ! installation-dependent program stop card
 ...
 2689 format ( ' overflow error stop.  argument usage is',
     1         ' limited in number to limarg =',  i5  )
      call window         ! output of character variable munit6
      call stoptp   ! installation-dependent program stop card
 ...
 2705 format ( 2a4, 2x, 2a4 )
      ...
      do 2708  j=1, n8   ! loop over all declaration, now stored at top
 ...
 2716 format ( ' done processing declarations.  n8, n11,',
     1         ' limcrd =',  3i5  )
      call window         ! output of character variable munit6
      write (munit6, 2717)
 2717 format ( '     row  kolinc  kkkdum  modarg  arginc ....' )
      call window         ! output of character variable munit6
      do 2719  j=1, numarg  ! print each row j of argument table
      write (munit6, 2718)  j, kolinc(j), kkkdum(j),
     1                         modarg(j), arginc(j)
 2718 format ( 4i8, a20 )
      call window         ! output of character variable munit6
 ...
 2721 n20 = 0        ! initialize number of argument usages found so far
      ...
      do 2766  n17 = n11, limcrd       ! process all non-declation cards
      ...
      do 2754  j=1, numarg        ! check present card for each argument
 ...
 2759 format ( ' done with this card.  n16, n20 =', 2i5,
     1         '   buff77=',  a80 )
      call window         ! output of character variable munit6
 ...
 2777 format ( ' done with identifying all arguments of all cards.',
     1         '   n20 =',  i4 )
      call window         ! output of character variable munit6
 2778 if ( (n20/25)*25 .ne. n20 )  go to 2783   ! 25i3 cards not filled
c     extra zero entry must be added, since otherwise card is full:
 ...
 2791 format ( a4, 25i3 ,/, (4x, 25i3) )
      do 2795  j=n11, limcrd           ! next, dump all input data cards
 ...
 9800 if ( ntacs .ne. 2 )  go to 9002
      ...
      do 3500  j = 1, numcrd
      if ( file6(j)(1:5) .ne. 'blank'
     1  .and. file6(j)(1:80) .ne. blan80(1:80) )   go to 3005
 ...
 3010 if (file6(j)(1:2) .eq. '99')   !convert type code '99' to
     1    file6(j)(1:2) = '  '       !'  ' for functions
      go to 3500
 3020 if ( file6(j)(1:1) .eq. 'c' .or. file6(j)(1:1) .eq. '9'
     1   .or. file6(j)(1:1) .eq. '$') go to 3500
