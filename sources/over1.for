...
6438 format (' *********  begin "m40." emtp solution.', '   size /label/ =', i7, '  integer words.   ', /, &
  ...
  go to 15
6452 write(lunit6, 83044)  locker
83044 format ( ' associated user documentation is the 864-page', 'EMTP rule book dated  June, 1984.', 3x, &
...
83047 format(' descriptive interpretation of new-case input data 1 input data card images printed below, all 80 columns, &
           character by character.' ,/, 51x, 1h0,  8(9x, i1))
  j = 0
  write (lunit6, 83048)  ( j, i=1, 8 )
83048 format ( 51x, 1h0,  8(9x, i1)  )
  write(lunit6, 83049)
83049 format (' --------------------------------------------------', '+', '----------------------------------------', &
           '----------------------------------------')
...
15 call cimage
  ...
  n9 = kolbeg
  kolbeg = 1
  read (unit = abuff(1), fmt = 3246) texcol
3246 format (80a1)
  call freone(d1)
3247 nright = 0
...
3270 format(31h+blank termination-of-run card.   )
  call interp
  ivolt = 7777
3273 kill = 9999
  nchain = 31
...
!     $$$$$    special-request word no. 15.   'start again'       $$$$$
 ...
 2857 format (40h+continue partially-completed data case. )
...
2882  format (  37h+terminator for switch closing times.   )
      read (lunit2)  locker
      do j = 1, 9999
...
7020  format(36h+set absolute tacs list size limits.   )
      !     read input card using cimage
      call cimage
...
7060  format(30h+1st tacs dimensions card.     , 3i6)
      go to 15
...
4205  format ( 13h+misc. data.   , 3e12.3  )
...
4210  format ( 13h+misc. data.  , 2i5, 8i3 )
      begmax(1) = maxout
      maxout = 2
...
630   format(17h+statistics data.,3i8, f9.4 )
...
6255  format(' ----- Warning. Nonzero misc. data ', ' parameter "xopt" differs from the ', ' power frequency of ',  f8.2, &
           ' . This is unusual.', /, 7x, 'a value of ', e13.4, ' was read from columns 17-24 of the data card just read. Execution &
           will continue using   ', /, 7x, 'this value, as suspicious as it seems to the EMTP.')
6260  xopt = d1
...
6256  format(' ----- Warning. Nonzero misc. data', ' parameter "copt" differs from the', ' power frequency of', f8.2, &
           ' .   This is unusual.         ', /, 7x,  'A value of', e13.4, ' was read from columns 25-32 of the data &
           card just read. Execution will continue using   ', /, 7x, 'this value, as suspicious as it seems to the EMTP.')
6265  copt = d2
...
4258  format( 10x, 6hdeltat, 11x, 4htmax, 11x,4hxopt, 11x, 4hcopt, 9x, 6hepsiln, 9x, 6htolmat, /, 1x, 6e15.5 , /,1x)
...
32699 format ( 45h+tacs hybrid setup.  tacs data cards follow.  )
      read (unit=abuff(1), fmt=1984) lstat(52)
1984  format ( 18x, i2 )
      go to 2691
22699 if ( noutpr  .eq.  0 ) write (kunit6, 42699)
42699 format ( 45h+tacs stand-alone setup.  data cards follow.  )
      read (unit=abuff(1), fmt=1984) lstat(52)
      go to 2691
...
4568  format ( 23h "exit  module over1." )
99999 return
...
4567  format ( 24h  "begin module tacs1c." )
...
6565  end do
      write ( lunit6, 1313 )
1313  format ( 49h $$$ no such sourcs, the card will be ignored $$$ )
      go to 2868
...
6822  if (dum3 .ne. 0. .or. ijk .lt. 0) ud1(ndy5+3) = dum3
 ...
 1515 format (  34h another tacs source changing card  )
2868  return
...
1000  if ( iprsup  .ge.  1 ) write ( lunit6, 4567 )
4567  format ( 23h begin module "swmodf". )
...
3131  format ( 43h no such switch, the card will be discarded )
      go to 209
3510  if ( it2 .ne. 0  .or.  kswtyp(msw) .ne. 0 )  go to 209
      if ( bus4  .ne.  text14 )  go to 7218
      if ( noutpr  .eq.  0 )  write ( kunit6, 1218 )
1218  format ( 45h+permanently-closed switch used for metering.   )
      gus3 = -1.0
      gus4 = fltinf
      go to 216
...
4568  format ( 25h  exit  module "swmodf." )
99999 return
...
!
    subroutine reques
      ...
      data  textay(1)   /  6hx       /
      data  jpntr(1)    /  1  /
      data  textax(1)    /  6hxforme  /
      data  textax(2)    /  6hr       /
      !     $$$$$    special-request word no. 2.   'saturation'        $$$$$
      data  textay(2)   /  6hs       /
      data  jpntr(2)    /  3  /
      data  textax(3)    /  6hsatura  /
      data  textax(4)    /  6htion    /
      !     $$$$$    special-request word no. 3.   'type99 limit'      $$$$$
      data  textay(3)   /  6htl       /
      data  jpntr(3)    /  5  /
      data  textax(5)    /  6htype99  /
      data  textax(6)    /  6hlimit   /
      !     $$$$$    special-request word no. 4.   'replot'            $$$$$
      data  textay(4)   /  6hr       /
      data  jpntr(4)    /  7  /
      data  textax(7)    /  6hreplot  /
      !     $$$$$    special-request word no. 5.   'begin new data case' $$$
      data  textay(5)   /  6hbndc    /
      data  jpntr(5)    /  8  /
      data  textax(8)    /  6hbegin   /
      data  textax(9)    /  6hnew     /
      data  textax(10)   /  6hdata    /
      data  textax(11)   /  6hcase    /
      !     $$$$$    special-request word no. 6.   'line constants'    $$$$$
      data  textay(6)   /  6hl!      /
      data  jpntr(6)    /  12  /
      data  textax(12)   /  6hline    /
      data  textax(13)   /  6hconsta  /
      data  textax(14)   /  6hnts     /
      !     $$$$$    special-request word no. 7.   'postprocess
      !     plot file'       $$$$$  m28. 927
      data  textay(7)   /  6hppf     /
      data  jpntr(7)    /  15  /
      data  textax(15)   /  6hpostpr  /
      ...
      data  textay(56)   /  6hfdu     /
      data  jpntr(56)    /  191  /
      data  textax(191)  /  6hfault   /
      data  textax(192)  /  6hdata    /
      data  textax(193)  /  6husage   /
      !     $$$$$$$  special request-word no. 57.   'fix source'  $$$$$$
      data  textay(57)   /  6hfxs     /
      data  jpntr(57)    /  194       /
      data  textax(194)  /  6hfix     /
      data  textax(195)  /  6hsource  /
      !     $$$$$$$  special request-word no. 58.   'user supplied     $$$$$$
      !     $$$$$$$                                  switch times'     $$$$$$
      data  textay(58)   /  6husst    /
      data  jpntr(58)    /  196  /
      data  textax(196)  /  6huser    /
      data  textax(197)  /  6hsuppli  /
      data  textax(198)  /  6hed      /
      data  textax(199)  /  6hswitch  /
      data  textax(200)  /  6htimes   /
      !     $$$$$$$  special request-word no. 59.   'ametani setup'  $$$
      data  textay(59)   /  6has      /
      data  jpntr(59)    /  201       /
      data  textax(201)  /  6hametan  /
      data  textax(202)  /  6hi       /
      data  textax(203)  /  6hsetup   /
      !     $$$$$$$  special request-word no. 60.   'hauer setup'  $$$$$
      data  textay(60)   /  6hhs      /
      data  jpntr(60)    /  204       /
      data  textax(204)  /  6hhauer   /
      data  textax(205)  /  6hsetup   /
...
83056 format(  50h+request for transformer impedance-matrix routine.   )
      nchain = 41
      go to 5617
      !     $$$$$    special-request word no. 2.   'saturation'        $$$$$
8002  if ( noutpr  .eq.  0 ) write (kunit6, 83057)
83057 format(  41h+request for magnetic-saturation routine.  )
      nchain = 42
      go to 5617
      !     $$$$$    special-request word no. 3.   'type99 limit'      $$$$$
...
3364  format (  34h+request to re-plot old plot data.    )
      degmax = 0.0
      ialter = lunit4
      call midov1
      call pfatch
...
4154  format(  37h+marker card preceding new data case.    )
      go to 15
      !     $$$$$    special-request word no. 6.   'line constants'    $$$$$
8006  nchain = 44
      if ( iprsup .ge. 1 )  write (lunit6, 3411)
3411  format ( 36h set nchain=44 for "line constants".   )
      go to 5617
      !     $$$$$    special-request word no. 7.   'postprocess
      !     plot file'       $$$$$  m28.1317
...
2614  format ( 32h+new plotter paper-height limit.   , 2x, e13.3 )
      go to 15
      !     $$$$$    special-request word no. 9.   'printer lines per inch'
...
2634  format (  38h+new printer spacing, lines/distance =, 2x, i8 )
      go to 15
      !     $$$$$    special-request word no. 10.   'mode voltage output' $$$
...
2648  format ( 39h+request for tricky karrenbauer output,   , i3, 8h  modes.    )
      go to 15
      !     $$$$$    special-request word no. 11.   'end last data case' $$$$
8011  if ( noutpr  .eq.  0 ) write (kunit6, 2654)
2654  format (  40h+special terminator record for all data.    )
      go to 15
      !     $$$$$    special-request word no. 12.   'analytic sources usage'
8012  if ( noutpr  .eq.  0 ) write (kunit6, 2667)
2667  format (  47h+request for use of analytic type 1-10 sources.   )
      kanal = 1
      go to 15
      !     $$$$$ special-request word no. 13.   'limit on plot oscillations'
...
2675  format ( 37h+change successive oscillation limit., 2x, i8 )
      go to 15
      !     $$$$$    special-request word no. 14.   'tacs emtp sources'  $$$$
8014  if ( noutpr  .eq.  0 ) write (kunit6, 2682)
2682  format (  47h+tacs names controlling type 1-10 emtp sources.     )
      if ( kolbeg  .gt.  0 )   go to 2683
      read (unit=abuff(1), fmt=2685) (vstacs(j), j=1, 10)
...
2705  format(43h+request for semlyen step-response routine.)
      nchain = 45
      go to 5617
      !     $$$$  special request-word no. 17.   'linear bias usage' $$$$$$$$$
8017  if ( noutpr .eq. 0 ) write (lunit6, 8719)
8719  format ( 40h+ramped linear variation of random bias.   )
      linsys = 1
      go to 15
      !     $$$$$    special-request word no. 18.   'cable constants'   $$$$$
8018  continue
...
2726  format ( 39h+transfer to "cable constants".  type =, i6  )
      nchain = 47
      go to 5617
      !     $$$$$    special-request word no. 19.   'auto name'         $$$$$
...
2743  format ( 40h+toggle branch naming option.   nmauto =, i8 )
      go to 15
      !     $$$$$    special-request word no. 20.   'renumber bypass'   $$$$$
8020  if ( noutpr  .eq.  0 ) write (kunit6, 2758)
2758  format (  41h+bypass of transient network renumbering.   )
...
2780  format ( 8h+f-scan.,  3e12.3,  i5  )
      go to 2781
...
8023  if ( kolbeg  .gt.  0 )   go to 2814
      read (unit=abuff(1), fmt=2811) (iprsov(i), i=1, 30)
...
2813  format ( 27h+diagnostic printout codes.,  5i4  )
      go to 15
      !     $$$$$    special-request word no. 24.   'power frequency'   $$$$$
...
2824  format ( 28h+redefined power frequency =,  e12.3,  5h  hz.   )
      go to 15
      !     $$$$$    special-request word no. 25.   'file request'      $$$$$
8025  if ( noutpr  .eq.  0 ) write (kunit6, 4654)
4654  format (  27h+call subroutine "midov1" .  )
      call midov1
      go to 15
      !     $$$$$    special-request word no. 26.   'user identification' $$$
...
4667  format ( 21h+user identification.,  1x,  4a6  )
      go to 15
      !     $$$$$    special-request word no. 27.   'convert zno'  $$$$$$$$$$
8027  iofbnd = 99876
      nchain = 42
      if ( noutpr  .eq.  0 ) write (kunit6, 4685)
4685  format (  48h+request to convert old zno data to new formats. )
      go to 5617
      !     $$$$$    special-request word no. 28.   'abort data case'   $$$$$
8028  if ( noutpr  .eq.  0 ) write (kunit6, 4695)
4695  format (  33h+request to abort this data case.   )
      l = 0
      !     read input card using cimage
4699  call cimage
      n9 = kolbeg
...
4713  format (  25h+   discarded card number,  i5, 19h   of skipped case.     )
      go to 4699
      !     $$$$$    special-request word no. 29.   'kill codes'        $$$$$
8029  ipntv(1) = -8888
...
4728  format ( 27h+listing of error messages.,  2i8  )
      ipntv(3) = kill
      write (lunit6, 4733)  kill
4733  format ( /,  28h message of kill-code number,  i4,  1h.  )
      bus1 = trash
      bus2 = trash
      bus3 = trash
      bus4 = trash
...
4739  format ( 35h+exponent of high resistance.   r =,  e12.2  )
...
7010  format ( 36h+request to average (smooth) output.   )
      go to 15
      !     $$$$$  special request-word no. 32.  'absolute tacs dimensions'  $
      !     $$$$$  special request-word no. 33.  'relative tacs dimensions'  $
...
7183  format ( 40h+request for  'statistics'  termination.  )
      nenerg = intinf
      go to 15
      !     $$ special request-word no. 35.  'statistics output salvage'
...
7199  if ( noutpr  .eq.  0 ) write (kunit6, 7200)  jflsos
 7200 format ( 38h+disk storage of energization results., i8 )
      go to 15
!     $$$$  special request-word no. 36.   'omit base case'    $$$$$$$$$
 8036 if ( noutpr  .eq.  0 ) write (kunit6, 7203)
7203  format (  46h+omit base case if statistics/systematic case.   )
      kbase = intinf
      go to 15
      !     $$$$  special request-word no. 37.   'change switch'     $$$$$$$$$
8037  if ( noutpr .eq. 0 ) write (kunit6, 7206)
7206  format (  42h+convert switched-l,r to pseudo-nonlinear. )
      iofbnd = 33666
      go to 15
      !     $$$$$$$  special request-word no. 38.   'miscellaneous data
      !     cards'          $$$$$$$$$m28.1692
8038  ifdep = -5555
      write (kunit6, 7212)
7212  format (  40h+request record before misc. data cards.  )
      go to 15
      !     $$$$$$$  special request-word no. 39.   'redefine tolerance
      !     epsiln'         $$$$$$$$$m28.1698
...
7226  format ( 32h+misc. data constant  'epsiln' .,  e12.3  )
      go to 15
      !     $$$$$$$  special request-word no. 40.   'change printout
      !     frequency       $$$$$$$$$m28.1708
8040  if ( noutpr  .eq.  0 ) write (kunit6, 7234)
7234  format (  44h+request record before printout frequencies.   )
      ktref = -7777
      go to 15
      !     $$$$$$$  special request-word no. 41.   'begin peak
...
7252  format ( 24h+extrema calc. begins at,  e13.4,  10h  seconds.  )
...
7268  if ( n13 .gt. 0 )  maxzno = n13
 ...
 7270 format ( 11h+zno const.,  i4,  3e11.3 )
      go to 15
      !     $$$$$$$  special request-word no. 44.   'peak voltage'  $$$$$$$$$$
      !     $$$$$$$                                  monitor'      $$$$$$$$$$$
8044  peaknd(1) = flzero
      if ( noutpr  .eq.  0 ) write (kunit6, 7273)
7273  format ( 35h+overall problem peak node voltage.   )
      go to 15
      !     $$$$$$$  special request-word no. 45.   'absolute u.m.  $$$$$$$$$$
      !     dimensions'    $$$$$$$$$$m28.1751
...
7284  if ( noutpr  .eq.  0 ) write (kunit6, 7287) nclfix, numfix, iotfix, ibsfix
 7287 format ( 18h+u.m. table sizes.,  4i6  )
      go to 15
!     $$$$$$$  special request-word no. 46.   'relative u.m.  $$$$$$$$$$
!     dimensions'    $$$$$$$$$$m28.1773
...
7309  format ( 20h+derived u.m. sizes.,  4i6  )
      go to 15
      !     $$$$$$$  special request-word no. 47.   'time step loop'  $$$$$$$$
8047  if ( noutpr  .eq.  0 ) write (kunit6, 7314)
7314  format ( 36h+transfer control to time-step loop.    )
      nchain = 16
      t = flstat(14)
...
7331  format ( 22h+deltat-loop printout.,  4i6  )
      go to 15
      !     $$$$$$$  special request-word no. 49.   'tacs warn limit' $$$$$$$$
...
7336  format ( 32h+warning controls.  lim, t-beg =, i6, e10.2 )
      go to 15
      !     $$$$$    special-request word no. 50.   'marti setup'       $$$$$
8050  continue
      !     $$$$$    special-request word no. 51.   'jmarti setup' $$$$$$$$$$
...
7359  format ( 35h+non-std. choice of disk plot file., 11h   m4plot =,  i2  )
      go to 15
      !     $$$$$$$  special request-word no. 53.   'output width 132' $$$$$$
8053  if ( noutpr  .eq.  0 ) write (kunit6, 7364)
 7364 format ( 36h+use full-width (132-col.) printout.   )
      kol132 = 132
      go to 15
      !     $$$$$$$  special request-word no. 54.   'output width 80'  $$$$$$
8054  if ( noutpr  .eq.  0 ) write (kunit6, 7368)
7368  format ( 31h+use narrow (80-col.) printout.    )
      kol132 = 80
      go to 15
      !     $$$$$$$  special request-word no. 55.   'modify switch logic  $$$
8055  n7 = ktrlsw(6) + 1
...
7374  format ( 40h+request for altered logic.  ktrlsw(6) =, i2 )
      go to 15
      !     $$$$$$$  special request-word no. 56.   'fault data usage' $$$$$$
...
7385  format (  35h+request for generator equivalents.  )
      istep = -6633
      nchain = 29
      go to 5617
      !     $$$$$$$  special request-word no. 57.   'fix source'  $$$$$$
...
7388  format ( 38h+declaration of desired load flow use.    )
      go to 15
      !     $$$$$$$  special request-word no. 58.   'user supplied     $$$$$$
      !     $$$$$$$                                  switch times'     $$$$$$
8058  if ( kolbeg  .gt.  0 )   go to 7396
 ...
 7406 format ( 40h+file of random switching times.  unit =, i6  )
      go to 15
!     $$$$$$$  special request-word no. 59.   'ametani setup'  $$$
 8059 if ( noutpr  .eq.  0 ) write (lunit6, 7411)
 7411 format ( 43h+request for ametani step-response routine.   )
      nchain = 46
      go to 5617
!     $$$$$$$  special request-word no. 60.   'hauer setup'  $$$$$
8060  if ( noutpr  .eq.  0 ) write (lunit6,7418)
7418  format ( 44h+request for hauer impulse-response routine.   )
      nchain = 48
      nenerg = 49
      go to 5617
      !     $$$$$$$  special request-word no. 61.   'line model freq. scan  $$
...
9201  format ( /,  17h exit  "reques" ., 32h  nchain  lunit6 lstat18    kill ,/,  17x,  4i8  )
      if ( iprsup  .ge.  1 ) write ( lunit6, 4568 )
4568  format ( 24h  "exit  module reques." )
      return
...
!
    subroutine sysdep
      ...
      character*1 lettra, lettrb, lettrc, colxxx(18)
...
140   continue
      ...
      !     open (unit=lunit2, type='scratch', form='formatted',
      !     1      status='delete' )
      !     open ( unit=lunit3, type='scratch', form='unformatted',
      !     1      status='delete' )
      !     unit 8 is calcomp plot file (unused with our vax-versatem24. 367
      !     connection )                                            m24. 368
...
6301  format (  ' electromagnetic transients program (emtp),', ' digital (dec) vax-11/780 translation as', &
           ' used by bpa in portland, oregon  97208;', '  usa.'  )
      read (unit=date1(1), fmt=3641) col(25), col(6), col(7)
 3641 format (2a1, 1x, a1)
      if ( col(25)  .eq.  '0' ) go to 3642
      if ( col(6)   .eq.  '0' ) col(6) = lettra
      if ( col(6)   .eq.  '1' ) col(6) = lettrb
      if ( col(6)   .eq.  '2' ) col(6) = lettrc
 3642 continue
      read (unit=date1(2), fmt=3641) col(8)
      read (unit=tclock(1), fmt=3652) (col(j), j=9, 14)
 3652 format (2a1, 1x, a1, 4x, a1, 1x, 2a1 )
      write (ansi32, 3668)  ( col(j), j=1, 18 )
...
!     following zero definition is redundant.
      ...
      userid = blank
      if ( noutpr  .eq.  0 ) write (lunit6, 6305)  date1, tclock, (col(j), j=6,18)
 6305 format ( 45h date (mm/dd/yy) and time of day (hh.mm.ss.)=, 1x, 2a4, 2x, 2a4,  11x,  'name of vax/vms plot data', &
           ' file (if any) =',  13a1 )
...
1483  format (   '   ****  ****   data crisis.   last', ' card has been read.   numcrd =', i6 ,/, &
           '                use "data" command of', ' spy to read in next block of data.'  )
...
1488  read (file6(n7), 1489)  ( abuff(j), j=1, 10 )
1489  format ( 10a8 )
...
5914  format (2hst, i1, 3hlog, 3i1, 4h.dat, 3x )
...
4538  end do                  ! end  do 4538  loop to find non-blank left edge
...
!
    subroutine tacs1
      ...
      data  sbn(2)    /  6hden.   /
      if (iprsup .ge. 1) write (lunit6, 4567)
 4567 format (23h  "begin module tacs1." )
...
703   format (   18h  use cells  n1 = ,  i8 ,/, 11h  kisblk = ,i5,  11h  krsblk = ,i5,  11h  kksus  = ,i5, &
           11h  kalksu = ,i5,  11h  kiuty  = ,i5,  11h  kud1   = ,i5, 11h  kinsup = ,i5,  11h  kivarb = ,i5 ,/, &
           11h  kprsup = ,i5,  11h  kawkcs = ,i5,  11h  kxar   = ,i5, 11h  kxtcs  = ,i5,  11h  klntab = ,i5,  11h  katcs  = ,i5, &
           11h  kcolcs = ,i5,  11h  kjout  = ,i5, /  )
7811  lstat(39) = n1
      koncur = 0
...
152   format (  42h+request for output of all tacs variables.   )
      go to 100
1399  if ( n  .ne.  0 )   go to 139
      if ( noutpr  .eq.  0 )  write ( kunit6, 137 )
137   format (  45h+blank card terminating all tacs data cards .   )
      go to 101
139   if ( dum(1)  .eq.  0.0 )  dum(1) = 1.0
      if ( noutpr  .eq.  0 ) write (kunit6, 179)  alnode, n, dum(1)
 179  format ( 16h+tacs function ', a6, 8h', order, i2, 1h., e14.4  )
...
7413  end do
      ...
      lstat(19) = 7420
      write (lunit6,3)
 3    format ( /,  34h error.  no input node specified.   )
      go to 9000
...
101   konsce = niu
 ...
 1998 format ( 7h nuk = , i4 ,/, 10x, 27h alksx    ksj   kfst    kni  )
      nuki = kisblk - 8
      do n = 1, nuk
         nuki = nuki + 8
...
2005  format ( 8h0 nsu = , i3  ,/, 8x, 11hksu    ksus  ,/, ( 1x, i3, 1x, 2i6 )  )
2315  write (lunit6,2006) niu
2006  format ( 8h0 niu = ,i3 ,/, 8x,11haliu   iuty, 7x, 3hud1, 12x, 3hud2, 12x, 3hud3, 12x, 3hud4, 12x, 3hud5 )
      ndx5 = kud1 - 5
      do n = 1, niu
         n1 = ilntab(kaliu+n)
...
2007  format (7h0 nsup=, i6, /, 8x, 24h  supvar    jsup    ksup, /, ( 4i8 ) )
...
2009  format (7h  kpar=, i8, 6x, 18hparsup as follows: ,/,(8e16.6))
      n22 = nsudv + 1
      if ( nsudv .lt. lstat(63) ) write (lunit6,10031) nsudv, ( n, ksus(kalksu+n), ksus(kksus+n), n = n22,  lstat(63))
10031 format (8h0 nsudv=, i4/12x,13halksu    ksus/(1x, 3i8))
      go to  4444
...
6537  niu = niu + 1
 ...
 170  format (  13h+tacs source.  ,   3e12.3  )
...
168   format (18h  type 90 source ",a6,23h" is not a recognizable, 45h node name in emtp. the card will be discard  )
      go to 8579
...
55168 format (19h  source 90 or 91 ",a6,23h" is not a recognizable, 52h switch node name in emtp. the card will be discard  )
8579  niu  = niu  - 1
      ndy5 = ndy5 - 5
      go to 9000
1092  if ( n1  .ne.  92 )   go to 8579
      write ( lunit6, 5566 )
5566  format ( 46h  ----- this card will be ignored, because #92, 33h source cannot be changed   ----  )
      go to  8579
479   ud1(ndy5+2) = j
      go to 9000
8107  kill = 122
...
154   format (  39h+tacs variables for emtp output vector.   )
      go to 100
 4466 lstat(19) = 1199
 9300 kill = 122
      lstat(16) = lstat(68)
 ...
 6567 if ( alnode  .ne.  blank )   go to 3838
      if ( noutpr .eq. 0 ) write ( kunit6, 4455 )
 4455 format ( 24h+ignor the illegal card. )
      go to 100
 3838 kxic = kxic + 1
 ...
 4195 format (14h+init. cond. ', a6, 1h', e16.6)
      go to 100
 113  call tacs1a
...
4568  format ( 23h  "exit  module tacs1." )
      return
...
!
    subroutine tacs1a
      ...
      data  text1   /  6hvalue   /
      if ( iprsup  .ge.  1 ) write ( lunit6, 4567 )
4567  format ( 24h  "begin module tacs1a." )
...
12516 format ( 10x, a1)
 ...
 1251 format ( 38h+tacs supplemental fortran expression  )
...
12517 if ( noutpr .eq. 0 )  write ( kunit6, 9304 )
 9304 format ( 28h+tacs supplemental variable  )
...
19305 format ( 31h+tacs supplemental device type , i2 )
...
8841  format ( 30h test delay line i.c., value =,  a6  )
      do i1 = kpar, n6
         ndx6 = kprsup + i1
         parsup(ndx6) = dum(1)
      end do
...
10715 format ( 18h+data values.             ,  2e13.4)
      kpar = kpar + 1
      ndx1 = kprsup +  kpar
      parsup(ndx1) = prx
...
10716 format ( 41h+end of data values for last-read device.   )
...
6799  if ( noutpr .eq. 0 ) write ( kunit6, 1104 )
 1104 format ( 22h+no such sup. device.  )
9000  if ( iprsup  .ge.  1 ) write ( lunit6, 4568 )
4568  format ( 24h  "exit  module tacs1a." )
      return
...
!
    subroutine tacs1b
      ...
      data  delay   / 6hdelay  /
      if ( iprsup  .ge.  1 ) write ( lunit6, 4567 )
4567  format ( 24h  "begin module tacs1b." )
      ll0 = 0
      ll1 = 1
      ll2 = 2
      ll3 = 3
...
1980  end do
 ...
 777  nukr = krsblk + nuk * 4
      write ( lunit6,  7171 )
 7171 format ( 47h --------- more than one limiter in the zs loop, 44h insert a delay s-block to decouple the loop  )
      nuki = kisblk - 4
      do lc = 1, nuk
         nuki = nuki + 8
...
1001     ilntab(ndx1-i) = ilntab(ndx2-i)
 ...
 8844 format ( 18h  $$$ ilntab $$$ =,/, ( 5x, 15i8 ) )
...
9200  kill = 500
      write ( lunit6, 9202 )
9202  format (  43h  the case will be killed because there are , 50h some loops in supplemental variables or devices. ,/, &
           49h  this is not allowed.    the looped supplemental, 25h  blocks are as follows :   )
      j = 0
      do i = 1, nsup
...
9204     format ( 10h  ********, 2x, i5, 4x, a6,  10h  ******** )
9208  end do
      go to 9000
21100 do i = 1, nsu
         ndx3 = kalksu + i
...
99          format (24h error.  name of limit  , a6, 13h  at block = , a6, 20h  is unidentifiable. )
            go to 9000
...
10    end do
      ...
      write ( lunit6, 2007 )
2007  format ( 42h         n    ifls   ilim1   ilim2   nuksp )
      nukn = kisblk - 8
      do  n = 1, nuk
         nukn = nukn + 8
...
2013  format ( 1h0, 17x,3hksu/( 2x, 2i8  )  )
...
2015  format ( 1h0,11x,13halksu    ksus, /, (1x, 3i8) )
1012  write (lunit6,2017) ktab, (n, ilntab(klntab+n), n=1,ktab)
2017  format (9h0 ktab = ,i5/9x,7h ilntab ,/,(1x,2i8))
      write (lunit6,2018) nuk
2018  format ( 10h   nuk  = ,i6  )
1006  n2 = 0
      min = nuk + niu
      nkn = nuk + lstat(64)
      do k = 1, ktab
...
2041        format ( /,   15h entries number,  i5,  6h   and,  i5, 48h   of the tacs name-vector both have contents  ', a6,  3h' . )
2045     end do
2046  end do
...
232      format(/, '   the user has requested a tacs variable named "', a6, '" for output purposes. but this is a non-existant &
              tacs', /,  ' variable, so the request will be disregarded.')
         go to 109
229      jr = jr + 1
         ndx1 = kjout + jr
         jout(ndx1) = j
...
192   format(44h0program output will be prepared for nodes 0/(1x,16i8))
...
7686     format ( 45h $$$$$ ignor the initial condition card since, 16h no such name ( , a6,  27h ) in the tacs table $$$$$ )
         go to 4111
412      jr = jr + 1
...
196   format (34h  user-defined initial condition  , a6, e20.8)
...
9000  if ( iprsup  .ge.  1 ) write ( lunit6, 4568 )
4568  format ( 24h  "exit  module tacs1b." )
      return
...
!
    subroutine date44 ( a )
      ...
      n5 = n(2) - 10 * n4
      n6 = n(1) / 10
      write (unit=a(1), fmt=1386) n4, n5, n6
1386  format ( 2i1,  1h/,  1i1  )
      write (unit=a(2), fmt=1394) n7, n3
