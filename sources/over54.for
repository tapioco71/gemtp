...
2 5 x, 107hmiscellaneous data card), indicating that random switch c
3 losing times for  'statistics'  switches are to be           ,/,
4 5 x, 111hgenerated.   since parameter  'isw'  was punched with the
5 key request value  '4444' ,   only an analysis of the       ,/,
6 5 x, 115hswitch closing times is desired (no emtp simulations are
...
1 mory.   only, i6,  34h  integer cells of   /label/   are      ,/,
2 5 x,  16havailable, while, i7,  82h  such cells are required in or
3 der to execute this data case.   a formula for this          ,/,
4 5 x,  46hrequired minimum integer storage space is ....      ,/,
5     30 x,  32hmin  =  ( 3 + ns*(ns-1)/2 ) * nc               ,/,
6 5 x, 103hwhere  'ns'  is the number of  'statistics'  switches,  a
7 nd  'nc'  is the number of compartments of the                )
...
2 5 x, 113hwith the latter figure equal to twice parameter  'sigmax'
3   divided by  'aincr'  (see  'statistics'  miscellaneous      ,/,
4 5 x, 117hdata card parameters).   in order to run this test of the
5   random number generator, either the number of  'statistics'   ,/,
6 5 x, 109hswitches or tabulation compartments must be appropriately
7   decreased, or the emtp must be re-dimensioned so as          )
    write (lunit6, 7451)
7451 format (5x, 107hto provide for more working space in memory.   if
1   the latter course is taken, it is recommended that all of    ,/,
2 5 x, 110hincreased space be confined to lists number  5,  7,  8,
313 ,  14,  or  15,  in which case there is no waste or        ,/,
4 5 x,  47hloss (the increase is  100 per cent effective).     )
    go to 6220
...
2 5 x, 111huse as tabular working space.   the emtp is variably-dime
3   nsioned, of course, with the present size of   /label/       ,/,
4 5 x, 113hsimply insufficient to allow for even the solution of a s
5   ingle-conductor problem.   available dimensioned working      ,/,
6 5 x,  33hspace in subroutine  'vdov44'  is, i5,  66h  cells.   fit
7   ting all matrices into this constraint requires that          )
    write (lunit6, 7252)  lstat(15)
7252 format (5x,  20ha conductor limit of, i5,  73h  conductors be impo
1   sed.   the user must redimension the emtp in order to         ,/,
2 5 x, 107hprovide for more   /label/   storage (with the distributi
3   on of this storage among the different lists being            ,/,
4 5 x,  34himmaterial), and resubmit the job.      )
    go to 6220
6153 write (lunit6, 7153)  bus1
7153 format (5x, 108has part of the tacs data which has now been comple
1   tely read by the emtp, the user has employed the same six-   ,/,
2 5 x,  30hcharacter alphanumeric name  ', a6,  66h'  for two differ
3   ent tacs variables.   unfortunately for the user,            ,/,
4 5 x, 109hhomographs are to be exiled for life from the emtp world.
5   yet should this succinct characterization be less         ,/,
6 5 x, 111hthan perfectly clear, suffice it to say that it is a viol
7   ation of emtp rules for the user to apply the same  a6        )
    write (lunit6, 7253)
...
1   etup'  case, and has discovered an inconsistency or          ,/,
2 5 x,  82hillegality in the numerical values which were punched by
3   the user.   specifically,            )
    write (lunit6, 7254)  lstat(13), lstat(14)
7254 format (5x, 101hdata parameters  'iotx' ,  'ioss' ,  and  'iofl'
1   are required to be octal (base-8) numbers.   digits          ,/,
2 5 x,  96h '8'  and  '9'  are thus illegal (nonexistant), and must
3   not be used.   but the field for number,  i4,   8h   among    ,/,
4 5 x,  32hthese was punched with the value,  i6,  22h .   naughty,
5   naughty.     )
    go to 6220
6155 write (lunit6, 7155)   lstat(13), lstat(14)
7155 format (5x, 108hwithin the computations for a  'semlyen setup'  da
1   ta case, the iterative procedure which is used for fitting    ,/,
2 5 x, 105hthe characteristic-admittance step-response has failed to
3   converge.   this fitting is done by means of an             ,/,
4 5 x, 113halgebraic adjustment in the frequency domain, not the mor
5   e well-known fitting of exponentials in the time domain.      ,/,
6 5 x,  22hthe iteration limit of,  i5,  54h   (parameter  'nitery'
...
1   ich have values,    e15.3,    6h   and,    e15.3,   2h ,     ,/,
2 5 x, 106hrespectively.   the user is reminded that these just-ment
3   ioned  'semlyen setup'  parameters are under user            ,/,
4 5 x, 111hcontrol by means of a special  'tolerances'  request-word
5   card, if this will help any.   alternatively, did the       ,/,
6 5 x, 109huser perhaps already redefined default values by means of
7   such a card, thereby making convergence impossible,         ,/,
8 5 x,  17hthe emtp wonders.     )
    go to 6220
6156 write (lunit6, 7156)  lstat(14)
7156 format (5x, 103hthe present   'semlyen setup'   data case has prov
1   en to be too much for the emtp logic, for mode number,  i4, 2h .)
...
1   ed on the speed of the highest frequency specified . ,/,5x,103h th
2   e time step used in the fitting process is established by a techni
3   que determined by parameter 'kfit' ,/, 5x, 43h for which the user
4   has supplied a value of , i2, 2h . ,/,
6  5x,  49hif  "0"    the time step is simply toe time / noo  ,/,
7  5x,  93hif  "1"    the time step is sized to put 25" of the spec
8   ified "npoint" points between the toe   ,/,  16x,  40hand the time
...
1   ot be possible because of noise at small times in inverse fourier
2   routines . ,/,  5x,  44hthis run has suffered from this difficulty
3   . ,             5x, i4, 55h iterations have failed to locate one
4   of these points . ,/,  5x,  62hthe solutions available are "  "a"
5   different value of "kfit", ,/,  5x,  80h"b" a higher number for "
6   nfit" which controls the permissible no. of iterations,  ,/, 5x, 1
708 h"c" a higher frequency range and/or more frequency points in an
...
1   teration are as follows : ,/,  5x,  42hthe initial size of the tim
2   e increment was , e15.5,  24h .  the target value was , e15.5  ,/,
...
2 5 x,  75hby inverse fourier transformation.   this procedure has n
3   ow been completed.    )
...
2 5 x, 104hat zero, and should rise monatonically to unity, as time
3   goes to infinity.   but the value for the final              ,/,
4 5 x,  12hpoint number,  i5,  5h   is,    e14.3,     66h ,   which
...
2 5 x,  55hinvalid, and execution will not be allowed to continue. )
    go to 6220
...
2 5 x,  63hmonotone-decreasing slope (concave downward).   at point
3   number,  i6,  37h ,   there is trouble,   let's denote       ,/,
4 5 x,  70hthis point by  'k' .   values for points  k-1, k, % k+1
...
2 5 x, 107h(r), (l)  format (type-51, 52, ... ).   this is a feature
3   of supporting routine  'xformer' .   but there is          ,/,
4 5 x,  89htrouble with the user's  'transformer'  data which is now
...
2 5 x, 109hof several cards used to define a saturable transformer c
3   omponent.   but the user has failed to punch columns         ,/,
4 5 x, 112h39-44  (field  'bustop' )   with a non-blank 6-character
5   (a6) alphanumeric name.   this is the node name for the       ,/,
6 5 x, 114hextra, internal node of the saturable transformer compone
...
2 5 x, 112hcomponent must be non-blank, and all should be unique ---
3   different from each other, and also different from all      ,/,
4 5 x,  25hother emtp network nodes.        )
    go to 6220
...
2 5 x, 110hfrequencies of interest.   on the frequency card which re
3   quested this mode of program execution, the field for        ,/,
4 5 x, 106hemtp variable  'ipunch'  (columns  66-68 ,   read using
5   i3  format)  contains an illegal numerical value.            ,/,
6 5 x, 101honly values  0,  1,  2,  3,  88,  and  89  are recognized
...
2 5 x, 111hcard).   but no valid emtp electric-network node names we
3   re specified by the user on either a preceding   'tacs       ,/,
4 5 x, 109hemtp sources'   card,  or on a preceding   'tacs outputs'
5   card.   hence the emtp concludes that there is no         ,/,
6 5 x, 109helectric network, that the present problem being inputted
...
2 5 x, 108hrequested that a tacs source be determined by a non-exist
3   ent part of the problem.   this discrepancy must be          ,/,
4 5 x,  56hcorrected, before execution will be allowed to continue.)
    go to 6220
...
3 5 x,  38hmay be changed by a "tolerances" card. )
    go to 6220
...
2 5 x,  28hmore separator characters  ',  a1,
3  38 h'  or  continuation-card characters  ',  a1,
4  42 h' .   well, emtp control was within module             ,/,
5 5 x, 114h'frefld'  of utpf overlay  (main00, -1)  when the data ra
...
2 5 x, 106hto be free-field data format.   then, he should check all
3   data items, counting separator characters (which           ,/,
4 5 x,  33hdefine the number of data items).                  )
    go to 6220
...
2 5 x, 112h'bus4'  field of columns  21-26  is to be the name of the
...
2 5 x, 114hemtp supporting program (e.g.,  'semlyen setup' ).   but
3   the emtp limit on such individually-specified frequencies     ,/,
4 5 x,   2his,  i5,  98h ,   which has been overflowed by the last-r
5   ead data card.   dimensioning of the crucial arrays in       ,/,
6 5 x, 111hquestion is not under user control.   a complete emtp rec
7   ompilation would be required in order to increase this    ,/,
8 5 x,  48hlimit, should this prove to be really necessary.     )
    go to 6220
...
2 5 x,  88hthe user specified floating-point miscellaneous data para
3   meters  'xopt'  and  'copt'  as,    e13.4,  6h   and         ,/,
4 5 x,    e13.4,  88h ,   respectively.   this is illegal.   both pa
...
2 5 x, 117hbeing the miscellaneous data card of a new case.   but th
...
2 5 x, 112h i5  format.   but only the values  1  (for overhead line
3   constants) and  2  (for cable constants) are legal, if      ,/,
4 5 x, 115hthe card in question is nonblank (recall that a blank car
...
2 5 x, 108han underground cable system is to be under consideration.
3   but field  'iearth'  of columns  16  through  20          ,/,
4 5 x, 110hhas been punched with the integer value  99 ,   which mea
5   ns that the stratified-earth option is desired.   but        ,/,
6 5 x, 107hthis combination of features is illegal (the emtp is inca
7   pable of finding the parameters for an underground           ,/,
8 5 x,  53hcable when the earth is not homogeneous and uniform).   )
    go to 6220
...
2 5 x, 106hread.   but one or more of these cards has been punched w
...
2 5 x,  78his physically impossible.   the inner radius of the condu
3   ctor has been read as,    e16.4,        16h   meters, while   ,/,
4 5 x,  19hthe outer radius is,    e16.4,     67h .    this violates
5   the emtp restriction that the tubular conductor             ,/,
6 5 x,  72hthickness must be positive.   zero or negative thickness
7   is not allowed.        )
    go to 6220
...
2 5 x,  67hconstants'  supporting program.   but within the iteratio
3   n limit of,  i5,   32h ,   the iterative algorithm has       ,/,
4 5 x,  46hfailed to satisfy the convergence tolerance of,
5   e14.2,  39h .    the resulting eigenvalues must be          ,/,
6 5 x,  83hviewed with suspicion (at best), so the solution is being
...
2 5 x, 104hviolation of the ordering restriction which is applicable
3   when dynamic synchronous machine (s.m.) source              ,/,
4 5 x, 111hcomponents are present.   recall that all non-s.m. source
5   cards must precede any s.m. source cards, in order of       ,/,
6 5 x,  37hdata input.   thusfar there have been, i4,   60h   s.m. s
...
2 5 x,  74hof source type-code  'itype'  (columns 1-2) punched with
3   the integer value, i5,  25h ,   which is not a legal         ,/,
4 5 x, 110hcharacterization for a s.m. source component.   the last-
5   read data card is thus either erroneous by itself, or        ,/,
6 5 x,  63hout of order (it must precede the first s.m. source compo
7   nent).      )
    go to 6220
...
2 5 x, 109hconnection for the third and final phase of a 3-phase dyn
3   amic synchronous machine (s.m.) component.   but the         ,/,
4 5 x, 108hsource type-code for this component is illegal (unrecogni
5   zable).   from the field  'itype'  (columns 1-2) of          ,/,
6 5 x, 108hthe first data card for this s.m. component --- two non-c
7   omment cards before the last-read data card --- was          ,/,
8 5 x, 19hread a type code of,  i2,    83h .    but only s.m. model
...
2 5 x, 108hthe two.   recall that these two classes of s.m.  are qui
3   te different, utilizing different solution methods.          ,/,
4 5 x,  62hthe just-read dynamic  s.m.  emtp  source component was n
...
2 5 x, 109hreally an overflow of emtp list number  11 ,   though not
3   due to conventional emtp electric-network branch or         ,/,
4 5 x, 110hnode quantities.   rather, it is the added burden of dyna
5   mic synchronous machine (s.m.) output variables, plus        ,/,
6 5 x, 103hthe burden of tacs output variables, which has now led to
...
2 5 x,  i5,  67h = number of conventional electric-network branch-ou
3   tput quantities                                              ,/,
4 5 x,  i5,  35h = number of s.m. output quantities            ,/,
5 5 x,  i5,  35h = number of tacs output quantities            ,/,
6 5 x,  34hthe last three figures above total,  i6,  70h ,   which i
...
2 5 x,  72hquantities, before trying once again to solve the data ca
3   se in question.              )
    kill = 1
    lstat(16) = 11
    nchain = 51
...
2 5 x,102hbefore the one which requests an automatic looping over lo
3   garithmically-spaced frequencies, there were, i4,            /,
4 5 x,    108hfrequency cards, each of which requests the line-const
5   ants calculation at an individual, discrete frequency.       ,/,
6 5 x,  77hbut the transposition flag (miscellaneous data parameter
...
2 5 x,  62hfrequency cards, while a transposed line requires exactly
3   one.    )
    go to 6220
6183 j = lstat(13)
    write (lunit6, 7154)
...
2 5 x, 110hthe emtp requires that all such cards apply to the same m
3   odel of the transmission line.   but it will be noted        ,/,
4 5 x,  28hthat an earth resistivity of,    e17.5,  49h   ohm-meters
5   was read from frequency card number, i4,   10h ,   while     ,/,
6 5 x,     e17.5,      94h   was read from the first such card.   th
...
2 5 x, 109hfirst, there should be the line-conductor cards, terminat
3   ed by a blank card.   then come the frequency cards,         ,/,
4 5 x, 104hwith only the last of these requesting the logarithmic fr
5   equency-looping option.   finally, a blank card              ,/,
6 5 x,  95hterminates the frequency cards, and a second blank card f
...
2 5 x,  82h 'iotx'  which controls printout.   but this exceeds the
...
2 5 x, 112hare inappropriate.   recall that within the line-constant
3   s data, the frequency card for logarithmic looping over      ,/,
4 5 x, 112ha range of frequencies is to be preceeded by a frequency
5   card for the steady-state frequency.   the semlyen code      ,/,
6 5 x, 112hrequires that the beginning loop-frequency exceed the ste
7   ady-state frequency.   but such is not the case for the       ,/,
8 5 x,  22huser's data (values of,   e16.4,    6h   and,    e16.4,
...
2 5 x, 108hfollowing the first semlyen miscellaneous data card, the
3   user must supply a card which bears either the text          ,/,
4 5 x, 110h 'line constants'   or the text   'cable constants' ,   p
5   unched in columns one onward.   not so for the user's        ,/,
6 5 x,  56hcard, the column 1-18 contents of which were read as   ',
...
2 5 x, 114hback in the  'semlyen setup'  overlay where matrices  (r)
3   ,  (x),  (g),  and  (b)  are being read from input/output      ,/,
4 5 x, 112hunit number  3 .   but the capacitance matrix is erroneou
5   s.   it should be independent of frequency, but is not.      ,/,
6 5 x,  49hin particular, the first diagonal entry  c(1,1) =,
...
2 5 x,  46hthese capacitances agree within a tolerance of,   e14.2,
3    47 h   (floating-point miscellaneous data parameter       ,/,
4 5 x, 108h 'epsiln' ).   for the user's data, this check has not be
5   en met.   the capacitance matrices are judged to be          ,/,
6 5 x,  46hinvalid, and the solution is being terminated.     )
    go to 6220
...
2 5 x, 110hhave now been read.   as the preceding printout shows, th
3   e vector of tacs variable names has repeated entries.         ,/,
4 5 x,   9hthere are,  i4,   94h   such conflicts.   the trouble can
