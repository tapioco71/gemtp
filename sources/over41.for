...
61 format (/, ' Problem.  ----  admittance matrix for transformer is too close to being singular to be invertible.   Hence    ', /, &
        17x, 'the inversion will be skipped.   Parameters which document this breakdown follow.   ', /, &
        20x, 9x, 'epsiln', 12x,  'cii',  13x, 'cr' , /, 20x, 3e15.6)
  go to 600
65 xx(ki)=cr+cii
  go to 40
...
113 format ( /, 1x )
120 format (' Warning. ----- if nothing is connected to windings which are delta-connected, either add capacitances or ground  ', /, &
         16x, 'one terminal of the delta.   Otherwise the delta connection is floating, with voltage indeterminate.')
...
4220 format ( 1x,  3h51,,  a6, 1h,,a6, 10h,  ,  ,      , 2( e22.13, 2h , ),  1x,  5h,,,,   )
     go to 4270
...
146 if(ichck.gt.0) go to 600
  j=(iwind-1)*3
  do i=1,j
     if ( xx(i)  .lt.  0.0 ) write (lunit6, 155)
...
1765 write (kunit6, 3904)  v1, v2, ploss, z
  call interp
  write (lunit6, 15)  iwind, cmag, pmag
  write (lunit6, 20)  text1(1), v1, text1(1), text1(3), ploss, z, pratg
  text1(2)=t3
  cmag=cmag  * pmag / 200.
...
7182 format (//,' Saturable transformer input data is now complete. Relevant scalar parameters may be summarized as follows ...' , /, &
          ...
          e20.8, '  =  magnetizing reactance at the power frequency in Ohms')
...
625     format( ' type-91:  piece-wise linear resistor', 14x, 1h1, a80 )
1000    continue
...
3110    format( 34x, 2e10.6 )
        ...
        n6 = 6
        call fltopt( d8, n6 )
        write( 7, 3300 ) card(3:14), work(1:6), ansi32(27:32), work(13:79), card(80:80)
...
3500    format( 7x, 9h0.0000001, 6x, a10, 50x )
        write( lunit9, 3525 ) card(3:14), work(1:6), ansi32(27:32), work(13:79), card(80:80)
...
4288 if ( n4 .ge. n15 ) go to 4293
  n15 = n4
4293 if ( iprsup .ge. 1 ) write (*, 4296)  n4, n17, n18, n15, n14, ansi32(n14:32)
...
12 i = 0
  ...
  write (lunit6, 4005)
4005 format(1h0,24x,17hpositive sequence,25x,13hzero sequence,10x, 13h closed delta,/,35h from  to  load loss(kw)  impedance, &
...
4202 format(2i2, 5e10.2, 2i2)
  ...
  xzero = xpos
  ratzer = ratpos
38 write (lunit6, 4007) ibg, ien, h1, xpos, ratpos, xzero, ratzer, idelt
...
705  end do
     ...
     write (lunit6, 721) ii50, name(nf), name(nt), (rout(i), y(i), i=1, ii)
721  format (1h , i2, 2a6, 12x, 2e17.10, /, (1h , 26x, 2e17.10) )
     write (lunit7, 726) ii50, name(nf), name(nt), (rout(i), y(i), i=1, ii)
726  format (i2, 2a6, 12x, 2e16.10, /, (26x, 2e16.10) )
     go to 724
722  write (lunit6, 723) ii50, name(nf), name(nt), (rout(i), y(i), i=1, ii)
723  format (1h , i2, 2a6, 12x, 2e17.10 )
     write (lunit7, 725) ii50, name(nf), name(nt), (rout(i), y(i), i=1, ii)
...
304 format(42h resistance matrix values are those which , 12hwere read in  )
  go to 19
310 write(lunit6, 311) i, in
311 format(5h only, i3, 30h short-circuit tests specified, 4h but, i3, 11h are needed  )
  call stoptp
320 write(lunit6, 321) itest, iput
321 format(15h either itest =, i3, 10h or iput =, i3, 14h not permitted   )
  call stoptp
110 write(lunit6, 101) j
101 format(24h diagonal element in row, i3, 14h close to zero)
  call stoptp
830 write(lunit6, 840) idelt, k, m
840 format(8h idelta=, i2, 36h wrong in short-circuit test between, i3, 4h and, i3  )
  call stoptp
832 kcode = 1
...
851 format (47h p.u. excitation loss larger than p.u. exciting, 42h current (either in pos. or zero sequence))
...
12 format(50h modification of zero sequence short-circuit test , 7hbetween, i3, 4h and, i3, 27h not possible. error code =, i3)
  call stoptp
1 return
