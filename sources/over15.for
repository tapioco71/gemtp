...
1030 call cimage
3038 continue
  read (unit = abuff, fmt = 3043) ijk, aupper
3043 format( i2, 13a6 )
  if ( ijk  .ne.  1 )   go to 8211
  numnvo = ntot1
  ivolt = ijk
  if ( noutpr  .eq.  0 ) write (kunit6, 3045)
3045 format( 41h+request for output of all node voltages.   )
  call interp
...
4675 write (lunit6, 4676)  n13, ntot, lbus
4676 format ( '   = = =  no, too many dummy node voltage', ' output channels for emtp dimensions.', /, &
  ...
  n13 = istep
  istep = 0
4682 if ( noutpr .eq. 0 ) write (kunit6, 4683)  n13
4683 format ( 33h+dummy node voltage output number,  i8  )
  do j=1, n13
...
!!!! 1            ' jbrnch(nv) =',   nv, ibrnch(nv), jbrnch(nv)
           go to 4962
2801    end do
        if ( noutpr  .eq.  0 ) write(lunit6, 6925 )  bus1
6925    format( 5x,  'ignore v-branch request involving nonexistent', ' node  "',  a6,   '"')
        go to 4962
5183 end do
4962 end do
  if ( noutpr  .eq.  0 ) write (kunit6, 3872)
...
3052 end do
     if ( noutpr  .eq.  0 ) write(lunit6, 3053 )  bus1
...
3056 if ( ivolt  .eq.  1 )  go to 8888
     ...
     lstat(19) = 3056
     go to 9000
8888 write ( lunit6, 8899 )
8899 format ( 48h there has been the request for all node voltage, 41h output, so this request will be ignored. )
     go to 1030
8215 l = 1
...
8227 end do
     write ( lunit6, 8229 )
8229 format ( 45h     request for branch output of nonexistent, 23h branch will be ignored )
     go to 8288
8311 iprint = 11
     lstat(19) = 8237
     go to 9000
...
8237 if ( ijk  .gt.  -2 )  go  to  8288
     ...
     if ( iprsup  .gt.  2 ) write(lunit6, 8244) npower, maxpe, koutvp(npower), koutvp(mpower)
8244 format ( /, 21h power output request, 4i10 )
...
8288 end do
  ...
  if ( noutpr  .eq.  0 ) write (kunit6, 3072)
3072 format( 43h+card of bus names for node-voltage output.   )
  go to 1030
3081 if ( noutpr  .eq.  0 ) write (kunit6, 54167)
54167 format(  49h+blank card ending node names for voltage output.    )
  call interp
...
1311 format ( ' (parameter  "nenerg"  of columns 65-72, of the 2nd misc. data card).  ', &
       ' switch closing or opening times for the ',/, ' specially-flagged switches (with "statistics"', &
       ' punched in columns 55-64) are varied randomly, according to a ',  a6, a1, ' distribution.')
  write (lunit6, 1324)
1324 format (" The user can select either uniform or normal distributions, based on the value of parameter  'idist'  of columns  ",/, &
       ' 17-24 of the special statistics misc. data card.   The following is a listing of switches whose closing times are   ',/, &
       ' to be statistically varied, along with the associated mean and standard deviation for the distribution, as requested   ', &
       ' by the user.')
  if ( xmaxmx  .lt.  0.0 ) write (lunit6, 1325)  xmaxmx
...
1329 text2 = text8
  ...
  text2 = text10
  text3 = text11
8329 write (lunit6, 1330)  text2, text3
1330 format ( ' (parameter  "nenerg"  of columns 65-72 of the 2nd misc. data card).', /, &
       " closing times for the specially-flagged switches (with  'systematic'  punched in columns 55-64) are varied   ",/,  1x, &
       'regularly between minimum and maximum times, with a uniform increment in between for each switch.   The following        ',/,1x, &
       'is a listing of the switches whose closing times are to be systematically varied for this data case.   Also tabulated   ',/, 1x, &
...
1338 format ( /, 10x, 'entry', 4x, 'switch', 6x, 'from', 8x, 'to', 9x, 'columns 15-24', 8x, 'columns 25-34', &
  ...
  do i=1, kswtch
     ...
     n1 = n1 + 1
     d1 = absz( topen(ndx1) )
     write (lunit6, 1346)  n1, i, bus(l), bus(m), crit(i), d1, kdepsw(i)
1346 format ( 5x, 2i10,  2(4x, a6), 2f21.6, 6x, i10 )
1352 end do
  if ( nenerg  .lt.  0 )   go to 1373
  write (lunit6, 1371)  degmin, degmax, statfr
1371 format ( " now in addition to switch-time variation caused by each switch's own distribution, there is the added random", /, &
       " delay which is the same for all switches, refered to by the term 'reference angle'.   Distribution for this   ",/, &
       ' angle is uniform over the time interval from', e14.5, ' to', e14.5, ' degrees based on', e13.3, ' Hz', /, &
  ...
  write(lunit6,1372)
1372 format( " However, if the parameter 'itest' (found in col. 9-16 of the statistics misc. data card) is greater than zero", /, &
       ' the reference angle is set to zero and, consequently, there is no added random delay.  ', /, 1x)
1373 write (lunit6, 1374)
1374 format (' In the following printout, switch times which are used for each energization are tabulated, followed by peak   ', /, &
  ...
  if ( nenerg  .gt.  0 ) write (lunit6, 1362)
...
7427 format (/, ' Mixture of integer arrays in over15.   Only the longest column(s) will not have garbage at the ends.   ', /, &
  ...
  do k=1, n2
     ndx1 = lswtch + k
7431 write (lunit6, 7432) k,kmswit(k),kmswit(ndx1),kswtyp(k),kpos(k), node(k), isourc(k), kbus(k), mbus(k),nonlk(k), nonlm(k)
  end do
7432 format ( 5x , 11i10 )
7436 iprint = 11
  if( iprsup .ge. 2 ) write(lunit6, 8025)  ibr, inonl, kswtch, nv, npower
8025 format( /, 9h at 8025   , 5i10 )
  if(kswtch.eq.0) go to 912
  if( iprsup .ge. 2 ) write (lunit6, 8026)  ( koutvp(l), l=1, npower )
  ndx1 = maxpe + 1
  ndx2 = maxpe + npower
  if ( iprsup  .ge.  2 ) write (lunit6, 8026)  ( koutvp(l), l=ndx1, ndx2 )
8026 format( /, 17h koutvp or koutie   ,/, ( 1x, 10i10 ) )
  do k=1,kswtch
     n3 = kpos(k)
     ...
     if ( iprsup  .ge.  4 )  write(lunit6, 8028)  k, nc, n3, n2
8028 format ( /, 9h at 8028 , 4i10 )
     do i = 1, npower
        mpower = maxpe + i
...
912 if ( inonl  .eq.  0 )  go to 8411
  do k = 1, inonl
     icheck=nonlm(k)
     if (iprsup  .gt.  3)  write(lunit6, 8029)  k, icheck, nc
8029 format( /, 9h at 8029  , 3i10 )
...
8411 n15 = nfrfld
  if ( numum  .gt.  0 ) call uminit (n15, spum(iureac), spum(iugpar), spum(iufpar), spum(iuhist), spum(iuumrp), &
       ispum(iunod1), ispum(iunod2), ispum(iujclt), ispum(iujclo), ispum(iujtyp), ispum(iunodo), ispum(iujtmt), &
       spum(iuhism), spum(iuomgm), spum(iuomld), spum(iutham), spum(iuredu),spum(iureds), spum(iuflds), &
       spum(iufldr), spum(iurequ), spum(iuflqs), spum(iuflqr), ispum(iujcds), ispum(iujcqs), spum(iuflxd), &
       spum(iuflxq), ispum(iunppa), spum(iurotm), ispum(iuncld), ispum(iunclq), ispum(iujtqo), ispum(iujomo), &
       ispum(iujtho), spum(iureqs), spum(iuepso), spum(iudcoe), ispum(iukcoi), spum(iuvolt), spum(iuangl), &
       ispum(iunodf), ispum(iunodm), ispum(iukumo), ispum(iujumo), spum(iuumou))
  if ( kill .gt. 0 ) go to 9200
  if ( iprsup  .ge.  1 ) write (lunit6, 7309)  numsub, n15, ncomp
7309 format ( /,  28h done setting comp. vectors., 24h  numsub     n15   ncomp  ,/,  28x,  3i8  )
  if ( iprsup  .lt.  2 )   go to 7321
  n11 = numsub + 1
  write (lunit6, 7312)  ( isubeg(k), k=1, n11 )
7312 format (  9h isubeg :,  10i10   )
  n12 = isubeg(numsub) + 4
  write (lunit6, 7315) ( nsubkm(k), k=1, n12)
7315 format ( 9h nsubkm :,  10i10  )
  write (lunit6, 7319)  ( kupl(k), k=1, inonl )
7319 format (  7h kupl :,  10i10   )
  write (lunit6, 7320)  ( vecnl2(k), k=1, inonl )
7320 format (  9h vecnl2 :,  10e12.4  )
7321 n14 = lcomp * lbus
  n13 = ncomp * ntot
...
7327 do k=1, ibr
     icheck=mbus(k)
     if (iprsup .ge. 3)  write(lunit6, 8030)  k, icheck, nc
8030 format ( 37h at 8030 of "over15", k, icheck, nc =, 3i6 )
...
4752 if( numsm .gt. 0  )    call smout
  ...
  n8 = ntot + maxbus
  n18 = n8 + n16
  if ( iprsup  .ge.  1 ) write (lunit6, 3207)  nc, lsiz12, nsmout, ioutcs, ntot, numnvo, nv, n8, maxbus, n9, n18
3207 format ( /,   32h before output-variable heading., 40h      nc  lsiz12  nsmout  ioutcs    ntot, &
          48h  numnvo      nv      n8  maxbus      n9     n18,/,  32x,  12i8  )
...
300 if (iplot .lt. 0) isplot = intinf
  ...
  rewind lunit4
  write (lunit4)  date1, tclock, n18, numnvo, numbco, nc,  ( bus(i), i=1, ntot), ( texvec(i), i = 1, maxbus ), ( busum(i), i=1, n16 )
  if ( iprsup  .ge.  1 ) write (*, 3207)  nc, lsiz12, nsmout, ioutcs, ntot, numnvo, nv, n8, maxbus, n9, n18
  if ( ivolt  .ne.  1 )   go to 3103
  write (lunit4)   ( i, i=2, ntot )
  go to 3104
3103 if( numnvo .gt. 0 ) write (lunit4)  ( ibsout(i), i=1, numnvo )
3104 if( nc .gt. 0 ) write (lunit4)  (ibrnch(i), i = 1, nc), (jbrnch(i), i = 1, nc)
3106 if ( kbase .ne. 1  .and.  kbase .ne. intinf )  go to 7802
  write (lunit9)  date1, tclock, ivolt, maxbus, ndx1, ndx2, n16, nsmout, ioutcs, numout, n18, numnvo, numbco, nc, kswtch
  write (lunit9)  (bus(i), i=1, ntot), ( texvec( i ), i = 1, maxbus ), ( busum(i), i=1, n16 )
  if ( iprsup  .ge.  1 ) write (*, 3207)  nc, lsiz12, nsmout, ioutcs, ntot, numnvo, nv, n8, maxbus, n9, n18
  if ( ivolt  .ne.  1 )   go to 4103
  write (lunit9)   ( i, i=2, ntot )
  go to 4104
4103 if( numnvo .gt. 0 ) write (lunit9)  ( ibsout(i), i=1, numnvo )
4104 if( nc .gt. 0 ) write (lunit9)  (ibrnch(i), i = 1, nc), (jbrnch(i), i = 1, nc)
7802 if ( ivolt  .eq.  1 )   go to 3107
  if ( numnvo  .gt.  nc )   lstat(31) = numnvo
3107 if ( noutpr  .ne.  0 )   go to 54242
  n4 = ncsave - nv
  if ( kol132  .eq.  132 )   go to 6637
  write (lunit6, 6632)  numnvo, nv, n4, nsmout, ioutcs
6632 format (' Time-step loop begins.  Number of node voltages, branch voltages, currents, tacs', /, &
  ...
  go to 6645
6637 write (lunit6, 3108)  k
3108 format ( /, 24h Column headings for the,  i4,  73h  emtp output variables follow.   These are ordered according to the five     ,/, &
          55h possible EMTP output-variable classes, as follows ....   )
  write (lunit6, 3114)  numnvo
3114 format ( 3x,  5hFirst,  i4,  85h  output variables are electric-network node voltages (with respect to local ground)|        )
  write (lunit6, 3115)  nv
3115 format ( 3x,  5h Next,  i4,  91h  output variables are branch voltages (voltage of upper node minus voltage of lower node)|    )
  write (lunit6, 3116)  n4
3116 format ( 3x,  5h Next,  i4,  87h  output variables are branch currents (flowing from the upper emtp node to the lower)|        )
  write (lunit6, 3117)  nsmout
3117 format ( 3x,  5h next,  i4,  92h  output variables pertain to dynamic synchronous machines, with names generated internally|     )
  write (lunit6, 3118)  ioutcs
3118 format ( 3x,  5hfinal,  i4,  81h  output variables belong to  'tacs'  (note internally-added upper name of pair).     )
  write (lunit6, 3122)
3122 format ( 105h branch power  consumption (power  flow, if a switch) is treated like a branch voltage for this grouping|         ,/, &
          105h branch energy consumption (energy flow, if a switch) is treated like a branch current for this grouping.     )
6645 if( iprsup .le. 0 )  go to 54242
  if( npower .le. 0 )  go to 54242
  write(lunit6, 3112)  ( ibrnch(i), jbrnch(i), i=1, nc )
3112 format( /,  72h 'ibrnch'  and  'jbrnch'  pairs of branch-output node numers, in  over15    ,/,  ( 1x, 20i6 ) )
  write(lunit6, 3113 )  ( koutvp(i), i=1, npower )
3113 format( /,  32h 'koutvp'  and  'koutie'  pairs.    ,/, ( 1x, 10i12 ) )
  ndx1 = maxpe + 1
  ndx2 = maxpe + npower
  write(lunit6, 3113)  ( koutvp(l), l=ndx1, ndx2 )
54242 k = 0
  n1 = 0
  if (nenerg .ne. 0)   n1 = 1
  kprsta = 1
...
3177 kcount = nv
  ...
  rewind 79
  do ip=1, 99999
     read (79, 8927, end=8644)  outlin
8927 format ( a132 )
8936 write (lunit6, 8927)  outlin
  end do
...
8710 if ( kpos(i)  .ge.  0 )   go to 8713
     kcount = kcount + 1
     bvalue(kcount) = d6
     if ( iprsup  .ge.  1 ) write (lunit6, 8712)  kcount, bvalue(kcount)
8712 format ( 25h kcount, bvalue(kcount) =,  i8,  e15.6 )
8713 end do
  ktrlsw(1) = n11
8719 nc = ncsave
...
subroutine top15
  ...
  if ( iprsup  .ge.  1 ) write (lunit6, 752)  nenerg, ibr, kswtch, inonl
752 format ( /,  17h enter  "top15" ., 32h  nenerg     ibr  kswtch   inonl,/,  17x,  5i8  )
  knum = 1
  kntbr=0
  iprsov(35) = 0
  kcount=2
...
6789 if ( iprsup  .ge.  1 ) write (lunit6, 7241)  k,  numsub, n15, ncomp, isubeg(numsub)
7241 format ( 41h  k, numsub, n15, ncomp, isubeg(numsub) =,  5i8 )
7248 end do
  if ( iprsup .ge. 7 ) write (*,*) ' top15.  kssfrq(1:ntot) =', ( kssfrq(k), k=1, ntot )
  do k=2, kpartb
     if ( kssfrq(k) .gt. ntot )  kssfrq(k) = 0
7251 end do
  if ( iprsup .ge. 7 ) write (*,*) ' after tamper..  kssfrq(1:ntot) =', ( kssfrq(k), k=1, ntot )
...
subroutine smout
  ...
  jk = 0
  jk1 = 0
  if ( iprsup  .ge.  1 ) write (lunit6, 1328)  nst, lsyn
1328 format ( /,  17h begin  'smout' ., 16h     nst    lsyn   ,/,   17x,  4i8  )
  n1 = 0
  n2 = 0
  ip1 = -2
  do ip = 1, nst
...
4224 end do
     ...
     return
3648 end do
  if ( iprsup  .ge.  3 ) write(lunit6, 4240) (i, ismout(3 * i), ismout(3 * i - 1), i = 1, jk)
4240 format ( /,  35h (i, jsmout(i), ksmout(i), i=1, jk)    ,/, ( 1x,  10( 3i4,  1x ) )  )
  nsmout = jk
  d12 = nbyte( 4 )
...
subroutine uminit(n15, reacl, gpar, fpar, hist,umcurp, nodvo1, nodvo2, jcltac, jclout, jtype, nodom, jtmtac, histom, &
  ...
  if ( iprsup  .ge.  1 ) write ( lunit6, 4567 )
4567 format ( 24h  "begin module uminit." )
  istart = 0
  ksubum = numsub + 1
  nclcom = n15 + 5
  if ( iprsup  .ge.  1 ) write (lunit6, 7302)  ksubum
7302 format ( /,  27h begin  "uminit" . ksubum =,  i4  )
...
1515 if ( num .gt. 0 ) go to 1616
     ...
     go to 1010
7777 write(lunit6,8888) k
8888 format(/,42h error stop. something has gone wrong with, 45h setting up the compensation tables in module, &
          18h "uminit" for um -,i4,14h. consult EMTP       ,/,   42h management after making sure that you did, &
          38h not violate the um rule regarding the, 25h three-phase restriction.)
     kill = 229
     n15 = isubeg(mark)
     lstat(15) = k
