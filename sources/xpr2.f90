...
3041 if(.not.(i.lt.k .and. stpflg.eq.0)) go to 3040
  i=i+1
  tacs_a = rsto(i0+i)
  if (.not. (tacs_a .eq. rnull)) go to 5091
  stpflg = 88
  stpi5 = i0 + i
  go to 5090
5091 continue
...
5130 continue
  go to 9500
9005 continue
  p = dmax1 (rmargn, dabs (rmargn * tacs_b))
  if (.not. ((ndx .eq. 3 .and. tacs_a .lt. tacs_b - p) .or. (ndx .eq. 4 .and. tacs_a .le. tacs_b + p) .or. (ndx .eq. 5 .and. tacs_a .gt. tacs_b + p) .or. (ndx .eq. 6 .and. tacs_a .ge. tacs_b - p) .or. (ndx .eq. 1 .and. tacs_a .ge. tacs_b - p .and. tacs_a .le. tacs_b + p) .or. (ndx .eq. 2 .and. (tacs_a .lt. tacs_b - p .or. tacs_a .gt. tacs_b + p)))) go to 5161
  tacs_c = one
  go to 5160
5161 continue
5160 continue
...
3081 if(.not.(i.lt.cnt5)) go to 3080
  i=i+1
  rsto(ptr4 + i) = rsto(rptr + i) * tacs_a
  go to 3081
3080 continue
  cnt4=cnt5
  go to 5260
...
3091 if(.not.(i.lt.cnt4)) go to 3090
  i=i+1
  rsto(ptr4 + i) = rsto(ptr4 + i) * tacs_a
  go to 3091
3090 continue
  go to 5260
5262 continue
...
5250 continue
  go to 9500
9008 continue
  if (.not. (dabs(tacs_a) .lt. rmargn .or. dabs(tacs_b) .lt. divzro)) go to 5271
  tacs_c = zero
  go to 5270
5271 continue
  tacs_c = tacs_a - dint (tacs_a / tacs_b) * tacs_b
5270 continue
  go to 9500
9009 continue
  if (.not. (dabs (tacs_a) .lt. rmargn)) go to 5281
  tacs_c = zero
  go to 5280
5281 if (.not. (dabs(tacs_b) .lt. rmargn)) go to 5282
  tacs_c = one
  go to 5280
5282 if (.not. (tacs_a .lt. zero)) go to 5283
  tacs_d = dnint (tacs_b)
  tacs_e = dabs (tacs_d - tacs_b)
  if (.not. (tacs_e .gt. rmargn)) go to 5871
  stpflg = 132
  stpr1 = tacs_a
  stpr2 = tacs_b
  continue
...
5870 continue
  go to 5280
5283 continue
  tacs_c = tacs_a ** tacs_b
5280 continue
  go to 9500
9010 continue
  tacs_a = rsto(rptr)
  if (.not. (dabs (tacs_a) .lt. divzro)) go to 5291
  stpflg = 82
  stpr1 = tacs_a
  continue
  call errstp
  go to 5290
5291 continue
  rsto(rptr) = one / tacs_a
5290 continue
  go to 9500
9011 continue
  tacs_a = rsto(rptr)
  if (.not. (dabs (tacs_a) .lt. divzro)) go to 5301
  flg = 1
  go to 5300
5301 continue
  flg = 0
...
9502 from = isto(sptr)
  sptr = sptr + 1
  tacs_a = rsto(ptr0 + 1)
  tacs_b = rsto(ptr0 + 1 + ndx12)
  p = dmax1 (rmargn, dabs (rmargn * r6))
  if (.not. ((ndx3 .lt. 0 .and. tacs_b .le. r6 + p) .or. (ndx3 .gt. 0 .and. tacs_b .ge. r6 - p))) go to 5321
  flg = 1
  go to 5320
5321 continue
  ndx4 = ndx2
...
3100 continue
  ...
  rsto(rptr) = r5
  go to 5330
5331 if (.not. ((ndx3 .lt. 0 .and. tacs_b .ge. r6 - p) .or. (ndx3 .gt. 0 .and. tacs_b .le. r6 + p))) go to 5332
  rptr = rptr + 1
  if (rptr .eq. rsptr) go to 930
  rsto(rptr) = tacs_a
  go to 5330
...
5951 if (.not. (dabs (r6 - r2) .le. dmax1 (rmargn, dabs (rmargn * r2)))) go to 5952
  ...
  tacs_a = r3 - dmax1 (rmargn, dabs (rmargn * r3))
  rptr = rptr + 1
  if(rptr.eq.rsptr) go to 930
  rsto(rptr) = tacs_a
  go to 6160
...
9020 continue
  ...
  tacs_c = rsto(k1 + 1)
  p = dmax1 (rmargn, dabs (rmargn * tacs_b))
  if (.not. (tacs_a * tacs_c .le. zero .or. tacs_a * tacs_b .le. zero)) go to 6121
  n = 1
  go to 6120
6121 continue
  if (.not. (dabs(tacs_a) .le. p)) go to 6091
  n = 1
  go to 6090
6091 continue
  tacs_g = dmax1 (tacs_g, dabs (dlog10 (dabs (tacs_a / tacs_b))))
  n = min0 (n, 2 * idint (6.0d0 / tacs_g))
  n = max0 (n, 1)
6090 continue
  if (.not. (tacs_c .gt. tacs_b)) go to 5971
  i1 = 1
  go to 5970
5971 if (.not. (tacs_c .lt. tacs_b)) go to 5972
  i1 = -1
  go to 5970
5972 continue
5970 continue
...
3311 if (.not. (i .lt. n .and. n .gt. 1)) go to 3310
  ...
  tacs_c = rsto(k1 + i)
  p = dmax1 (rmargn, dabs (rmargn * tacs_b))
  if (.not. ((tacs_c .gt. tacs_b .and. i1 .lt. 0) .or. (tacs_c .lt. tacs_b .and. i1 .gt. 0))) go to 6021
  n = i - 1
  go to 6020
6021 continue
6020 continue
  if (.not. (dabs (tacs_c - tacs_b) .le. p)) go to 6031
  n = i - 1
  go to 6030
6031 continue
  tacs_g = dmax1 (tacs_g, dabs (dlog10 (dabs ((tacs_c - tacs_b) / tacs_b))))
6030 continue
  if (.not. (tacs_b * tacs_c .le. zero)) go to 6101
  n = i - 1
  go to 6100
6101 continue
6100 continue
  if (.not. (dabs(tacs_c) .le. p)) go to 6111
  n = i - 1
  go to 6110
6111 continue
  tacs_g = dmax1 (tacs_g, dabs (dlog10 (dabs (tacs_c / tacs_b))))
6110 continue
  n = min0 (n, 2 * idint (6.0d0 / tacs_g))
  n = max0 (n, 1)
  go to 3311
...
3321 if (.not. (i .lt. n)) go to 3320
  i = i + 1
  tacs_c = rsto(k1+i)
  rsto(k3 + i) = tacs_a - tacs_c
  rsto(k4 + i) = rsto(k2 + i)
  if (.not. (i4 .eq. 0)) go to 6071
  rsto(k4 + i) = rsto(k4 + i) * (tacs_c ** i3)
  go to 6070
6071 continue
6070 continue
  go to 3321
...
3341 if (.not. (i .lt. n)) go to 3340
  i = i + 1
  tacs_b = rsto(k3 + i)
  tacs_e = tacs_b * tacs_d - tacs_c * rsto(k4 + i)
  rsto(k4 + i) = tacs_e / (tacs_b - tacs_c)
  go to 3341
3340 continue
  go to 3331
3330 continue
...
5481 if(.not.(ndx.eq.299)) go to 5482
  if (.not. (tacs_a .lt. zero)) go to 5821
  stpflg = 1
  go to 5820
5821 if (.not. (tacs_a .eq. zero)) go to 5822
  tacs_c = zero
  go to 5820
5822 continue
  tacs_c = dsqrt (tacs_a)
...
5483 if (.not. (ndx .eq. 303)) go to 5484
  if (.not. (tacs_a .lt. zero)) go to 5511
  stpflg = 2
  go to 5510
5511 if (.not. (tacs_a .eq. zero)) go to 5512
  tacs_c = -rinf
  go to 5510
5512 continue
  tacs_c = dlog (tacs_a)
...
5484 if (.not. (ndx .eq. 305)) go to 5485
  if (.not. (tacs_a .lt. zero)) go to 5521
  stpflg = 3
  go to 5520
5521 if (.not. (tacs_a .eq. zero)) go to 5522
  tacs_c = -rinf
  go to 5520
5522 continue
  tacs_c = dlog (tacs_a) / dlog (ten)
5520 continue
  go to 5480
5485 if (.not. (ndx .eq. 307)) go to 5486
  if (.not. (tacs_a .lt. zero)) go to 5531
  stpflg = 4
  go to 5530
5531 if (.not. (tacs_a .eq. zero)) go to 5532
  tacs_c = -rinf
  go to 5530
5532 continue
  tacs_c = dlog (tacs_a) / dlog (two)
5530 continue
  go to 5480
5486 if (.not. (ndx .eq. 309)) go to 5487
  if (.not. (dabs(tacs_a) .le. divzro)) go to 5541
  stpflg = 5
  go to 5540
5541 continue
  tacs_c = one / tacs_a
...
5487 if (.not. (ndx .eq. 311)) go to 5488
  if (.not. (tacs_a .lt. -half)) go to 5551
  stpflg = 6
  go to 5550
5551 continue
  tacs_b = one
  tacs_c = one
3161 if (.not. (tacs_a .ge. tacs_b + half)) go to 3160
  tacs_b = tacs_b + one
  tacs_c = tacs_c * tacs_b
  go to 3161
3160 continue
5550 continue
  go to 5480
...
5489 if (.not. (ndx .eq. 315)) go to 5490
  tacs_c = tacs_a - dint (tacs_a)
  go to 5480
5490 if (.not. (ndx .eq. 317)) go to 5491
  tacs_c = dnint (tacs_a)
  go to 5480
5491 if (.not. (ndx .eq. 319)) go to 5492
  if (.not. (tacs_a .lt. rmargn)) go to 5561
  tacs_c = -one
  go to 5560
5561 if (.not. (tacs_a .gt. rmargn)) go to 5562
  tacs_c = one
  go to 5560
5562 continue
  tacs_c = zero
...
5498 if (.not. (ndx .eq. 333)) go to 5499
  if (.not. (tacs_a .lt. -one - rmargn)) go to 5571
  stpflg = 7
  go to 5570
5571 if (.not. (tacs_a .lt. -one)) go to 5572
  tacs_a = -one
  go to 5570
5572 if (.not. (tacs_a .gt. one + rmargn)) go to 5573
  stpflg = 7
  go to 5570
5573 if (.not. (tacs_a .gt. one)) go to 5574
  tacs_a = one
  go to 5570
5574 continue
5570 continue
...
5499 if (.not. (ndx .eq. 335)) go to 5500
  if (.not. (tacs_a .lt. -one - rmargn)) go to 5591
  stpflg = 8
  go to 5590
5591 if (.not. (tacs_a .lt. -one)) go to 5592
  tacs_a = -one
  go to 5590
5592 if (.not. (tacs_a .gt. one + rmargn)) go to 5593
  stpflg = 8
  go to 5590
5593 if (.not. (tacs_a .gt. one)) go to 5594
  tacs_a = one
  go to 5590
5594 continue
5590 continue
...
5505 if (.not. (ndx .eq. 347)) go to 5506
  if (.not. (tacs_a .lt. one-rmargn)) go to 5611
  stpflg = 9
  go to 5610
5611 if (.not. (tacs_a .lt. one)) go to 5612
  tacs_c = zero
  go to 5610
5612 continue
  tacs_c = rnull
...
5506 if (.not. (ndx .eq. 349)) go to 5507
  if (.not. (tacs_a .lt. -one-rmargn)) go to 5621
  stpflg = 10
  go to 5620
5621 if (.not. (tacs_a .lt. -one)) go to 5622
  tacs_a = -one
  go to 5620
5622 if (.not. (tacs_a .gt. one + rmargn)) go to 5623
  stpflg = 10
  go to 5620
5623 if (.not. (tacs_a .gt. one)) go to 5624
  tacs_a = one
  go to 5620
5624 continue
5620 continue
...
5507 if (.not. (ndx .eq. 363)) go to 5508
  if (.not. (tacs_a .gt. rmargn)) go to 5641
  tacs_c = one
  go to 5640
5641 continue
  tacs_c = zero
...
5660 continue
  tacs_a = rsto(ptr6 + 1)
  tacs_b = rsto(ptr6 + 2)
  if (.not. (dabs (tacs_a) .le. rmargn .and. dabs (tacs_a) .le. rmargn)) go to 5851
  stpflg = 131
  stpr1 = tacs_a
  stpr2 = tacs_b
  continue
...
5851 continue
5850 continue
  tacs_c = datan (tacs_a / tacs_b)
  if (.not. (tacs_b .lt. zero)) go to 5671
  if (.not. (tacs_c .lt. zero)) go to 5681
  tacs_c = tacs_c + pi
  go to 5680
5681 continue
  tacs_c = tacs_c - pi
...
5690 continue
  ...
  tacs_b = rsto(rptr)
  tacs_b = dnint (tacs_b)
  if (.not. (tacs_a .lt. zero .or. tacs_b .le. zero .or. tacs_a .gt. tacs_b)) go to 5701
  stpflg = 13
  stpr1 = rsto(rptr)
  stpr2 = rsto(rptr + 1)
  continue
...
5701 continue
5700 continue
  if (.not. (tacs_a .eq. zero .or. tacs_a .eq. tacs_b)) go to 5711
  tacs_c = one
  go to 5710
5711 continue
  tacs_d = tacs_b - tacs_a
  if (.not. (tacs_a .gt. tacs_d)) go to 5721
  tacs_e = tacs_a
  tacs_a = tacs_d
  tacs_d = tacs_e
  go to 5720
...
5720 continue
  tacs_c = one
3171 if (.not. (tacs_d .lt. tacs_b)) go to 3170
  tacs_d = tacs_d + one
  tacs_c = tacs_c * tacs_d
  go to 3171
3170 continue
3181 if (.not. (tacs_a .gt. one)) go to 3180
  tacs_c = tacs_c / tacs_a
  tacs_a = tacs_a - one
  go to 3181
3180 continue
5710 continue
...
5730 continue
  ...
  tacs_b = rsto(rptr)
  tacs_b = dnint (tacs_b)
  if (.not. (tacs_a .lt. zero .or. tacs_b .le. zero .or. tacs_a .gt. tacs_b)) go to 5741
  stpflg = 15
  stpr1 = rsto(rptr)
  stpr2 = rsto(rptr + 1)
  continue
...
5740 continue
  tacs_d = tacs_b - tacs_a
  tacs_c = one
3191 if (.not. (tacs_d .lt. tacs_b)) go to 3190
  tacs_d = tacs_d + one
  tacs_c = tacs_c * tacs_d
  go to 3191
3190 continue
...
3201 if(.not.(rptr.gt.ptr6+1)) go to 3200
  rptr = rptr - 1
  tacs_b = rsto(rptr)
  if (.not. (tacs_b .lt. tacs_a)) go to 5751
  tacs_a = tacs_b
  go to 5750
5751 continue
5750 continue
...
3211 if (.not. (rptr .gt. ptr6 + 1)) go to 3210
  rptr = rptr - 1
  tacs_b = rsto(rptr)
  if (.not. (tacs_b .gt. tacs_a)) go to 5761
  tacs_a = tacs_b
  go to 5760
5761 continue
5760 continue
...
9028 continue
  tacs_a = rsto(rptr)
  tacs_a = tacs_a * tacs_a
3221 if (.not. (rptr .gt. ptr6 + 1)) go to 3220
  rptr = rptr - 1
  tacs_b = rsto(rptr)
  tacs_a = tacs_a + tacs_b * tacs_b
  go to 3221
3220 continue
  rsto(rptr) = dsqrt (tacs_a)
  go to 9500
9029 continue
  tacs_a = rsto(rptr)
3231 if (.not.(rptr .gt. ptr6 + 1 .and. tacs_a .gt. rmargn)) go to 3230
  rptr = rptr - 1
  tacs_a = rsto(rptr)
  go to 3231
3230 continue
  if (.not. (tacs_a.gt.rmargn)) go to 5771
  tacs_a = zero
  go to 5770
5771 continue
  tacs_a = one
...
9030 continue
  tacs_a = rsto(rptr)
3241 if (.not. (rptr .gt. ptr6 + 1 .and. tacs_a .le. rmargn)) go to 3240
  rptr = rptr - 1
  tacs_a = rsto(rptr)
  go to 3241
3240 continue
  if (.not. (tacs_a .le. rmargn)) go to 5781
  tacs_a = one
  go to 5780
5781 continue
  tacs_a = zero
...
3370 continue
  ...
  write (unit = bkfile, rec = ptr4) (isto(k + i), i = 1, cnt0)
  go to 6230
6231 if (.not. (ndx7 .eq. 2)) go to 6232
  write (unit = bkfile, rec = ptr4) (rsto(k + i), i = 1, cnt0)
  go to 6230
6232 continue
6230 continue
  go to 6220
...
3380 continue
  ...
  read (unit = bkfile, rec = ptr4) (isto(k + i), i = 1, cnt0)
  go to 6260
6261 if (.not. (ndx7 .eq. 2)) go to 6262
  read (unit = bkfile, rec = ptr4) (rsto(k + i), i = 1, cnt0)
  go to 6260
6262 continue
6260 continue
  go to 6250
