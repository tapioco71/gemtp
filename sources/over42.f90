...
142  psiold = psiold + d1*d2
  ...
  flux(n) = psiold
  write (lunit6, 147)
147 format ( 10x,  100(1h-)  ,/, 10x, 'Rremember.   The just-completed conversion began with a current vs. incremental inductance     ', /, &
...
500 if ( kthird  .ne.  1 )   go to 503
  ...
  do j=1, n1
     ...
     if ( ipnch  .eq.  0 ) write (lunit7, 505)  d1, d2
502 end do
503 do i=1, n
...
504  format( i10, 2f30.10 )
     if ( i  .eq.  1 )   go to 510
     if ( ipnch  .eq.  0 ) write (lunit7, 505)  currr(i), flux(i)
505  format ( 2e16.8 )
510 end do
  write (lunit6, 527)
527 format ( 36x, 4h9999 )
  if ( ipnch  .eq.  0 ) write (lunit7, 531)
...
subroutine hysdat
  ...
  h(49) =  1.60
  h(50) =  2.20
3548 if ( iprsup  .ge.  1 ) write (lunit6,90)
...
3648 format (  37h+begin b-h.  (itype, level, ipunch) =, 3i4  )
  if (itype .eq. 1) go to 125
  write (lunit6,110) itype
110 format(1x,40hjob is being halted- the value of itype=,i3, 2x,10his illegal)
  go to 3548
125 if( (level .ge. 1) .and. (level .le. 4) ) go to 150
  write (lunit6,130) level
130 format(1x,40hjob is being halted- the value of level=,i3, 2x,10his illegal)
  go to 3548
150 if ( iprsup  .ge.  1 ) write(lunit6,155)
...
1302 format(/, 31h required number of nodes equal, i10, 36his greater than maximum which equals, i10 )
  go to 9200
1308 continue
  nk1 = ndat + 1
  ndat = ndat + 12
...
1235 format( 10h+arrester., 3i4, 2e12.3 )
  if ( iprzno  .ge.  3 ) write (lunit6, 1240)
1240 format ( //,  22h begin next data case., 35h **********  **********  **********, 12h  **********     )
  jcase = jcase + 1
  ifit = 1
...
1328 format ( /,  35h illegal non-positive  "a1"  equals, e15.5  )
  go to 9200
1340 if( a2  .gt.  0.0  )   go   to  1360
  lstat(19) = 1348
  write (lunit6, 1348)  a2
1348 format ( /,  35h illegal non-positive  "a2"  equals, e15.5  )
  go to 9200
1360 if( a3  .gt.  0.0  )   go   to   1380
  lstat(19) = 1366
  write (lunit6, 1366)  a3
1366 format ( /,  35h illegal non-positive  "a3"  equals, e15.5  )
  go to 9200
1380 if( a4  .gt.  0.0  )   go   to   1400
  lstat(19) = 1387
  write (lunit6, 1387)  a4
1387 format ( /,  35h illegal non-positive  "a4"  equals, e15.5  )
  go to 9200
1400 if( amin .gt. 0.0  )   go  to  1420
  lstat(19) = 1402
  write (lunit6, 1402)  amin
1402 format ( /,  37h illegal non-positive  "amin"  equals, e15.5  )
  go to 9200
1420 if( a5 .eq. 0.0  )   a5 = 1.0
  if( vref .gt. 0.0 )  go  to  1421
  vref = 2.0 * a2
1421 if( a5 .eq. 1.0  .and.  vflash .eq. 0.0 ) go to 1425
  if( a5 .ne. 1.0  .and.  vflash .gt. 0.0 ) go to 1425
  lstat( 19 ) = 1421
  write( lunit6, 1422 ) a5, vflash
1422 format( /, 34h illegal "a5"-"vflash" combination, 2e15.5 )
  go to 9200
1425 if( iprzno  .lt. 3  )   go  to  1451
  write( lunit6, 1320 ) nexp, vref, iphase, vflash
1320 format( 5x, 19h number of segments, 5x, 18h reference voltage, 5x, 17h number of phases, &
          5x, 18h flashover voltage, /, 5x, i19, 5x, e18.8, 5x, i17, 5x, e18.8 )
  write( lunit6, 1440 ) a1,a2,a3,a4,a5
1440 format ( 2x, 38hconstants for conversion of input data    ,/,  7x, &
          15horiginal rating,  12x,  10hnew rating,  4x, &
          18hvoltage multiplier,  4x, 18hcurrent multiplier,  6x, &
          24hadditional blocks factor   ,/,  4e22.8,  e30.8  )
...
1587 format( /, 41h nonpositve or nonmomtonic "a" and/or "b", 23h the read-in values are, 2e15.6 )
  lstat( 19 ) = 1580
  go to 9200
1590 ck1 = a
  ci1 = b
  ...
  v(nk) = b
  c(nk)= a
  if ( iprzno  .ge.  8 ) write (lunit6, 1600)  a, b
...
1622 format (' Input data after conversion to a log-log plane  ==================  ', /, 23x,  7hcurrent, &
          18x, 7hvoltage  ,/,  ( 5x, 2e25.15 ) )
  if( ifit .lt. 0 )  go to 3100
  !     code for user defined segments***********************************
  if( iprsup .gt. 1 )   write (lunit6, 1640) d,e,f,g
1640 format ( 1x,  35hconstants for the fitting procedure   ,/, 4h d =,  e16.7,  5x,  3he =,  e16.7,  5x, &
          3hf =,  e16.7,  5x,  3hg =,  e16.7  )
  j = j + 1
  a = ( g - (d * e) / i ) / ( f - (d * d) / i )
  ju = j + jd
  s( ju ) = a
...
1650 bg( ju ) = bv
  if( iprzno  .gt.  1  ) write (lunit6, 1660)  j, i, s( ju ), bg( ju )
1660 format ( /,  27h calculated values.  j, i =,  2i5, 5x,      15hs(ju), bg(ju) =,  2e20.10  )
  nt( j ) = i
...
3400 end do
  ...
  s( ju ) = a
  nt( j ) = i1
  if( iprzno .gt. 1 ) write( lunit6, 1660 ) j, i1, s( ju ), bg( ju )
  go to 3300
...
1800 format ( /,  3x,  10h**********, 30h  error statistics  **********    ,/, &
          3x,   7hsegment,  14x,  11hlocal error, 8x,  17haccumulated error   )
  jk = 0
  b = 0.
  do j = 1, nexp
     a = 0.0
...
1860 format ( i10, 2x, 2e20.10 )
  ...
  write (lunit6, 1880)
1880 format ( /,  35h  comparison between input data and, 24h the recalculated points  ,/, &
          8x,  13hinput voltage,  7x,  13hinput current, 2x,  18hcalculated current,  10h  &&&&&&&&    )
  do  k = 1, jk
...
1940 if( a5 .eq. 0.0 .or.  a5  .eq. 1.0 )  go  to  2080
  ...
  if ( iprzno  .ge.  1 ) write (lunit6, 1960)   a5
1960 format ( /, 42h  reprocess input data.  multiplier "a5" =, e20.10 )
  if ( iprzno  .ge.  2 ) write (lunit6, 1622)
  jk = 0
  jkp = jkp + 1
  do 2000 j = 1, nexp
     i = nt(j)
...
1980 end do
     ...
     s(ju) = a
     bg( ju ) = bg( j ) - a * ala5
     if( iprzno .gt. 1 ) write (lunit6, 1660)   j, kp, s(ju), bg(ju)
2000 end do
  a5 = 0.
  go  to  1680
2020 continue
...
2240 format ( //,  35h  **********   80-column card-image, 33h listing of unit-7 punched cards., &
          10h   *******     )
  write (lunit6, 2243)
2243 format ( 1x, 40h----------------------------------------, &
          40h----------------------------------------   )
...
2248 format (  11h c rating =,  f9.2,  10h  v-mult =, e13.5,  10h  i-mult =,  e13.5,  2x,  2a4 )
  write (lunit7, 2252)  a2, a3, a4, text3, text4
2252 format (   10hc rating =,  f9.2,  10h  v-mult =, e13.5,  10h  i-mult =,  e13.5,  2x,  2a4 )
  go to 2320
...
2263 format( a2, 2a6, 25x, a5 )
     write (lunit6, 2341)
2341 format ( 1x,  1hc, 12x,  12h  vreference, 15x,  10hvflashover   )
     write( lunit6, 2381 ) vref, vflash
2381 format( 1x,2e25.15 )
     write (lunit7, 2343)
2343 format (      1hc, 12x,  12h  vreference, 15x,  10hvflashover   )
     write( lunit7, 2382 ) vref, vflash
2382 format( 2e25.15 )
     j = 0
     write (lunit6, 2340)
2340 format ( 1x,  1hc, 12x,  12h  multiplier, 17x,  8hexponent,  21x,  4hvmin   )
     write (lunit7, 2344)
2344 format (      1hc, 12x,  12h  multiplier, 17x,  8hexponent,  21x,  4hvmin   )
     do ij = 1, jkp
        ju = j + 1
        jk = j + nd( ij )
        do i = ju, jk
...
2420 end do
     go to 2440
2422 continue
     write( lunit6, 2264 ) text92,textna(nkc+1),textna(nkc+2), textna(1),textna(2),text55
2264 format( 1x, a2, 4a6, 13x, a5 )
     write( lunit7, 2265 ) text92,textna(nkc+1),textna(nkc+2), textna(1),textna(2),text55
...
3427 format( e16.0 )
  ...
  if( zk( 11 ) .eq. zk( 3 )  .and.  zk( 7 ) .eq. zk( 15 ) ) vflash = -100.
  vzero = zk( 4 )
  write( lunit7, 3422 )    vref, vflash, vzero
  write( lunit6, 3423 )    vref, vflash, vzero
  write( lunit7, 3422 )    ( zk( l ), l = 1, 3 )
