...
4567 format ( 24h  "begin module over12." )
  ...
  ikf = 0
  isfd = 0
  if (iprsup .ge. 1) write (lunit6, 2716)  kswtch, kbase, isw, nenerg, ( kdepsw(i), i=1, kswtch )
2716 format (/,  25h at beginning  'over12' ., 32h  kswtch   kbase     isw  nenerg,  /, &
       25x,  4i8  ,/,   37h (kdepsw(i), i=1, kswtch)  follow ...   ,/, ( 1x,  20i6 ))
  !     define lstat(15) as a flag for omit base case in a stat. run
  if ( kbase .eq. intinf)   lstat(15) = intinf
  if (lastov .eq. 1) go to 13000
  if ( iprsup  .ge.  1 ) write (lunit6, 4624)  knt, kbase, kloaep, mtape, nenerg, jseedr, ltdelt, kswtch
...
164 if ( isw  .eq.  4444 )   go to 4088
  ...
  if (iprsup .ge. 1) write (lunit6, 2721) n4, nword1, nword2, ltlabl
  call tables
  rewind  lunt10
  read (lunt10)  n8
  if ( n8  .le.  0 )   go to 7246
  do i=1, n8
     read (lunt10)  j, akey(j), tstat(j), topen(j + lswtch)
     if ( iprsup  .ge.  2 ) write (lunit6, 7242) i,j,akey(j),tstat(j),topen(j+lswtch)
7242 format ( 20h stat. open.  i, j =,  2i5,  5x,  3e15.6 )
7245 end do
7246 inecho = 0
  rewind lunt12
  if ( knt .eq. 1 ) mtape = lunit5
  lunit5 = lunt12
  rewind lunit5
...
76403 if ( loopss(1) .ne. 7766 )  go to 13000
  if ( iprsup .ge. 1 ) write (lunit6, 7474)  numum, it1, ibr
7474 format ( 37h in "over12".  b4 sneaky restoration., 20h   numum, it1, ibr =,  3i8  )
...
13000 n1 = nchain
  ...
  rewind lunit9
  write (lunit9)  ntot, nenerg
  if (jflsos .gt. 0)  write (lunit9) pu
  inecho = lunt12
  rewind lunt12
13010 if ( kbase .ge. 2  .and.  kbase .ne. intinf )   go to 156
  ...
  iout = ioutin
  if (iprsup .ge. 1) write (lunit6, 2719)  iout, indstp, multpr, kprchg
2719 format(/, 28h in 'over12' after 'tacs2' .,33h    iout  indstp  multpr  kprchg  ,/,  31x, 4i8  )
...
156 iftail = 0
  ...
  if (iprsup .ge. 1) write (lunit6, 2721) n4, nword1, nword2, ltlabl, ktab, isw, lunit2, iv, lbrnch
2721 format ( /,  27h unit 2 read/write indices., 32h      n4  nword1  nword2  ltlabl, &
       32h    ktab     isw  lunit2      iv, 8h  lbrnch  ,/,  27x,  9i8  )
  rewind lunt10
  n8 = 0
  do i=1, kswtch
     if ( akey(i)  .ne.  -44444. )   go to 7256
     n8 = n8 + 1
7256 end do
  write (lunt10)  n8
  do i=1, kswtch
     if ( akey(i)  .ne.  -44444. )   go to 7259
     j = i + lswtch
     write (lunt10)  i, akey(i), tstat(i), topen(j)
7259 end do
  if ( kbase .ne. intinf )  go to 7263
  knt = 1
  go to 166
7263 nenerg = 0
  if ( iprsup  .ge.  1 ) write (lunit6, 4624)  knt, kbase, kloaep, mtape
  if ( iprsup  .ge.  4 ) write (lunit6, 76)  kswtch, kloaep, ( kdepsw(ip), ip=1, ll6 )
76 format (/, 6h at 76, 2i10, /, 1x, 6i5)
  do j = 1, kswtch
     if ( absz(akey(j)) .ne. 44444.)   go to 42630
     n = kdepsw(j)
     if ( n  .eq.  0 )   go to 42300
     tma = tclose(n)
     if (akey(n) .eq. -44444.) tma = topen(n)
     tmb = tclose(j)
     if (akey(j) .eq. -44444.) tmb = topen(j)
     tmc = tma + tmb
     if (akey(j) .eq.  44444.) tclose(j) = tmc
     if (akey(j) .eq. -44444.) topen(j) = tmc
     go to 42500
42300 if ( kloaep  .le.  0 )  go to 42500
     tmt = tclose(kloaep)
     if ( akey(kloaep) .eq. -44444.) tmt = topen(kloaep)
     if ( akey(j) .eq. 44444. )  tclose(j) = tmt
     if ( akey (j) .eq. -44444. )  topen(j) = tmt
42500 crit(j) = 0.0
     if ( akey(j)  .eq.  -44444. )   go to 42630
     topen(j) = fltinf
     akey(j) = 0.0
42630 end do
  if (iprsup .lt. 1)  go to  598
  write (lunit6, 52630)  (tclose(j), j=1, kswtch)
52630 format (31h at 52630, tclose after sorting  ,/,(1x,8e15.6))
  write (lunit6, 5263)  (topen(j), j=1, kswtch)
5263 format (29h at 5263, topen after sorting  ,/,(1x,8e15.6))
  go to 598
4088 ii = 0
  write (lunit3)  (bus(i), i= 1, ntot)
  do i = 1 , kswtch
     j = lswtch + i
     write (lunit3) kmswit(i), kmswit(j), akey(i), tstat(i), topen(j), kdepsw(i)
  end do
  ...
  n3 = 0
  do i = 1, kswtch
     if ( absz(akey(i)) .ne. 44444.)   go to 4090
     n3 = n3 + 1
4090 end do
  n4 = nbyte(3) / nbyte(4)
  iofcnt = n4 * ncompt
...
4097 if ( iprsup  .ge.  1 ) go to 4095
  go to 4099
4095 write (lunit6, 2345)
2345 format ( /, 20h at 2345 of over12  )
  do i = 1 , kswtch
     j = lswtch + i
4096 write (lunit6, 61234) kmswit(i), kmswit(j), akey(i), tstat(i), topen(j)
  end do
...
409 if (nenerg .lt. 0)   go to 188
  ...
  d8 = randnm(seed)
  if ( iprsup  .ge.  1 ) write (lunit6, 646)  knt, kswtch, jseedr, nenerg, itest, idist, seed, d8
646 format ( /,  48h in  =over12=  just after 1st use of  =randnm= . , &
         /, 1x, 48h     knt  kswtch  jseedr  nenerg   itest   idist, &
         16x, 4hseed, 18x,  2hd8  ,/,  1x,  6i8,  2e20.10 )
  bias = 0.0
...
1615 secfrq = (1.0/statfr)/360.
  ...
  if(window.lt.0.0) window = 360. * secfrq
  zerofl = 0.0
  d14 = knt
  d14 = d14 / nenerg
  do j = 1, nswtpe
...
1646 do ksw = 1, kswtch
     if ( absz(akey(ksw))  .ne. 44444.)   go to 700
     ndxi = ksw + lswtch
...
1663 if( itest .gt. 2 )  go to  662
     if ( akey(ksw) .eq. +44444. ) go to 663
     bias = 0.0
     go to 664
663  bias = ranoff
     go to 664
662  if ( akey(ksw) .eq. -44444. )  go to 663
     bias = 0.0
664  timev = tstat(ksw)
     n = kdepsw(ksw)
...
665  rng = randnm(zerofl)
     if ( iprsup  .ge.  2 ) write (lunit6, 666 )  ksw, rng, bias, angle
666  format ( /,  49h at 666  of  "over12" .   ksw, rng, bias, angle =, 1i10, 3e16.6  )
     if ( idist  .gt.  0 )  go to 675
     if( topen(ndxi) .lt. 0.0 ) go to 675
     d9 = rng
     if ( d9  .eq.  0.0 )   go to 665
     if ( d9  .eq.  1.0 )   go to 665
     if ( 2.0*d9  .gt.  1.0 )   d9 = 1.0 - d9
     d7 = alogz( 1.0/(d9*d9) )
     eh = sqrtz(d7)
     xn=2.515517+eh*(0.802853+0.010328*eh)
     xd=1.0+eh*(1.432788+eh*(0.189269+0.001308*eh))
     xq=eh-xn/xd
     if ( xq  .gt.  sigmax ) xq = sigmax
     if ( 2.0*rng  .lt.  1.0 )   xq = -xq
     rng = timev + topen(ndxi)*xq + bias
     go to 680
675  d1 = 3.0
     rng = timev + bias + sqrtz(d1) * absz(topen(ndxi)) * (2.0 * rng - 1.0)
680  if ( akey(ksw) .eq. -44444. )   go to 7680
     tclose(ksw) = rng
     if ( n .eq. 0) go to 700
     if ( akey(n) .eq.  44444. ) tclose(ksw) = tclose(n) + tclose(ksw)  -  bias
     if ( akey(n) .eq. -44444. ) tclose(ksw) = topen(n) + tclose(ksw)  -  bias
     if (tclose(ksw).lt.deltat) go to 690
     go to 700
7680 topen(ksw) = rng
     if ( n .eq. 0) go to 700
     if ( akey(n) .eq.  44444. ) topen(ksw) = tclose(n) + topen(ksw)  -  bias
     if ( akey(n) .eq. -44444. ) topen(ksw) = topen(n) + topen(ksw)  -  bias
...
690  call time44(atim(1))
     ...
     rng = randnm(zerofl)
     ltdelt=ltdelt+1
     if ( akey(ksw) .eq. 44444. )  tclose(ksw) = 0.0
     if ( akey(ksw) .eq. -44444. )  topen(ksw) = 0.0
700 end do
  if ( moncar(7) .le. 0 )  go to 300
  !     now read user-supplied tclose of "user supplied switch times"
  n17 = moncar(7)
  read (n17, 4372)  ( tclose(ksw), ksw=1, kswtch )
  read (n17, 4372)  (  topen(ksw), ksw=1, kswtch )
4372 format ( 5e15.6 )
300 write (lunit3)  ranoff, (tclose(k1), k1 = 1, kswtch)
  write (lunit3)  ranoff, (topen(k1), k1 = 1, kswtch)
  if ( iprsup  .ge.  1 ) write (lunit6, 4624)  knt, kbase, kloaep, mtape, nenerg, jseedr, ltdelt, kswtch,  seedr, ranoff, &
       (tclose(k1), k1 = 1, kswtch), (topen(k), k = 1, kswtch)
4624 format ( /,  " at 4624 of  'over12' .  ", '     knt   kbase  kloaep   mtape  nenerg  jseedr  lt             delt  kswtch   ', 10x, 'seedr', 9x, 'ranoff', /, 23x, &
       8i8, 2e15.5, /, (4x, 8e16.6))
  if (isw .ne. 4444)   go to 703
  npair = 0
  irbase = ioftab
  if (idist .gt. 0)   go to 840
  do j = 1 , kswtch
     if ( absz(akey(j)) .ne. 44444.)   go to 830
     k = j + lswtch
     if (topen(k) .lt. 0)   go to 830
...
754  j1 = j - 1
     do i=1, j1
        if ( absz ( akey(i) ) .ne. 44444.)   go to 820
        if ( i .eq. j )  go to 820
        ni = i + lswtch
        if ( topen(ni) .lt. 0 )  go  to 820
        npair = npair + 1
...
800     sumdt = sumdt * aincr
        k1 = tdiff/sumdt
        if (iprsup .ge. 1) write (lunit6, 803)  npair, tdiff, sumdt, k1
803     format (22h npair,tdiff,sumdt,k1=, 10x, i6, 2e15.6, i6)
        if (tdiff .lt. 0.0)   go to 805
        k1 = k1 + nhalf + 1
        go to 810
805     k1 = k1 + nhalf
810     k1 = k1 *n4 - 1
...
840 continue
  ...
  irbase = ioftab
  do j = 1, kswtch
     if ( absz(akey(j)) .ne. 44444.)   go to 895
     j1 = j - 1
     do i = 1, j1
        if ( absz(akey(i)) .ne. 44444.)   go to 890
        if (i .eq. j)   go to 890
        npair = npair + 1
        k1 = kmswit(j)
        ndx1 = lswtch + j
        ...
        bus3 = bus(k3)
        bus4 = bus(k4)
        write (lunit6, 845)  npair, bus1, bus2, bus3, bus4
845     format (//, 12h switch pair, i4, 15x, 3h  ', a6, 8h'  to  ', a6, 11h'   and   ', a6, 8h'  to  ', a6, 1h')
        if (idist .gt. 0)   go to 850
        if (topen(ndx1) .lt. 0)   go to 850
        if (topen(ndx2) .lt. 0)   go to 850
        go to 858
850     write (lunit6, 855)
...
858     n3 = ncompt- 1
        do l = 1, n3
           ...
           d1 = .3989423 * expz ( -d9*d9*onehaf )
           d8 = 1.0 - d1* t * ((((1.330274 *t -1.821256) * t + 1.781478) * t - .3565638) * t + .3193815 )
           if ( d9  .ge.  0.0 )  go to 864
           d8 = 1.0 - d8
864        n9 = m + ncompt
           frandn(n9) = d8
868     end do
...
869     if (n2 .gt. ncompt)   n2 = ncompt
        write (lunit6, 870)  (frandn(l), l= n1, n2)
870     format (1x, 4htime, 10x, 14f8.4)
        n6 = n1 + ncompt
        n7 = n2 + ncompt
        write (lunit6, 871)  (frandn(l), l = n6, n7)
871     format (1x, 6hsample, 8x, 14f8.4)
        n8 = n1 + n3
        n9 = n2 + n3
        write (lunit6, 872)  (frandn(l), l = n8, n9)
872     format (1x,11htheoretical,3x, 14f8.4,/,1x)
        n1 = n1 + 14
        if (n1 .gt. ncompt)   go to 890
        n2 = n2 + 14
        go to 869
890  end do
     if (ii .eq. 0)   go to 895
     write (lunit6, 893)  ii, nenerg, sigmax
...
703 if (isw .eq. 0)   go to 702
  ...
  do k1 = 1, kswtch
     ...
     if ( akey(k1) .ne.  44444. )   go to 4216
     volti(n6+1) = tclose(k1)
     go to 4213
4208 if ( akey(k1) .ne. -44444. )   go to 4216
     volti(n6+1) = topen(k1)
4213 n6 = n6 + 1
...
4215 lastxx(n6) = k1
4216 end do
  write (lunit6, 701) knt, (lastxx(k1), volti(k1), k1=1, n6 )
...
200 tstbeg = tstat( klorow )
  ...
  tstbeg = tstbeg - 0.5 * ( tdns(klorow) - 1.0 ) * topen(nj)
203 if ( akey(klorow) .eq. 44444. ) tclose(klorow) = tstbeg + (indtv(ilorow) - 1) * topen(nj)
  if ( akey(klorow) .eq. -44444. ) topen(klorow) = tstbeg + (indtv(ilorow) - 1) * topen(nj)
  k = 1
  do i = 1, kswtch
     if ( k  .ne.  ilorow )   go to 207
     k = k + 1
     if ( indtv(k)  .eq.  0 )  indtv(k) = 1
207  if ( i  .eq.  klorow )   go to 230
     if (ipntv(k) .eq. 0)   go to 210
     if (i .eq. ipntv(k))   go to 220
210  if ( absz(akey(i)) .ne. 44444.)   go to 230
     n = kdepsw(i)
     if ( akey(i) .eq. 44444. )  tclose(i) = tclose(n) + tstat(i)
     if ( akey(i) .eq. -44444. )  topen(i) = topen(n) + tstat(i)
     go to 230
220  j = i + lswtch
     if (kloaep .eq. 0)   go to 225
     n2 = ipntv(k)
     if ( akey(i) .eq. 44444. ) tclose(i) = tclose(kloaep) + topen(j)*(indtv(k)-.5-.5*tdns(n2))
     if ( akey(i) .eq. -44444. ) topen(i) = topen(kloaep) + topen(j) * (indtv(k)-.5-.5*tdns(n2))
     go to 228
225  tstbeg = tstat(i)
     if (itest .gt. 0)   go to 227
     tstbeg = tstat(i) - 0.5 * ( tdns(i) - 1.0 ) * topen(j)
227  if ( akey(i) .eq. 44444. ) tclose(i) = tstbeg + (indtv(k) - 1) * topen(j)
     if ( akey(i) .eq. -44444. ) topen(i) = tstbeg + (indtv(k) - 1) * topen(j)
228  k = k + 1
     if (indtv(k) .eq. 0)   indtv(k) = 1
230 end do
  go to 300
  !     preparing r,x,
598 d2 = 1.0 / (deltat * 500. )
...
5763 if ( iprsup  .lt.  2 )  go to 6300
  write (lunit6,6)   nenerg
6 format (/, 32h at s.n. 6 of over12.   nenerg =, i3 )
  do i = 1 , kswtch
     ndx3 = lswtch + i
     write (lunit6, 9) kmswit(i), kmswit(ndx3), akey(i),tstat(i), topen(ndx3)
  end do
...
5527 j = length(k)
  ...
  d22 = d2
  ci1 = d2 / 1000.
  if( xopt .gt. 0.0 )  d22 = d2 * 1000. / ( twopi * xopt )
  ...
  n1 = iabs( nr(k) )
  n2 = n1 - 1  +  j*(j+1)/2
  if( iprsup .ge. 2 ) write(lunit6, 5528)  k, length(k), nr(k), n1, n2, d22
5528 format( /,   9h at 5528.   , 5i10, e15.5 )
...
433 if ( kswtch  .eq.  0 )   go to 430
  ...
  go to 477
455 write (lunit6, 2666)
2666 format ( /, " switch table vectors, before switch table processing in  'over12' .       ", /, &
       '     row   bus1    bus2     kpos  isourc', 14x, 'tclose',  15x,  'topen',  16x, 'crit', 14x, &
       'fkposi')
  do k = 1, kswtch
     ndx4 = lswtch + k
     write (lunit6,2713) k, kmswit(k), kmswit(ndx4), kpos(k), isourc(k), kswtyp(k), tclose(k), topen(k), crit(k)
  end do
2713 format  ( 6i8,  3e20.10 )
477 if ( iprsup  .ge.  1 ) write (lunit6, 2714)  ( k, energy(k), adelay(k), k=1, kswtch )
2714 format ( /, 38h continuation of switch table vectors.  ,/, 5x, 3hrow, 9x, 6henergy,  9x, &
       9x, 6hadelay, /, ( i8, 2e15.6 ) )
  do k=1,kswtch
     n1 = kmswit(k)
     ndx5 = lswtch + k
     n2 = kmswit(ndx5)
     icheck = kswtyp(k)
     if ( absz(adelay(k)) .eq. 44444.)   go to 422
     if(adelay(k).le.0.) go to 428
     adelay(ndx5) = adelay(k)
     go to 422
428  if(icheck.le.0) go to 422
     if(istead.eq.0) go to 422
     if (icheck .eq. 9976)  go to 422
     nn1=kbus(icheck)
     n15 = nn1
     if ( n15  .eq.  1 )   n15 = n2
     n16 = iabs ( kssfrq(n15) )
...
430 k=1
  ...
  n1 = ibr+1
  if ( n1  .lt.  10 )   n1 = 10
  if ( iprsup  .ge.  2 ) write (lunit6, 5306)  ( i, kbus(i), mbus(i), length(i), kodebr(i), kodsem(i), indhst(i), &
       cik(i), cki(i),ckkjm(i), i=1, n1 )
...
500 if(kbus(k).lt.0) go to 535
  ...
  ii = l + jt - 1
  if( iprsup .ge. 2 ) write(lunit6, 4435)  ( cmr(i), i=1, jt )
4435 format( /,  31h at 4435.  intermed. matrix (d)  ,/, ( 1x,6e20.10))
  ...
  if( iprsup .ge. 2 ) write(lunit6, 4451)  k, l, it2, ( x(i), r(i), i=l, ii )
4451 format( /, 40h at 4451.  final (g), (h) from (a), (b).   ,3i10, /, ( 1x, 6e20.10) )
  go to 520
...
535 it2=iabs(length(k))
  ...
  n1 = k
  n3 = 1
5350 if (cki(n1) .lt. 0.0) go to 5353
  n1 = n1 + 1
  n3 = n3 + 1
  go to 5350
5353 n2 = k + n3 - 1
...
534 if (ck(k) .ge. 0.0) go to 6482
  ...
  i = nhist(kf)
  nr(k) = i
  if( iprsup .ge. 2 ) write(lunit6, 20601)  kf, i, k, ii, iline, it2, kbus(k), mbus(k)
20601 format ( /,  22h freq. dependent mode., 32h      kf       i       k      ii, &
       32h   iline     it2    kbus    mbus  ,/,  22x,  8i8  )
...
8892 format( ' in over12, offset variable isecti =', i5, ' is too samll.  it should be increased to the value of ',i5, &
       /, 'which is the number of coupled phases currently modelled', ' with new marti frequency-dependent format.     stop.')
  stop
8891 do iq =1, nqtw
...
8893 koff21 = koff20 + nqtt
  ...
  do j = 1, nrz
     ...
     if ( iprsup .gt. 0 ) write(*,5487)j,nrz,sconst(n5),sconst(n6),sconst(n7),sconst(n8),sconst(n9)
5487 format(30h j,nrz,cj,dj,ej,esum,d0sum=y05,/,1x,i3,2x,i3,3x,e14.5,3x,e14.5,3x,e14.5,3x,e14.5,3x,e14.5)
50007 end do
  jgl = 0
  !     if ( iprsup .gt. 0 )
  !    1 write(lunit6,4445)
  ...
  do j = 1, nra
     ...
     if ( absz( sconst(n2) ) .eq. 0.0 ) go to 5324
     fac1 = sconst(n3) * deltat
     fac2 = sconst(n2) / sconst(n3)
     sconst(n5) = expz(-fac1)
     hi = (one - sconst(n5)) / fac1
...
5331 if ( iprsup  .lt.  4 )   go to 5335
  if (noutpr .eq. 0) write (lunit6, 5334)  k, ii, length(k), iline, it2, i, n3, h3, cik(k)
5334 format ( /,  35h after processing new mode of line.   ,/, 56h       k      ii  length   iline     it2       i      n3, 13x, &
       2hh3,  9x,  6hcik(k)   ,/,  7i8,  2e15.6  )
...
81520 k = k + iabs(it2)
  ...
  write (lunit6, 208)   ( k, kbus(k), mbus(k), nr(k), kodebr(k), length(k), kodsem(k), indhst(k), k=1, ibr )
...
209 format ( /, 5x, 3hrow, 7x, 3hbus, 13x, 2hci, 13x, 2hck, 12x, 3hcik, 12x, 3hcki,10x,5hckkjm,/,( i8,4x,a6,5e15.6 ) )
  write (lunit6, 4394) (sconst(i), i=1, ifsem)
4394 format(/, 24h (sconst(i), i=1, ifsem)   ,/, 8(1x,e15.7))
  write (lunit6, 4395) (cnvhst(i), i=1, ifx)
...
4568 format ( 22h exit module "over12".  )
99999 return
...
1 format(41h integer counters at start of dteqiv.....,17x,3hikf,6x,4hisfd,6x,4hizkf,6x,4hipkf,/,51x,4i10)
  d21 = d2 * delta2
  do kb = 1, 2
     ur(kb) = 0.
     kc = kb + idk
...
4    ur(kb) = 1. / ur(kb)
  ...
  azr = ur(1)
  azi = ur(2)
  if( iprsup .gt. 0 ) write( lunit6, 5 ) azr, azi
5 format(37h equivalent branch impedances (modal),4x,2e22.10)
  return
...
1001 format (33h enter "tacs2".  lastov, m4plot =,  2i6)
  do i = 1, 8
     lstat( i) = lstat(i+60)
  end do
  picon = 360. / twopi
  ...
  write (lunit6, 6213) ioutcs, iplot, iprsup, ntcsex, nstacs, ktab, niu, nuk, nenerg, iuty(kiuty+11)
6213 format ( /,  1x, 48h  ioutcs   iplot  iprsup  ntcsex  nstacs    ktab, &
       32h     niu     nuk  nenerg  infexp, /, 10i8  )
  write (lunit6, 6215)  t, twopi, fltinf, xopt, copt
6215 format ( 1x, 14x, 1ht,  10x, 5htwopi, 9x, 6hfltinf, 11x, 4hxopt,  11x, 4hcopt   ,/, &
       1x, 8e15.6)
6219 kxai = kxar + lstat(68)
  ...
  do i = niunrs, niu
     ndy5 = ndy5 + 5
     if (iuty(kiuty+i) .ne. 14 )  go to 6223
...
484  end do
     ...
     lstat(14) = i
     lstat(19) = 490
     write(lunit6,166) vstacs(i), i
166  format(22h0error.  emtp source =,a6,8h= (type ,i2,46h) is not a recognizable variable name in tacs.)
     go to 9000
490  vstacs(i) = char(j)
     go to 482
483  vstacs(i) = char(0)
...
480 nuki = kisblk - 8
  ...
  do i = 1, nuk
     nuki = nuki + 8
     nukr = nukr + 4
     if ( isblk(nuki+2)  .eq.  1 )  go to 215
     j = isblk( nuki + 3 )
     k = isblk( nuki + 2 )
     n1 = j + 8
     n2 = j - 1 + k * 6
     ...
     f1 = 1.0
     ndx2 = kprsup + j + 2
     if ( parsup( ndx2) .eq. 0.0 )  go to 4061
...
3678 format ( /,   9h ntotac =,  i4, 49h .   ( ismtac( j ),  j = 1, ntotac )  follows .       ,/, &
       (  12( 1x, i5 ) ) )
  ndy5 = kud1
  do i = niunrs, niu
     ndx1 = ilntab( kaliu + i )
...
406 if ( nuk .eq. 0 )  go to 271
  ...
  do i = 1, nuk
     nuki = nuki + 8
     if ( isblk(nuki+2)  .eq.  1 )  go to 4747
...
270  if ( iprsup  .lt.  2 )  go to 238
     ...
     write ( lunit6, 4412 ) i, ( parsup(n), n = n1, n2, 6 )
4412 format('   function number', i8 ,/, 8h  pn    , 8e15.6)
     write ( lunit6, 4414 ) ( parsup(n+1), n = n1, n2, 6 )
...
32201 if ( n .ge. ioutcs ) go to 233
  ...
  write (lunit6, 235) ( dumj(i), i = 1, j )
235 format ( /,  17x,  9(6x, a6)  )
  go to 32201
233 write (lunit6, 235)
  !                                         $$$  form  sparse  matrix  $$$
...
260 end do
  ...
  write (lunit6, 2023)  ia, (isblk(kisblk+n*8), n=1, nuk)
2023 format(1h0/4h ia=,i4,8x,27hnext is irowcs(1,2,...,nuk)/(1x,30i4))
  write (lunit6, 2024) (icolcs(kcolcs+m),atcs(katcs+m), m=1, ia)
2024 format (16h0icolcs and atcs ,/, (1x, 6(i3,e15.6,2x)))
...
4477 kill = 500
     write ( lunit6, 4488 )
4488 format( 51h ***** the crest of field winding voltage source of, 39h #59 machine is beyond its limit ***** &
          ,/,   45h stop at tacs2 of over12 near statement 4499  )
     go to 9000
...
6513 continue
  ...
  do i=1, iofbnd
     ...
     go to 1762
1761 read (lunit2)  d1, ( volti(j), j=1, iofgnd )
...
1763 end do
  ...
  epsuba = t
  ndx1 = ndx1 + niunrs
  call mover ( volti(1), xtcs(ndx1), iofgnd )
  go to 505
1768 ndy5 = kud1 - 5
  do i = niunrs, niu
     ndy5 = ndy5 + 5
...
4321 if ( isprin  .gt.  0 )   go to 320
  ...
  write (lunit6, 321) istep, t, (atcs(i), i=ndx1, ndx2)
...
32819 if (ndx2 .ge. ndx1) go to 320
  ...
  write (lunit6, 322) (atcs(i), i=ndx3, ndx2)
...
320 if ( isplot  .gt.  0 )   go to 310
  ...
  volti(1) = t
  call mover ( atcs(ndx1), volti(2), ioutcs )
  ndx2 = ioutcs + 1
  call pltfil ( ndx2 )
  go to 1649
1648 ndx2 = kbtcs + ioutcs
  write (lunit4) t, (atcs(i), i=ndx1, ndx2)
1649 isplot = iplot
  !                                                $$$  time  control  $$$
310 if ( t  .gt.  tmax )   go to 900
  if ( iprsup  .ge.  3 ) write (lunit6, 8461)   istep, isprin, isplot, iac, idctcs
8461 format ( /,  37h in  'tacs2' ,   increment  'istep' ., 40h   istep  isprin  isplot     iac  idctcs   ,/, &
          3 x,  7i8)
  istep = istep + 1
  t = istep * deltat
...
3111 end do
  ...
  go to 250
  !                                                  $$$  termination  $$$
900 if ( iprsup  .ge.  1 ) write(lunit6,901)
901 format(37h0normal termination within  'tacs2' .   )
  ...
  if ( begmax(1)  .eq.  0.0 ) write (lunit6, 904)
904 format ( /, 47h minima and maxima of all tacs output variables  )
  if ( begmax(1)  .eq.  1.0 ) write (lunit6, 42905)
42905 format ( /, 37h minima and maxima of all tacs output, 29h variables  (extrema over all, &
  ...
  if ( begmax(1)  .eq.  2.0 ) write (lunit6, 52905)
...
62905 format ( 7x, 3hrow, 6x, 4hname, 8x, 7hmaximum, 8x,7hminimum, 9x, 11htime of max, 4x,  11htime of min    )
  do i=1,ioutcs
     ...
     ndx4 = kawkcs + i
     ndx5 = kbwkcs + i
905  write(lunit6,906) i, texvec(ndx1),xar(ndx2),xar(ndx3),awkcs(ndx4),awkcs(ndx5)
  end do
...
3260 end do
     ...
     write (lunit6, 2023)  ia, (isblk(kisblk+n*8), n=1, nuk)
     write (lunit6, 2025)  ( icolcs(kcolcs+m), atcs(katcs+m),atcs(kbtcs+m),  m=1, ia )
2025 format(/,  19h icolcs, atcs, btcs   ,/,(1x,3(i3,2h: ,e13.6,6h  + j ,e13.6)))
     write (lunit6, 13019)
13019 format ( //,  35h calc. of phasor forcing functions.   ,/,1x, 32h     row       j  idctcs     niu, &
          9x, 6hxar(j), 9x, 6hxai(j)    )
     !     ***  set  inputs  ***
3019 ndx1 = kxar   + 1
     call mover0 ( xar(ndx1), ktab )
     ndx1 = kxai   + 1
     call mover0 ( xar(ndx1), ktab )
     ndy5 = kud1 - 5
     do i = niunrs, niu
        ndy5 = ndy5 + 5
        if ( ud1(ndy5+4) .ge. 0.0 )  go to 3020
...
3003    d1 = ud1( ndy5 + 1 )
        ...
        xar(ndx3) = d1 * cosz(d2)
        xar(ndx2) = d1 * sinz(d2)
3030    if ( iprsup  .ge.  1 )  write ( lunit6, 13001 ) i, j, idctcs, niu, xar(ndx3), xar(ndx2)
13001   format ( 1x, 4i8, 2e15.6 )
3020 end do
     if ( iprsup  .ge.  1 ) write (lunit6, 13020) (xar(kxar+i), xar(kxai+i), i=1, ktab)
13020 format ( /,  28h (xar(i), xai(i), i=1, ktab)  ,/,  (1x, 8e16.6) )
3412 ite = ite + 1
     ipass = 0
...
3334    m = m + 1
        ...
        if ( iprsup  .ge.  2 ) write (lunit6, 13334)  i, j, k, m, n, d1, d2, xar(ndx1), xar(ndx2)
13334   format ( 9h at 13334, 5i8, 4e20.6 )
        go to 3334
3024    l = isblk( nukj + 7 )
        if ( l  .gt.  0 ) call csupac( l, omegar )
...
2389 ndx1 = kxtcs + lstat(68)
     ...
     d5 = d1/d4
     d6 = -d3/d4
     if ( iprsup  .ge.  1 ) write (lunit6, 1304)  ktab, omegar, deltat, d2, d3, d5, d6
1304 format (/, 5x, 4hktab, 9x, 6homegar, 9x, 6hdeltat, 13x, 2hd2,13x, 2hd3, 13x, 2hd5, 13x, 2hd6, /, 1x, i8, 6e15.6, /, 1x )
     ...
     do i=1,nuk
        ...
        ndxj = kbwkcs + i
        awkcs(ndxj) = 0.0
        if ( iprsup  .ge.  4 ) write (lunit6, 43033)
43033   format (/, 9x, 40h       i       m       j       k       n, 18x,2hd1, 12x, 8hawkcs(i), 12x, 8hbwkcs(i)     )
        do m=j,k
           ...
           if ( iprsup  .ge.  4 ) write (lunit6, 13033)  i, m, j, k, n, d1, awkcs(ndxi), awkcs(ndxj)
13033      format ( 9h at 13033, 5i8, 3e20.6 )
3033    end do
        kh = isblk( nuki + 2 )
        k = kh - 1
        j = isblk( nuki + 3 ) + k * 6
        if ( iprsup  .ge.  4 )  write ( lunit6, 53033 )
53033   format (/, 9x, 48h       i       m       j       k      j1      m1, 12x, 8hhstr(m1), 12x, 8hhsti(m1)    )
        do m = 1, k
           ...
           if ( iprsup  .ge.  4 ) write (lunit6, 23033) i, j, k, kh, atcs(ndx1), atcs(ndx4)
23033      format ( 9h at 23033, 4i8, 2e20.6 )
           ...
           if ( iprsup  .ge.  4 ) write (lunit6, 33033)  atcs(ndx1), atcs(ndx4)
...
8888 ite = 0
  ...
  do i = niunrs, niu
     mdy5 = mdy5 + 5
     if ( ud1(mdy5+3) .lt. 0.0 ) ud1(mdy5+3) = - ud1(mdy5+3)
  end do
  go to 405
...
1001 format(36h0entering subroutine  csupdc  at  t=, e13.6   ,/, 8h0e nsup=, i6, 8h   karg=, i8,  8h   kpar=, i6 )
...
653 if ( parsup(nn+3) .eq. 0.0 ) go to 10
  ...
  go to 11
  !     ---  digitizer  ---
655 if ( parsup(nn+1) .ne. 1.0 ) go to 10
  m = ivarb( n1 + 3 )
  n = ivarb( n1 + 4 )
...
656 if ( parsup(nn+1) .ne. 1.0 ) go to 10
  m = ivarb( n1 + 3 )
  n = ivarb( n1 + 4 )
  if ( parsup(nn) .ne. 0.0 )  b = b * parsup(nn)
  ndx1 = kprsup +  m + 1
...
662 contss = parsup( nn + 1 )
  if ( contss  .lt.  0.5 ) go to 10
  a = parsup( nn + 2 )
  if ( contss  .gt.  1.5 )  a = b
  go to 11
  !     --- min/max tracking, controlled accumulator or couhter ---
664 if ( parsup(nn)  .eq.  0.0 )  go to 10
  if ( parsup(nn)  .eq.  99999. ) parsup(nn) = 0.0
...
9999 return
  ...
  subroutine premec
    ...
    n17 = 0
    do ilk = 1, nst
       if ( iprsup  .ge.  2 ) write (lunit6, 5289)  ilk
    end do
5289 format ( /,  38h begin next gen in  'premec' .     ilk, /, 30x,  i8   )
    i30 = 30 * ilk - 29
    ...
    num8 = num4 + num4
    iml = imk + 1
    call mover0( shp(iml), num2 )
    iml = iml + num8
    call mover0( shp(iml), num4 )
    if( iprsup .gt. 0 ) write( lunit6, 90 ) numask, num2, num4, imk, nst
90  format(2x,16hin premec b4 708,/,2x,13i10)
...
711 end do
    ...
    write(lunit6,901) ilk
901 format(2x,35hconstant matrices for generator no.,i5)
    id = ic + 1
    ik = ic + num4
    write( lunit6, 902 )  id, ik, ( shp(ii), ii = id, ik )
902 format(2x,16hmatrix y(i), i =,i5,2x,2hto,i5,/,2x,(2x,6e21.12))
    id = imk + 1
    ik = imk + num2
    write( lunit6, 902 )  id, ik, ( shp(ii), ii = id, ik )
...
713    shp( n8 ) = shp( n8 ) * 2.0 * csp
    ...
    n7 = n17 - numask + 1
    write( lunit6,905 )  n7, n17, ( histq( k ),  k = n7, n17 )
905 format(2x,16harray power from,i6,4h  to,i6,/,(2x,6e20.11))
1   continue
    return
...
402    ilk = ilk + 1
       ...
       if ( iprsup  .ge.  1 ) write( lunit6, 6701 )  ilk, numsm
6701   format( /,42h companion network model for generator no., i5, i8 )
...
404    write( lunit6, 6702 )  ilk, ismdat( i30+9 ), ismdat( i30+10 )
6702   format( 1x, 3h***, 10x, 11hmachine no., i6, 10x,31hbegin operation on segments no., 2x, 2i10 )
       ...
       do k = 1, 6
          n2 = 6 * k
          n1 = n2 - 5
          write( lunit6, 6703 )   k,  ( z(i), i=n1, n2 )
       end do
6703   format ( /,   11h new column,  i6  ,/,  ( 1x, 7e17.8 )  )
406    call mover( z(1), x1(1), ll36)
       ll7 = 6
       ll3 = 2
       call reduce (x1(1), ll7,ll3)
       if ( iprsup  .ge.  1 ) write( lunit6, 6704 )  ilk
6704   format( /, 23h reduced network model., i10 )
       if( iprsup .lt. 1 )   go  to  408
       do k = 1, 6
          n2 = 6 * k
          n1 = n2 - 5
407       write( lunit6, 6703 )   k,  ( x1(i), i=n1, n2 )
       end do
...
408    a = x1( 1 )
       ...
       b = ( etot-sum ) / 3.0
       a = ( etot+2.0*sum ) / 3.0
       if ( iprsup  .ge.  2 ) write( lunit6, 6705 )  a, b
...
1408   a = 1.0 / sum
       ...
       elp( i75+53 ) = a * athtw
       elp( i75+54 ) = b * asqrt3
       if( iprsup .gt. 0 ) write( lunit6, 6707 )   elp(i75), elp(i75+1), elp(i75+53), elp(i75+54)
       ...
       write( lunit6, 6708 )  ilk
6708   format( 5x,20h arrays for mach no.,i5,17h  in order of ac ,1x,4ha21 ,1x,4ha22 ,1x )
       write( lunit6, 6707 )  elp(i75+2), elp(i75+3), elp(i75+51), elp(i75+52)
       idt = i75 + 5
       ids = idt + 3
       write( lunit6, 6707 )    ( elp( iu ),  iu = idt, ids )
       ids = ids + 1
       idt = ids + 7
       write( lunit6, 6707 )    ( elp( iu ),  iu = ids, idt )
...
409    elp( i75+17 ) = z( 50 ) - ra * akm
       ...
       write( lunit6, 6709 )  ilk
6709   format ( /,   33h history matrix for generator no.,  i5  )
       in = i75 + 9
       do k = 1, 5
          in = in + 7
          n2 = in + 7
          n1 = in + 1
          write( lunit6, 6710 )   in, ( elp( i ),  i = n1, n2 )
       end do
6710   format( 1x, 12hnew  row    , i6, /, ( 1x, 7e17.9 ) )
412    if( k1 .gt. 0 )    go  to  402
    ...
    do ik = 1, ilk
       ibl = ibk + 1
       ibk = ibk + 101
       write( lunit6, 6711 )    ( elp( i ), i = ibl, ibk )
