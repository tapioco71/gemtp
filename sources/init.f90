...
9000 continue
  ...
  tacs_a = rsto(rptr)
  rptr = rptr - 1
  if (.not. (tacs_a .lt. rmargn)) go to 5001
  tacs_a = zero
  go to 5000
5001 continue
5000 continue
  if (.not. (tacs_b .le. rmargn)) go to 5011
  stpflg = 118
  go to 5010
5011 continue
5010 continue
  p = dmax1 (rmargn, dabs (rmargn * tacs_b))
  if (.not. (tacs_a .gt. tacs_b + p)) go to 5021
  stpflg = 119
  go to 5020
5021 continue
5020 continue
...
5031 continue
5030 continue
  if (.not. (tacs_a .gt. half * tacs_b + p)) go to 5041
  tacs_g = half * tacs_b
  go to 5040
5041 continue
  tacs_g = tacs_a
5040 continue
  r1 = rsto(base13 + 6)
  tacs_c = rsto(base13 + 8) - r1
  tacs_e = r1
  if (.not. (tacs_e .gt. tacs_b + p)) go to 5051
...
9006 continue
  ...
  rsto(rptr) = rsto(base7 + 1)
  cnt2 = isto(ndx10 + 5)
  tacs_a = rsto(base3 + 8) + rsto(base3 + 6)
  rsto(base7 + 1) = tacs_a
3041 if (.not. (cnt3 .lt. cnt2)) go to 3040
  cnt3 = cnt3 + 1
  tacs_a = rsto(base7 + 1) - rsto(base3 + 6)
  rsto(base7 + 1) = tacs_a
  xprndx = isto(ndx10 + 3)
  to = 9000
  call xpr1
...
5201 continue
5200 continue
  tacs_a = rsto(base3 + 8) + rsto(base3 + 5)
  rsto(base7 + 1) = tacs_a
3051 if (.not. (cnt3 .lt. cnt2)) go to 3050
  cnt3 = cnt3 + 1
  tacs_a = rsto(base7 + 1) - rsto(base3 + 5)
  rsto(base7+1) = tacs_a
  xprndx = isto(ndx12 + 3)
  to = 9000
  call xpr1
...
9009 continue
  ...
  tacs_a = seedy (atim(1)) + 1000.0d0 * (d1 + d2) ! wsm + thl manual modification for bpa emtp
  tacs_a = randnm (tacs_a)                        ! wsm + thl manual modification for bpa emtp
  rsto(base7 + 10) = pi
  rsto(base7 + 11) = rinf
  rsto(base7 + 12) = zero
  rsto(base7 + 13) = one
