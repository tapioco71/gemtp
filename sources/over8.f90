...
contains
  ...
  subroutine umrenu (reacl, gpar, fpar, hist,umcurp, nodvo1, nodvo2, jcltac, jclout,jtype,nodom, jtmtac, histom, omegm, omold, thetam, reamdu, reamds, flxds, flxdr, reamqu, flxqs, flxqr, jcdsat, jcqsat, flxd, flxq, nppair, rotmom, ncld, nclq, jtqout, jomout, jthout, reamqs, epsom, dcoef, kcoil, voltum, anglum, nodfum, nodmum, kumout, jumout, umoutp)
    ...
    integer(4), intent(in) :: nclq(:), jtqout(:), jomout(:), jthout(:)
    integer(4), intent(in) :: kcoil(:)
    integer(4), intent(in) :: nodfum(:)
    integer(4), intent(out) :: nodmum(:)
    integer(4), intent(in) :: kumout(:), jumout(:)
    ...
    real(8), intent(in) :: reamdu(:), reamds(:)
    real(8), intent(out) :: flxds(:), flxdr(:)
    real(8), intent(out) :: reamqu(:)
    real(8), intent(out) :: flxqs(:), flxqr(:)
    real(8), intent(out) :: flxd(:), flxq(:)
    ...
    !
    integer(4) :: i, imach, inpust, itcap
    integer(4) :: jacob, jgauss, jm
    integer(4) :: kcl, kclde, kcle, kclqe
    integer(4) :: lfim3i, lpss9
    integer(4) :: n13, n14, n15, n20, ncl, nclind, nclout, norton, nshare, ntyp59, numibr
    real(8) :: cdsat, cqsat, curmt
    real(8) :: d19, d20, dumma1(3, 3), dumma2(3, 3)
    real(8) :: fjm(4, 4), fjv(4), flxda, flxdrr, flxqa, flxqrr
    real(8) :: gmat(6, 7)
    real(8) :: seltat, sflxd, sflxq, stored, storeq
    real(8) :: tqgen
    real(8) :: zthri, zthrr, zths3(3, 3), zths3i(3, 3)
    ...
    loopss(1) = 1
305 lfim3 = 0
    if (iprsup .ge. 1) write (unit = lunit(6), fmt = 307) numum, ncltot, numout, istart, kconst, ibr, loopss(1), loopss(2), loopss(4), loopss(8), loopss(9), loopss(10)
307 format (/, ' Begin  "umrenu" .   numum  ncltot  numout  istart  kconst     ibr  lopss1  lopss2  lopss4  lopss8  lopss9  lopss10', /, 18x, 11i8, 1x, i8)
    if (loopss(4) .gt. 0) go to 13100
    ! ........ code for um steady-state initialization ............
    call umrnu2 (reacl, gpar, fpar, hist, umcurp, nodvo1, jtype, nodom, jtmtac, thetam, imach, reamdu, reamqu, nodvo2, nppair, rotmom, ncld, jcltac, kcl,nclq, epsom, dcoef, kcoil,voltum, anglum,nodfum, nodmum, umoutp)
...
8002   imach = 1
       ...
       if (imach .eq. 1) slip = voltum(jm) / 100.0d0
       zthrr = 0.0d0
       zthri = 0.0d0
       do n1 = 1, 40
          vinp(n1) = 0.0d0
       ...
       do n1 = 1, 3
          n2 = n1 + kcoil(jm) - 1
          if (nodvo1(n2) .eq. nodvo2(n2)) cycle
...
8100   kcld1 = kcl + 3
       ...
       d1 = epsiln * 1.0d+3
       if (xopt .ne. 0.0d0) d1 = epsiln * twopi * xopt
       do n10 = 1,3
          n1 = kcl - 1 + n10
          ...
          if (n2 .lt. 0) n2 = - n2
          tx(n2) = d1
          if (iprsup .ge. 1) write (unit = lunit(6), fmt = 8860) n11, tr(n2), tx(n2)
       end do
       go to 9045
8204   d1 = anglum(jm) * twopi / 360.0d0
       dumvec(1) = d1 * con(3)
       dumvec(2) = (dumvec(1) - twopi / 3.0d0) * con(3)
       dumvec(2) = dumvec(2) + d1 * con(2)
       dumvec(3) = (dumvec(1) + twopi / 3.0d0) * con(3)
       dumvec(3) = dumvec(3) + (dumvec(2) + twopi / 4.0d0) * con(2)
       d2 = epsiln
       n1 = nodvo1(kcl + 2)
       n2 = nodvo2(kcl + 2)
       ...
       write (unit = lunit(6), fmt = 8310) jm
...
8840   n11 = -nr(numibr)
       ...
       if (d11 .lt. 0.0d0) d1 = d1 + twopi / 2.0d0
       d2 = con(2) * d1 + con(3) * (d1 - twopi / 3.0d0)
       d3 = con(2) * (d1 + twopi / 4.0d0) + con(3) * (d1 + twopi / 3.0d0)
       d3 = d3 + con(1) * d1
       do n1 = 1, 3
          n2 = n1 + kcl - 1
          n11 = 3 * (jm - 1)
          ...
          n10 = - nr(n12)
          if (n10 .lt. 0) n10 = - n10
          tr(n10) = 1.0d0 / gpar(kcl + 2)
          if (iprsup .ge. 1) write (unit = lunit(6), fmt = 8860) n12, tr(n10), tx(n10)
...
8870      d15 = d1
          ...
          crest(nclout) = d10
          time1(nclout) = d15
          if (iprsup .ge. 1) write (unit = lunit(6), fmt = 8862) nclout, sfreq(nclout), crest(nclout), time1(nclout), tstop(nclout)
...
8500      if (nodvo2(n2) .eq. 1) go to 8505
          ...
          if (iprsup .ge. 1) write (unit = lunit(6), fmt = 8862) nclout, sfreq(nclout), crest(nclout), time1(nclout), tstop(nclout)
          nclout = nclout + 1
8505      n3 = int (fpar(kcl) + n1 - 1, kind (n3))
          n4 = - nr(n3)
          if (n4 .lt. 0) n4 = - n4
          tx(n4) = d17 * 1.0d0+3
          if (xopt .ne. 0.0d0) tx(n4) = d17 * twopi * xopt
          if (iprsup .ge. 1) write (unit = lunit(6), fmt = 8860) n3, tr(n4), tx(n4)
       end do
...
9002   cdsat = 0.0d0
       ...
       ! .dm excitation code *****************************************
       d2 = 1.0d0 / gpar(kcl + 2)
       thetam(jm) = twopi / (4.0d0 * nppair(jm))
...
9004   d11 = umcur(1)
       ...
       d3 = d14 / d13
       d3 = atanz (d3)
       if (d13 .lt. 0.0d0) d3 = d3 + twopi / 2.0d0
       d8 = (d13 * d13 + d14 * d14) / 2.0d0
       d8 = sqrtz (d8)
       d7 = cqsat * sflxq * flxqa * seltat
       d7 = d7 + (1.0d0 - cqsat) * sflxq * flxqrr * seltat
       if (d7 .eq. 0.0d0) go to 9005
       d6 = d8 * d8 - d7 * d7
       d6 = sqrtz(d6)
       d6 = -d7 / d6
       d6 = atanz (d6)
...
9005   d4 = cosz(d3)
       d5 = sinz(d3)
       thetam(jm) = (d3 + twopi / 4.0d0) / nppair(jm)
       umcur(3) = (d4 * d11 + d5 * d12) / sroot2
       umcur(2) = (d4 * d12 - d5 * d11) / sroot2
9006   d1 = (1.0d0 - cdsat) * reamdu(jm) - (1.0d0 - cqsat) * reamqu(jm)
       d1 = d1 + cdsat * reamds(jm) - cqsat * reamqs(jm)
       d1 = d1 * umcur(2)
       d3 = (reacl(kcl + 1) - reacl(kcl + 2)) * umcur(2)
       d1 = d1 + d3
       umcur(4) = d8 / seltat - cdsat * sflxd * flxda - d1
       umcur(4) = umcur(4) - (1.0d0 - cdsat) * sflxd * flxqrr
       d3 = (1.0d0 - cdsat) * reamdu(jm) + cdsat * reamds(jm)
       umcur(4) = umcur(4) / d3
...
9009   if (flxdr(jm) .lt. 0.0d0) flxdr(jm) = -flxdr(jm)
...
9060   if (loopss(9) .eq. 2) go to 12100
       ...
       if (fpar(kcl+3) .ge. 0.0d0) go to 12000
       n3 = int(-fpar(kcl + 3))
       d1 = umcur(4) * umcur(4)
       crest(n3) = -2.0d0 * d1 / (gpar(kcld1) * omegm(jm))
       go to 12000
       ! .im equations code *****************************************
...
10000  norton = 0
       ...
       do n10 = 1, 3
          n11 = n10 + kcl - 1
          if (nodvo1(n11) .eq. nodvo2(n11)) cycle
          n1 = nodvo1(n11)
          n1 = norder(n1)
          n2 = nodvo2(n11)
          n2 = norder(n2)
...
10030  n1 = nodvo1(kcl + 1)
       ...
       n10 = 1
       go to 10030
10040  if (con(1) .ne. 1.0d0) go to 10080
       n1 = nodvo1(kcl + 2)
       n1 = norder(n1)
       n2 = nodvo2(kcl + 2)
       n2 = norder(n2)
...
10080  do n1 = 1, 3
          ptheta(3,n1) = 1.0d0 / sroot3
       end do
       ptheta(1, 1) = -1.0d0 / (sroot2 * sroot3)
       ptheta(1, 3) = sroot2 / sroot3
       ptheta(1, 2) = ptheta(1, 1)
       ptheta(2, 1) = -1.0d0 / sroot2
       ...
       do n1 = 1, n17
          ...
          if (slip .lt. 0.0d0) vinp(n1 + 23) = -vinp(n1 + 23)
       end do
       n20 = 1
       do n1 = 1, n17
          n2 = ndum(n1) + n20 * ntot
...
10700  do n1 = 1,4
       ...
       do n1 = 1, 2
          n2 = n1 + 2
          fjm(n1,n1) = fjm(n1,n1) - 1.0d0 / reamdu(jm)
          fjm(n2,n2) = fjm(n2,n2) - 1.0d0 / reamqu(jm)
       end do
       do n1 = 1,4
          do n2 = 1, 4
...
11000  d10 = reacl(kcl+1) * seltat
       ...
       d11 = d11 + zthri + (d10 - d11) / 2.0d0
       d12 = zthrr + 1.0d0 / gpar(kcl + 2)
       d13 = 2.0d0 * (d11 * d11 + d12 * d12)
...
11554  n15 = kcle
       ...
       do n10 = 1,3
          do n11 = 1, 3
             zthevr(n10, n11) = 0.0d0
             dummat(n10, n11) = 0.0d0
          end do
       end do
       n7 = kcl + 3
       do n10 = n7, n15
       ...
       do n10 = 1, 6
          gmat(n10, n10) = gmat(n10, n10) + 1.0d0
       end do
       do n10 = 1,3
          n11 = n10 + 3
          n12 = n11 + 20
...
11720  if (jacob .lt. 6) go to 10650
       ...
       omegm(jm) = (1.0d0 - slip) * seltat / nppair(jm)
       omold(jm) = omegm(jm)
       do n10 = 1, 3
          n11 = kcl - 1 + n10
          hist(n11) = umcur(n10)
...
11800  d14 = con(3) / (sroot2 * sroot3) + con(2) / 2.0d0 + con(1)
...
11818     n11 = numibr - 3 + n1
          n12 = - nr(n11)
          if (n12 .lt. 0) n12 = - n12
          tr(n12) = 1.0d0 / gpar(kcl + 2)
          tx(n12) = 0.0d0
          if (norton .ne. 2) go to 11821
          tr(n12) = d5
          tx(n12) = d6 * 1.0d+3 / seltat
          if (xopt .ne. 0.0d0) tx(n12) = d6 * xopt * twopi / seltat
          go to 11823
11821     d11 = d1
          if (n1 .eq. 2) d11 = d2
          if (n1 .eq. 3) d11 = d3
          if ((n1 .eq. 1) .and. (lfim3i .ne. 0)) nclout = int (hist(kcl + 2), kind (nclout))
...
11822     if (nodvo2(n2) .eq. 1) go to 11823
          ...
          if (iprsup .ge. 1) write (unit = lunit(6), fmt = 8862) nclout, sfreq(nclout), crest(nclout), time1(nclout), tstop(nclout)
          nclout = nclout + 1
11823     if (loopss(8) .eq. 0 .and. lfim3i .eq. 0) cycle
          if ((iprsup .ge. 1) .and. (norton .ne. 1)) write (unit = lunit(6), fmt = 8860) n11, tr(n12), tx(n12)
       end do
       if (loopss(9) .ne. 2) go to 11825
       if (norton .eq. 2) go to 11825
       norton = 2
       go to 10580
11825  if (lfim3i .ne. 0) cycle
       nclind = int (fpar(kcl + 1), kind (nclind))
       do n10 = 4, ncl
          n11 = n10 + 20
          n12 = n10 + kcl - 1
          if (slip .lt. 0.0d0) umcur(n11) = -umcur(n11)
          if (nodvo1(n12) .eq. nodvo2(n12)) cycle
          if (loopss(9) .eq. 2) go to 11828
          d2 = umcur(n10) * umcur(n10) + umcur(n11) * umcur(n11)
          d2 = sqrtz (d2)
          d3 = umcur(n11) / umcur(n10)
          d3 = atanz (d3)
          if (umcur(n10) .lt. 0.0d0) d3 = d3 + twopi / 2.0d0
11828     if (nodvo1(n12) .eq. 1) go to 11830
          if (loopss(9) .eq. 2) go to 11829
          crest(nclind) = -d2
          time1(nclind) = d3
          if (iprsup .ge. 1) write (unit = lunit(6), fmt = 8862) nclind, sfreq(nclind), crest(nclind), time1(nclind), tstop(nclind)
11829     nclind = nclind + 1
11830     if (nodvo2(n12) .eq. 1) cycle
...
12000  d1 = 3.0d0 * con(3) + 2.0d0 * con(2) + con(8) + con(1)
       ...
       if (iprsup .ge. 1) write (unit = lunit(6), fmt = 8862) nclout, sfreq(nclout), crest(nclout), time1(nclout), tstop(nclout)
       if (iprsup .ge. 1) write (unit = lunit(6), fmt = 12010) (hist(n1), n1 = 1, ncl)
12010  format (/, ' hist(1 : ncl) =', (1x, 6e15.6))
       if (iprsup .ge. 1) write (unit = lunit(6), fmt = 12020) tqgen
...
13100 kconst = loopss(6)
    do jm = 1, numum
       ...
       if (iprsup .ge. 1) write (unit = lunit(6), fmt = 13110) jm
...
13112  if (n2 .eq. 0) go to 13115
       write (unit = lunit(6), fmt = 13113) jm,n1,n2
13113  format (/, ' ******************************************** ******************************************** Warning : um number', i4, ', ', i4, ', and', i4)
       write (unit = lunit(6), fmt = 13114)
...
13115  write (unit = lunit(6), fmt =  13116) jm,n1
13116  format (/, ' ******************************************** ******************************************** Warning : um number', i4, ', and', i4)
       write (unit = lunit(6), fmt =  13114)
       write (unit = lunit(6), fmt =  93116)
13118  if (iprsup .ge. 1) write (unit = lunit(6), fmt = 13120)
13120  format (' *********** Slack bus source:', 2x, 'kconst', 4x, 'node', 9x, 'sfreq', 9x, 'crest', 9x, 'time1', 8x, 'tstart', 9x, 'tstop')
       ...
       if (imach .ne. 1 .and. iprsup .ge. 1) write (unit = lunit(6), fmt = 13122) n10,node(n10),sfreq(n10),crest(n10),time1(n10),tstart(n10),tstop(n10)
...
13140  n12 = nodom(jm)
       ...
       tstop(n11) = crest(n11)
       crest(n11) = 0.0d0
       if (iprsup .ge. 1) write (unit = lunit(6), fmt = 13123) n11, node(n11), sfreq(n11), crest(n11), time1(n11), tstart(n11), tstop(n11)
       n13 = kssfrq(n12)
       if (n13 .lt. 0) cycle
       n14 = n13
13142  if (n13 .eq. n11) go to 13144
       if (tstart(n13) .ne. -7777.0d0) go to 13144
       tstop(n13) = crest(n13)
       crest(n13) = 0.0d0
       if (iprsup .ge. 1) write (unit = lunit(6), fmt = 13123) n13, node(n13), sfreq(n13), crest(n13), time1(n13), tstart(n13), tstop(n13)
...
13150  if (imach .eq. 1) go to 13160
       ...
       if (iprsup .ge. 1) write (unit = lunit(6), fmt = 13122) n10, node(n10), sfreq(n10), crest(n10), time1(n10), tstart(n10), tstop(n10)
...
13162  if (n13 .eq. n11) go to 13164
       if (tstart(n13) .ne. -7777.0d0) go to 13164
...
13170  if (nshare .ne. 10) cycle
       n2 = int (fpar(kcl + 2), kind (n2))
       c(n2) = 1.0d+8
       if (copt .ne. 0.0d0) c(itcap) = c(itcap) * copt * twopi
       tr(n2) = 0.0d0
       kconst = kconst + 1
       if (kconst .le. lexct) go to 6784
       write (unit = lunit(6), fmt =  6783) lexct
...
6784   node(kconst) = -nodom(jm)
       ...
       if (iprsup .ge. 1) write (unit = lunit(6), fmt = 13172) kconst, node(kconst), sfreq(kconst), crest(kconst), time1(kconst), tstart(kconst), tstop(kconst)
13172  format (' ****** Temporary tqgen source:', 1x, i6, 2x, i6, 5e14.5)
    ...
    kssout = loopss(7)
    istart = 0
    if (iprsup .ge. 1) write (unit = lunit(6), fmt = 13400)
13400 format (/, ' Permanent exit of umrenu **************')
    go to 14000
13500 istart = istart + 1
    if (iprsup .lt. 4) go to 14000
    write (unit = lunit(6), fmt =  13616)  (i, kbus(i), mbus(i), nr(i), length(i), kodebr(i), kodsem(i), i = 1, ibr)
13616 format (/, ' Branch table vectors', /, '       row      kbus    mbus        nr    length    kodebr    kodsem', /, (1x, 7i10))
    write (unit = lunit(6), fmt =  13716) (i, iform(i), node(i), crest(i), time1(i), tstart(i), sfreq(i), i = 1, kconst)
13716 format (/, ' Source table vectors', /, '       row     iform    node', 15x, 'crest', 15x, 'time1', 14x, 'tstart', 15x, 'sfreq', /, (3i10, 4e20.10))
14000 if (iprsup .ge. 1) write (unit = lunit(6), fmt =  14100) istart, istep, loopss(1), loopss(2), loopss(4), loopss(8), loopss(9), loopss(10), kconst, ibr, inonl, kswtch, istead, omegrf
14100 format (/, ' ************************* Setting at this umrenu exit :', /, '  istart   istep  lopss1  lopss2  lopss4  lopss8  lopss9 lopss10  kconst     ibr   inonl  kswtch  istead', 8x, 'omegrf', /, 13i8, e14.5, /)
  ...
  subroutine umrnu2 (reacl, gpar, fpar, hist, umcurp, nodvo1, jtype, nodom, jtmtac, thetam, imach, reamdu, reamqu, nodvo2, nppair, rotmom, ncld, jcltac, kcl, nclq, epsom, dcoef, kcoil, voltum, anglum, nodfum, nodmum, umoutp)
    use blkcom
    use labcom
    use space2
    use umcom
    ...
    integer(4), intent(out) :: jtmtac(:)
    integer(4), intent(out) :: nodom(:)
    integer(4), intent(out) :: imach
    integer(4), intent(in) :: nppair(:)
    integer(4), intent(in) :: ncld(:), nclq(:)
    integer(4), intent(out) :: kcl
    integer(4), intent(in) :: kcoil(:)
    integer(4), intent(in) :: nodfum(:)
    integer(4), intent(out) :: nodmum(:)
    real(8), intent(out) :: reacl(:)
    ...
    real(8), intent(out) :: umcurp(:)
    real(8), intent(out) :: thetam(:)
    real(8), intent(in) :: reamdu(:), reamqu(:)
    real(8), intent(in) :: rotmom(:)
    real(8), intent(out) :: epsom(:)
    real(8), intent(in) :: dcoef(:)
    real(8), intent(out) :: umoutp(:)
    ...
    integer(4) :: i
    integer(4) :: j
    integer(4) :: k, kcle, kclj
    integer(4) :: n13, n14, n15, n20, ncl, nummec
    !
    ! "first part" of umrenu code is entered twice if load-flow
    ! is requested (loopss(10) = 1 or 2). This is needed to
    ! let the load-flow do its work to adjust the sources.
...
1246   n1 = n1 + 1
       ...
       write (unit = lunit(6), fmt = 81245) n1,i
...
81246  nummec = 0
       ...
       n2 = nummec - 1
       write (unit = lunit(6), fmt = 1249) i, n2
1249   format (/, ' Error stop. um number', i4, ' is connected mechanically to ', i2, " other um's. This can not be honored because at the most only", /, " 3 um's can be connected to a same mechanical network. You may resolve this problem by inserting stub lines in order to seperate")
       write (unit = lunit(6), fmt = 81249)
81249  format (" the mechanical network from the remaining um's.")
       call stoptp
       ! .default value for epsom(i) :
1250   if (epsom(i) .ne. 0.0d0) go to 1260
       epsom(i) = 0.001d0
       if (jtype(i) .eq. 13) go to 1255
       if (inpu .eq. 1) go to 1260
1255   epsom(i) = 0.001d0 * rotmom(i) / nppair(i)
...
1262   if (jtype(i) .ne. 13) go to 1270
       ...
       n5 = int (fpar(kcl + 4), kind (n5))
       fpar(kcl + 4) = norder(n5)
       n5 = int (fpar(kcl + 5), kind (n5))
...
1270   n2 = 3 * (i - 1)
    ...
    if (n5 .ne. -9999.0d0) go to 1340
    n5 = numout + 4
    n6 = int (umoutp(numout + 3)) - 2
...
1338 n5 = int (umoutp(numout + 3))
    if (iprsup .ge. 1) write (unit = lunit(6), fmt = 1339) (umoutp(i), i = 1, n5)
...
1340 if (iprsup .lt. 1) go to 1350
    write (unit = lunit(6), fmt = 1341)
1341 format (/, ' um nodes after renumbering :')
    write (unit = lunit(6), fmt = 1345) (nodvo1(i), i = 1, ncltot)
1345 format (' nodvo1(1 : ncltot) :', 20i5)
    write (unit = lunit(6), fmt = 1346)  (nodvo2(i), i = 1, ncltot)
1346 format (' nodvo2(1 : ncltot) :', 20i5)
    n5 = 3 * numum
    write (unit = lunit(6), fmt = 1349) (umcurp(i), i = 1, n5)
1349 format (' umcurp(1 : 3numum) :', 20f5.1)
    write (unit = lunit(6), fmt = 1347) (kcoil(i), i = 1, numum)
1347 format (' kcoil(1 : numum)   :', 20i5)
    write (unit = lunit(6), fmt = 1348) (nodom(i), i = 1, numum)
...
1500 n15 = 0
    ...
    do k = 1, numum
       ...
       if (iprsup .ge. 1) write (unit = lunit(6), fmt = 1850)
1850   format (/, ' *************************************** Preparing Thevenin calculation for um type-3 excitation circuits due to ld-flow request:')
       go to 2008
1900   if (iprsup .ge. 1) write (unit = lunit(6), fmt = 1902)
1902   format (/, ' *************************************** Momentary creation of additional EMTP elements (permanent branches if power', /, ' *************************************** Coils are non-comp). For steady-state calculations of')
       if (iprsup .ge. 1) write (unit = lunit(6), fmt = 1904) k
...
1950      if ((n3 .ge. n19) .and. (n3 .le. n20)) go to 1960
          ...
          sfreq(n3) = d3 * sfreq(n3)
          if (slip .lt. 0.0d0) time1(n3) = -time1(n3)
          d4 = time1(n3) * 360.0d0 / twopi
          n6 = node(n3)
          if (n13 .eq. 1) write (unit = lunit(6), fmt = 1952) k
          if (n13 .eq. 1) write (unit = lunit(6), fmt = 1954)
          write (unit = lunit(6), fmt = 1956) bus(n6), crest(n3), sfreq(n3), d4
          if (n13 .eq. n18) write (unit = lunit(6), fmt = 1958)
...
2002   write (unit = lunit(6), fmt = 2004)
...
2008   if (loopss(10) .eq. 1) go to 2048
       ...
       if (iprsup .ge. 1) write (unit = lunit(6), fmt = 2010) k, numsub
...
2045   if (iprsup .ge. 1) write (unit = lunit(6), fmt = 2047) numsub
...
2048   ncomum = ncomcl
       ...
       if (loopss(10) .eq. 2) anglum(k) = time1(kconst + 1) * 360.0d0 / t
2050   do n1 = 1, 3
          n2 = n1 + kcoil(k) - 1
          n4 = 3 * (k - 1)
          if (nodvo1(n2) .eq. nodvo2(n2)) cycle
          ...
          if (xopt .ne. 0.0d0) tx(it) = reamqu(k) * twopi * xopt
          if (iprsup .ge. 1) then
             write (unit = lunit(6), fmt = 2052) kbus(ibr), mbus(ibr), ibr, it, tr(it), tx(i)
          end if
...
2053      if ((kbus(ibr) .ne. 1) .and. (mbus(ibr) .ne. 1)) tr(it) = epsiln
          ...
          if (imach .eq. 1) tr(it) = 1.0d0
          if (iprsup .ge. 1) write (unit = lunit(6), fmt = 2054) kbus(ibr), mbus(ibr), ibr, it, tr(it), tx(it)
...
2055      if (imach .eq. 1) d10 = 0.0d0
          do n3 = 1, 2
             if (n3 .eq. 2) go to 2060
             if(nodvo1(n2) .eq. 1) cycle
             kconst = kconst + 1
...
2064         if (n1 .ne. 3) go to 2065
             d2 = d1 + twopi / 3.0d0
             if (jtype(k) .eq. 2) d2 = d1 + twopi / 4.0d0
             if (jtype(k) .eq. 5) d2 = d1 + twopi / 4.0d0
             d1 = d2
...
2066         if (loopss(10) .eq. 0) go to 2068
             ...
             if (iprsup .ge. 1) write (unit = lunit(6), fmt = 2067) node(kconst), kconst, sfreq(kconst), crest(kconst)
2067         format (' ********* Voltage source for power coil', 4x, i4, 50x, i6, e14.5, 1x, e13.5)
             cycle
2068         if (iprsup .ge. 1) write (unit = lunit(6), fmt = 2069) node(kconst), kconst, sfreq(kconst), crest(kconst)
2069         format (' ********* Current source for power coil', 4x, i4, 50x, i6, e14.5, 1x, e13.5)
          ...
          if (n5 .lt. 0) n5 = - n5
          tx(n5) = epsiln * 1.0d+3
          if (xopt .ne. 0.0d0) tx(n5) = epsiln * twopi * xopt
          if (iprsup .ge. 1) write (unit = lunit(6), fmt = 2074) kbus(n10), mbus(n10), n10, n5, tr(n5), tx(n5)
2074      format (' *********  Leakage ind. change:', 12x, i4, 4x, i4, 2i6, 2e14.5)
          !  load-flow preparation of im type-4 power series branche :
2076      if (lfim4i .ne. 1) cycle
          n10 = int(fpar(kcl) + n1 - 1)
          n5 = -nr(n10)
          if (n5 .lt. 0) n5 = - n5
          tr(n5) = 1.0d0 / gpar(kcl + 2)
          if (iprsup .ge. 1) write (unit = lunit(6), fmt = 2078) kbus(n10), mbus(n10), n10, n5, tr(n5), tx(n5)
...
6787      node(kconst) = -nodvo1(n5)
          go to 2084
2083      if (nodvo2(n5) .eq. 1) cycle
          kconst = kconst + 1
          !          if (kconst .gt. lexct) go to 6788
          if (kconst .gt. lexct) then
             write (unit = lunit(6), fmt = 6783) lexct
             call stoptp
          end if
...
2084      iform(kconst) = 14
          ...
          if (iprsup .ge. 1) write (unit = lunit(6), fmt = 2085) node(kconst), kconst, sfreq(kconst), crest(kconst)
...
2087   if (loopss(10) .eq. 2) go to 2100
       ...
       do j = kcld1, kcle
          ...
          if (slip .lt. 0.0d0) time1(n3) = -time1(n3)
2090      if (iprsup .ge. 1) write (unit = lunit(6), fmt = 2088) node(n3), n3, sfreq(n3), crest(n3)
...
2093      go to 2090
       ...
       d13 = twopi / 36.0d0
       d14 = twopi / 3.0d0
       if (jtype(k) .eq. 3) d14 = -twopi / 4.0d0
       d15 = 2.0d0 * d14
       if (jtype(k) .eq. 3) d15 = - d14
       if (slip .gt. 0.0d0) go to 2096
       d16 = - d14 - d13
       d17 = - d14 + d13
       ...
       d17 = d15 + d13
       if ((d12 .gt. d16) .and. (d12 .lt. d17)) go to 2097
       write (unit = lunit(6), fmt = 2095) k
2095   format (/, ' Error stop. um number', i4, ' is an ind. mach and is requested to be initialized with a negative slip value. The phase sequence of', /, ' the specified external excitation sources is incorrect. It should be negative sequence rather than positive sequence.')
       call stoptp
2096   d16 = d14 - d13
       d17 = d14 + d13
       if ((d12 .gt. d16) .and. (d12 .lt. d17)) go to 2097
       ...
       d17 = - d15 + d13
       if ((d12 .gt. d16) .and. (d12 .lt. d17)) go to 2097
       write (unit = lunit(6), fmt = 82096) k
82096  format (/, ' Error stop. um number', i4, ' is an ind. mach and is requested to be initialized with a positive slip value. The phase sequence of', /, ' the specified external excitation sources is incorrect. It should be positive sequence rather than negative sequence.')
       call stoptp
2097   if (nminum .ge. 10000) go to 2100
       if (node(nminum) .gt. 0) go to 2100
       if (lfim4i .eq. 0) go to 2100
       write (unit = lunit(6), fmt = 2098)
2098   format (/, ' Warning :   you have requested a load-flow for a network containing an induction machine. You chose the exciter sources as', /, ' current sources and will therefore not be adjusted by the load-flow module.', /)
2100   if (jtmtac(k) .le. 0) go to 2150
       write (unit = lunit(6), fmt = 2110)
2110   format ('0program stop because of conflicting request, i.e.')
       write (unit = lunit(6), fmt = 2120)
2120   format (' no initialization if no mechanical network option')
       call stoptp
       !   creating temporary speed-source for each um mass :
2150   if (loopss(10) .eq. 3) cycle
       kconst = kconst + 1
       !      if (kconst .ge. lexct)  go to 6788
       if (kconst .ge. lexct) then
          write (unit = lunit(6), fmt = 6783) lexct
          call stoptp
       end if
       iform(kconst) = 14
...
2154   tstart(kconst) = -1.0d0
       tstop(kconst) = 0.0d0
       n10 = nodmum(k)
       sfreq(kconst) = sfreq(n10)
       crest(kconst) = 0.0d0
2160   format (' ********* um mass speed-source', 13x, i4, 50x, i6, e14.5, 1x, e13.5)
       if (iprsup .ge. 1) write (unit = lunit(6), fmt = 2160) node(kconst), kconst, sfreq(kconst), crest(kconst)
    end do
    ...
    do k = 1, numum
       ...
       slip = voltum(k) / 100.0d0
       if (iprsup .ge. 1) write (unit = lunit(6), fmt = 2201)
2201   format (/, ' *************************************** Momentary changes or creation of momentary elements for load-flow involving ind. mach.')
       ...
       do n1 = 31, 33
          ...
          length(ibr) = 1
          nr(ibr) = -it
          tr(it) = 1.0d0 / (gpar(kcl + 4) * slip)
          c(it) = 0.0d0
          tx(it) = reacl(kcl + 4) * 1.0d+3
          if (xopt .ne. 0.0d0) tx(it) = reacl(kcl+4) * twopi * xopt
          if (iprsup .ge. 1) write (unit = lunit(6), fmt = 2208) kbus(ibr), mbus(ibr), ibr, it, tr(it), tx(it)
...
2222   write (unit = lunit(6), fmt =  2224)
2224   format (/, ' Error stop. You have requested a load-flow of a network containing an induction machine with ungrounded power coils and/or excitation coils. This request is not honored in the current emtp version. These coils are both to be y-connected with grounded neutrals.')
       call stoptp
...
2226   if (loopss(8) .ne. 0) go to 2230
       do n1 = 1, 3
          ...
          tr(it) = epsiln
          tx(it) = 0.0d0
          if (iprsup .ge. 1) write (unit = lunit(6), fmt = 2054) kbus(ibr), mbus(ibr), ibr, it, tr(it), tx(i)
          cycle
2227      tx(it) = reamqu(k) * 1.0d+3
          if (xopt .ne. 0.0d0) tx(it) = reamqu(k) * twopi * xopt
          if (iprsup .ge. 1) write (unit = lunit(6), fmt = 2052) kbus(ibr), mbus(ibr), ibr, it, tr(it), tx(i)
       end do
...
2240      n1 = n1 + 1
       ...
       write (unit = lunit(6), fmt = 82245) k
       call stoptp
82242  do n10 = 1, n13
          n11 = kbus(n10)
          if (n11 .lt. 0) n11 = - n11
...
82244        if (n18 .lt. 40) go to 82246
             write (unit = lunit(6), fmt = 82245) k
82245        format (/, ' Error stop. um number', i4, ' has more than 40 excitation branches or sources. Overflow is avoided by increasing the dimension of ',/, ' the arrays "vinp" and "ndum" in umdeck to be one higher than these used elements.')
             call stoptp
82246        if (iprsup .ge. 1) write (unit = lunit(6), fmt =  2243) n11, n12, n10, n9, tr(n9), tx(n9)
...
2300 if (loopss(10) .eq. 3) go to 2400
    ...
    do n5 = 1, numum
       ...
       if (n1 .eq. 0) cycle
       do n2 = 1, ibr
          if (kbus(n2) .ne. nodvo2(n3 + 2)) cycle
          if (mbus(n2) .eq. nodvo1(n3 + 2)) cycle
          n1 = mbus(n2)
       end do
...
9   i=ij+k
  ...
  subroutine equiv (req, xeq, geq, beq, z, r, tau, w, n)
    ...
    real(8), intent(out) :: req(:)
    real(8), intent(out) :: xeq(:)
    real(8), intent(out) :: geq(:)
    ...
    integer(4) :: j
    integer(4) :: l
    real(8) :: a, anumr, anumx
    real(8) :: c, c2, cosb, cosha
    real(8) :: d, d1, d2, d3, d4, denr, denx
    real(8) :: e, e2, e2div8, eneg, epos
    real(8) :: piov2
    real(8) :: rs, rzero
    real(8) :: sinb, sinha, sinhgr, sinhgi
    real(8) :: tden
    real(8) :: xzero
    real(8) :: yneg, ypos
    real(8) :: zs
    equivalence (anumr, rzero)
    equivalence (anumx, xzero)
    equivalence (ypos, sinb)
    equivalence (yneg, cosb)
    equivalence (epos, sinhgr)
    equivalence (eneg, sinhgi)
    equivalence (c, cosha)
    equivalence (d, sinha)
    !
    do l = 1, n
...
1786   format (/, " Within l-loop of  'equiv' .       l       n", 15x, 'w', 10x, 'tau(j)', 14x, 'zs', 14x, 'rs', /, 28x, 2i8, 4e16.7)
       ...
       anumr = cosha * cosb - 1.0d0
       anumx = sinha * sinb
       go to 2
1      e = rs / zs
       if (zs .lt. 0.0d0) go to 3
       a = a / 2.0d0
       c = sinz (a)
       d = cosz (a)
       e2 = e ** 2
       c2 = c ** 2
       denr = (48. + e2) / 32.
       denr = (1.0d0  - c2 * denr) * rs
       e2div8 = e2 / 8.0d0
       denx = (3.0d0 * e2div8 + 2.0d0) * c * d * zs
       anumr = (-2.0d0 - e2div8) * c2
       anumx = e * c * d
2      e = denr ** 2 + denx ** 2
       req(l) = denr
       xeq(l) = denx
...
subroutine over8
  ...
  integer(4) :: iujclo, iujtho, iujtmt, iujtqo, iujtyp, iujumo, iukcoi, iukumo
  integer(4) :: iuncld, iunclq, iunod1, iunod2, iunodf, iunodm, iunodo, iunppa
  integer(4) :: iuomgm, iuomld, iureac, iureds, iuredu, iureqs, iurequ, iurotm
  integer(4) :: iutham, iuumrp, iuvolt, iuumou
  integer(4) :: j, j0, j1, j2, j30, j31, jgl, jip, jk, jkl, jqy
  integer(4) :: k, km1, koff1, koff2, koff3, koff4, koff5, koff6, koff7
  integer(4) :: koff8, koff9, koff10, koff13, koff14, koff15, koff16, koff17
  integer(4) :: koff18, koff19, koff20, kqv, kqvv, kqy, ksma
  integer(4) :: l, ll0, lq
  integer(4) :: m, m7, m8
  integer(4) :: n1, n2, n3, n4, n5, n6, n7, n8, n9sq, n11, n12, n13, n15, n16
  integer(4) :: n20, n3tst, ndx1, nhst, nn1, nn2, nn3, npa, nphs2, npz, nq1
  integer(4) :: nq2, nq3, nsave, nteq
  real(8) :: aki, akr, alpha, api, apr
  real(8) :: beta, bi, br
  real(8) :: cc1, cc2, ccc3, ccc4, cosha, csihim, csihre, ctahim, ctahre
  real(8) :: d, d1, d2, d3, d4, d5, d6, d7, d8, d9, d10, d11, d12, d13, dd1, dd2
  real(8) :: dd3, ddd3, ddd4, dki, dkr, dqq3, dqq4, dtemp, dum, dyik, dyrk
  real(8) :: gjl, gus1, gus2, gus3, gus4
  real(8) :: h1, h2, h3, h4, hh3
  real(8) :: omctem, omltem
  real(8) :: qiik, qijk, qrik, qrjk
...
4141 do i = 1, ibr
  ...
  if (iprsup .gt. 1) write (unit = lunit(6), fmt = 73825)  kpartb, ntot
73825 format (//, ' Final renumbering maps.  kpartb= ', i5, 5x, 'ntot=', i3, /, 9x, 'i', 4x, 'bus(i)', 3x, 'norder(i)', 5x, 'ich1(i)', 5x, 'kode(i)')
  if (iprsup .gt. 1) write (unit = lunit(6), fmt = 73826) (i, bus(i), norder(i), ich1(i), kode(i), i = 1, ntot)
...
2240 begmax(1) = 0.0d0
  ...
  tmax = fminfs
  go to 9600
3234 if (delffs .lt. 0.0d0) go to 3218
  fminfs = fminfs  +  delffs
  go to 3224
3218 fminfs = fminfs * (-delffs)
3224 do j = 1, kconst
     if (iform(j) .eq. 18) cycle
     sfreq(j) = fminfs
  end do
  if (iprsup .ge. 2) write (unit = lunit(6), fmt = 3227) knt, ltlabl, fminfs, fmaxfs
3227 format (/, ' Begin next freq.     knt  ltlabl', 9x, 'fminfs', 9x, 'fmaxfs', /, 17x, 2i8, 2e15.6)
1413 if (iprsup .ge. 1) write (unit = lunit(6), fmt = 1417)  kconst, ibr, inonl, kswtch, istead, xopt, copt, twopi
1417 format (1x, '  kconst     ibr   inonl  kswtch  istead', 11x, 'xopt',  11x, 'copt', 10x, 'twopi', /, 1x, 5i8, 3e15.6)
  if (iprsup .ge. 1) write (unit = lunit(6), fmt = 1419) (xoptbr(j), coptbr(j), j = 1, ibr)
...
401 n1 = length(k)
  ...
  xopt = xoptbr(k)
  copt = coptbr(k)
  omltem = 1000.0d0 / twopi
  omctem = 1000.0d0 * omltem
  if (xopt .gt. 0.0d0) omltem = xopt * tenm3 * twopi *omltem
  if (copt .gt. 0.0d0) omctem = copt * twopi * omctem
  n16 = iabs (kssfrq(n15))
  d2 = omltem / sfreq(n16)
  h3 = omctem / sfreq(n16)
  ...
  do i = 1, it2
     ...
     cc1 = cosha * cosz (beta) - 1.0d0
     cc2 = sinha * sinz (beta)
     d7 = volti(i) ** 2 + voltk(i) ** 2
     volt(i) = (volti(i) * cc1 + voltk(i) * cc2) / d7
     vim(i + lsiz26) = (volti(i) * cc2 - voltk(i) * cc1) / d7
     if (iprsup .ge. 1 ) write (unit = lunit(6), fmt = *) ' for mode ', i,' pi-equiv. zseres, 1/2 yshunt=', volti(i), voltk(i), volt(i), vim(i + lsiz26)
  end do
  n9sq = it2 * it2
  read (unit = lunit(9)) (qfd(i), i = 1, n9sq), (sfd(i), i = 1, n9sq)
  if (iprsup .ge. 1) write (unit = lunit(6), fmt = *) ' qfd and sfd read from lunit(9).', (qfd(i), i = 1, n9sq), (sfd(i), i = 1, n9sq)
  n2 = 1
  go to 460
40002 length(k) = it2
  ...
  n3 = n1 + n5
  n4 = n3 + n5
  write (unit = lunit(6), fmt = 8001) cik(k)
8001 format (//, 5x, 'sfd(', f4.0, ') ...', /)
  do i = 1, it2
     n2 = i + n1
     write (unit = lunit(6), fmt = 8002) i, (sfd(j), j = n2, n3, it2)
8002 format (/, 2x, i5, 8(1x, e15.7), /, (7x, 8(1x, e15.7)))
     n2 = i + n3
     write (unit = lunit(6), fmt = 8003) (sfd(j), j=n2, n4, it2)
8003 format (7x, 8(1x, e15.7))
  end do
  write (unit = lunit(6), fmt = 8005) cik(k)
8005 format (///, 5x, 'qfd(', f4.0, ') ...', /)
  do i = 1, it2
     n2 = i + n1
     write (unit = lunit(6), fmt = 8002) i, (qfd(j), j=n2, n3, it2)
     n2 = i + n3
     write (unit = lunit(6), fmt = 8003) (qfd(j), j=n2, n4, it2)
  end do
8009 do i = 1, it2
     ii = i + k - 1
     length(ii) = it2
     n1 = indhst(ii)
     if (ci(ii) .gt. 0.0d0) n1 = n1 + 1
     if (cnvhst(n1 + 4) .gt. 0.0d0) go to 8700
     n2 = iabs (kodsem(ii)) + 1
     n3 = int(ci(ii))
     d1 = 0.0d0
     d2 = 0.0d0
...
8470 n3 = iabs (kodsem(ii))
     ...
     go to 8600
8500 d5 = 0.0d0
     if (iprsup .ge. 9) write (unit = lunit(6), fmt = 8311) n2, n3, sconst(n2), sconst(n2 + 1), d5, d6, d1, d2
     if (n3 .ge. 0) go to 8320
     n3 = n3 + 1
     d6 = -omega * sconst(n2)
     d5 = sconst(n2 + 1) - d5
...
8010 format (//, ' Warning...  Steady state modal parameters for recursive-convolution component connecting nodes ', "'", a6, "'", ' and ', a6, "'", /, 13x, 'for mode ', i2, ' are determined at angular frequency of', e12.5, ' radians/sec.  The steady-state solution frequency', /, 13x, 'being used is ', e12.5, ' radians/sec.')
...
8085 format (/, 5x, a6, ' to ', a6, 5x, 'br. index = ', i3, 5x, 'n1 = ', i3, /, 1x, 'cnvhst(n1) ... cnvhst(n1+4) =  ', 5(1x, e15.7), /, 1x, 'd3, d4, d5, d6, d7, d8 =  ', 6(1x, e15.7), /, 1x, 'd9, d10, volt, volti, voltk, vim =   ', 6(1x, e15.7))
  ...
  do i = 1, it2
     ...
     if (iprsup .lt. 6) cycle
     n3 = itadd - 1
     n4 = nr(ii)
     write (unit = lunit(6), fmt = 8125) n4, nr(ii), ii, (tr(j), tx(j), r(j), c(j), j = n4, n3)
8125 format (//, 5x, 'n4 = ', i4, 5x, 'nr(ii) = ', i4, 5x, 'ii = ', i4, 5x, '(tr(j), tx(j), r(j), c(j), j=n3, n4)  ....', /, (8(1x, e15.8)))
  end do
  it2 = 1
8140 if (cki(k) .lt. 0.0d0) go to 407
  k = k + 1
  go to 8140
1536 continue
  n1 = it2
  ...
  !      ktrlsw(5) = 1
  if (iprsup .ge. 1) write (unit = lunit(6), fmt = 50002) omega
50002 format (' The following lines are frequency dependent parameters at ', f10.3, ' radians/sec.', /, ' bus1', 7x, 'bus2', 14x, 'surge impedance', 20x, ' propagation function', 12x, 'travel time')
  do i = 1, it2
     j = i + k -1
     npz = int (cki(j))
     npa = int (ckkjm(j))
     ...
     gjl = 56789.0d0
     jgl = 0
     if (iprsup .ge. 1) write (unit = *, fmt = 1990)
...
8015    d1 = sconst(n4)
        ...
        if (iprsup .ge. 1) write (unit = *, fmt = 1991) npz, ii, n4, sconst(n4), sconst(n5), cnvhst(n8 + 1), cnvhst(n8 + 2)
1991    format (1x, i2, 2x, i3, 2x, i6, 2x, 4e26.9)
     end do
     cnvhst(n8 + 3) = 0.0d0
     cnvhst(n8 + 4) = 0.0d0
     jkl = 0
     if (iprsup .ge. 1) write (unit = *, fmt = 1988)
...
1989    jkl = jkl + 1
        ...
        akr = sconst(n6) / 1.0e+15
        aki = sconst(n6 + 1) / 1.0e+15
        apr = sconst(n7) / 1.0e+15
        api = sconst(n7 + 1) / 1.0e+15
        dkr = apr * akr - api * aki
        dki = apr * aki + api * akr
        akr = dkr
        aki = dki
...
5486    jkl = 0
1993    if (iprsup .ge. 1) write (unit = *, fmt = 1992) npa, ii, n6, sconst(n6), sconst(n6 + 1), sconst(n7), sconst(n7 + 1), cnvhst(n8 + 3), cnvhst(n8 + 4)
1992    format (1x, i2, 2x, i3, 2x, i6, 2x, 6e19.9)
     ...
     d4 = cnvhst(n1 + 0) * cnvhst(n1 + 3) + cnvhst(n1 + 1) * cnvhst(n1 + 2)
     d5 = sqrtz (d3 * d3 + d4 * d4)
     if (d3 .lt. 0.0d0) go to 2017
     d5 = sqrtz ((d3 + d5) * onehaf)
     d6 = onehaf * d4 / d5
     go to 2018
2017 d6 = sqrtz ((-d3 + d5) * onehaf)
     if (d4 .ge. 0.0d0) go to 2019
     dtemp = -d6
     d5 = onehaf * d4 / dtemp
     go to 8050
...
2021 d11 = d3 * d3 + d4 * d4
     ...
     n2 = - kbus(ii)
     n3 = iabs(mbus(ii))
     write (unit = lunit(6), fmt = 2023) bus(n2), bus(n3), ii, n1, cnvhst(n1 + 0), cnvhst(n1 + 1), cnvhst(n1 + 2), cnvhst(n1 + 3), cnvhst(n1 + 4), d3, d4, d5, d6, d7, d8, d9, d10, volt(i), volti(i), voltk(i), vim(ndx1)
2023 format (/, 5x, a6, ' to ', a6, 5x, 'br. index = ', i3, 5x, 'n1 = ', i3, /, 1x, 'cnvhst(n1) ... cnvhst(n1+4) =  ', 5(1x, e15.7), /, 1x, 'd3, d4, d5, d6, d7, d8 =  ', 6(1x, e15.7), /, 1x, 'd9, d10, volt, volti, voltk, vim =   ', 6(1x, e15.7))
2022 d4 = cnvhst(n8)
     d9 = omega * d4
     d1 = dcos(d9)
     dd2 = -dsin(d9)
     ...
     j1 = n8 + 1
     j2 = n8 + 4
     if (iprsup .ge. 2) write (unit = lunit(6), fmt = 70001) bus(n11), bus(n12), (cnvhst(l), l = j1, j2),  d4
70001 format (' ', a6, 5x, a6, 5x, 2f15.10, 5x, 2f15.10, 5x, f15.10)
     ...
     if (iprsup .ge. 1) write (unit = lunit(6), fmt = 90001) i, wk1(koff7 + i), wk1(koff9 + i), wk1(koff8 + i), wk1(koff10 + i)
...
5136 n1 = it2
  ...
  ktrlsw(5) = 1
  if (iprsup  .ge. 1) write (unit = lunit(6), fmt = 20005)  omega
...
10007 format (' ', a6, 5x, a6, 5x, 2f15.10, 5x, 2f15.10, 5x, f15.10)
     ...
     csihim = -cnvhst(n8 + 4) * (1 / d9 + 1.0d0) * onehaf
     d3 = 1 + d9 + 2.0d0 * cnvhst(n8 + 3)
     ctahre = (1.0d0 - d9) / d3
     ctahim = -2.0d0 * cnvhst(n8 + 4) / d3
     ...
     if (iprsup .ge. 1) write (unit = lunit(6), fmt = 10009) i, volti(i), voltk(i), volt(i), vim(ndx1)
...
460 n3 = itadd
  if (iprsup .ge. 1) write (unit = lunit(6), fmt = 1460) it2, (volti(i), voltk(i), volt(i), vim(i + lsiz26), i = 1, it2)
1460 format (' b4 do 464, volti,voltk, volt, and vim for i=1 to', i3, 'are', /, (1x, 8e15.6))
  ...
  if (iprsup .ge. 1) write (unit = lunit(6), fmt = 1465) k, litype(k), it2, itadd, n3
1465 format (' At 1465, k, litype(k), it2, itadd, n3, are ', 10x, 5i10)
  ...
  do i = 1, it2
     do j = 1, i
        ...
        tr(itadd) = gus1
        tx(itadd) = gus2
        if (iprsup .ge. 1) write (unit = lunit(6), fmt = 1462) itadd,  gus3, gus4, gus1, gus2
1462    format (' gus3, gus4, gus1 and gus2 at 1462 for itadd =', i3, 'are', 4e15.6)
  ...
  n4 = itadd - 1
  ll0 = 0
  if (iprsup .ge. 1) write (unit = lunit(6), fmt = 3465) n3, (tr(i), tx(i), i = 1, n3)
3465 format (' b4 call cxred8, tr and tx for i =1 to', i4, ' are', /, (1x, 8e15.6))
  call cxred8 (tr(n3 :), tx(n3 :), it2, ll0)
  if (iprsup .ge. 1) write (unit = lunit(6), fmt = 2465) n3, (tr(i), tx(i), i = 1, n3)
...
640 inoff2 = 2 * lbrnch
  ...
  nphs2 = it2 * it2
  do kqv = 1, nphs2
     nteq = int (wk1(koff20 + kqvv))
     xk(kqv)=sconst(nq1)
     xm(kqv) = 0.0d0
     if (iprsup .ge. 1) write (unit = *, fmt = 5692)
5692 format ('kqv nteq    nq1    nq2    nq3   sconst(nq1)   sconst(nq2)   sconst(nq3)       qr(kqv)')
     do lq = 1, nteq
        nq2 = nq1 + lq
        nq3 = nq2 + nteq
        if (absz (sconst(nq2)) .ge. 1.0e+13) go to 3000
...
3000    jkl = jkl + 1
        ...
        akr = sconst(nq2) / 1.0e+15
        aki = sconst(nq2 + 1) / 1.0e+15
        apr = sconst(nq3)   / 1.0e+15
        api = sconst(nq3 + 1) / 1.0e+15
        dqq3 = apr ** 2 + (omega + api) ** 2
        ddd3 = (akr * apr + aki * (omega + api)) / dqq3
        ddd4 = (aki * apr - akr * (omega + api)) / dqq3
        ...
        if (jkl .eq. 1) go to 1993
3002    jkl = 0
3001    if (iprsup .ge. 1) write (unit = *, fmt = 1333) kqv, nteq, nq1, nq2, nq3, sconst(nq1), sconst(nq2), sconst(nq3), xk(kqv)
1333    format (1x, i2, 3x, i2, 2x, i5, 2x, i5, 2x, i5, 4e14.5)
  ...
  if (iprsup .ge. 1) write (unit = lunit(6), fmt = *) 'following are qr(k) by rows____________'
  if (iprsup .ge. 1) write (unit = lunit(6), fmt = 4515) (xk(j), j = 1, nphs2)
  if (iprsup .ge. 1) write (unit = lunit(6), fmt = *) 'following are qi(k) by rows____________'
  if (iprsup .ge. 1) write (unit = lunit(6), fmt = 4516) (xm(j), j = 1, nphs2)
4515 format ('+ qr-i:', 3e14.5)
4516 format ('+ qi-i:', 3e14.5)
  !
  do ik = 1, nphs2
     if (iprsup .ge. 3) write (unit = lunit(6), fmt = 4160) nphs2, xk(ik), xm(ik)
4160 format ('nphs2 & qr(ik) & qi(ik)=', i3, 2x, 2e15.6)
  ...
  it0 = it2 * (it2 + 1) / 2
  if (iprsup .ge. 3) write (unit = lunit(6), fmt = 5284) (i, tr(i), tx(i), r(i), c(i), i = itadd, itadd + it0)
...
393 if (k .le. ibr) go to 401
  ...
  if (ktrlsw(5) .eq. 1) call vecsav (cnvhst, lhist, n13)
  i = it + 1
  j0 = itadd - 1
  if (lastov .eq. nchain - 1 .or. iprsup .ge. 1) write (unit = lunit(6), fmt = 47884) i, j0
47884 format (' pi-equiv branches of distrib lines in tr, tx, etc. between limits    ', 2i6)
  if (iprsup .ge. 3) write (unit = lunit(6), fmt = 2584) (i, tr(i), tx(i), r(i), c(i), i = 1, j0)
2584 format (7x, 'row', 13x, 'tr', 13x, 'tx', 14x, 'r', 14x, 'c', /, (i10, 4e15.5))
  if (inonl .eq. 0) go to 414
  if (noutpr .eq. 0) write (unit = lunit(6), fmt = 406)
406 format (' Nonlinear and time-varying resistances ignored in steady state solution. Nonlinear inductances included with linear part')
  d2 = 1000.0d0
  if (xopt .gt. 0.0d0) d2 = xopt * twopi
...
4269 j0 = nonle(i)
     if (j0 .gt. 0) cycle
     if (vzero(i) .eq. 0.0d0) cycle
     if (anonl(i) .gt. 0.0d0) go to 4272
     kill = 14
     lstat(19) = 406
     flstat(16) = anonl(i)
     n1 = nonlk(i)
     ...
     go to 9200
4272 call ibrinc
     if (iprsup .ge. 3) write (unit = lunit(6), fmt = 65457) (k, kbus(k), mbus(k), nr(k), length(k), kodebr(k), k = 1, ibr)
...
subroutine frqchk
  ...
  integer(4) :: j
  integer(4) :: k
  integer(4) :: n1, n2, n4, n5, n6, n7, n8, n9, n10, n11, n12, n13, n14, n15
  integer(4) :: n16, n17, n18, n19, n20, ndx1
  real(8) :: d12
  !
  if (iprsup  .ge.  1) write (unit = lunit(6), fmt = 1428) ntot, kconst, ibr, kswtch, it
1428 format (' Top of "frqchk".    ntot  kconst     ibr  kswtch      it', /, 17x, 8i8)
  ...
  do i = 1, kconst
     n4 = iabs ( node(i) )
     kpsour(i) = i
     if (kssfrq(n4) .gt. 0) cycle
     if (iabs (iform(i)) .ne. 14) cycle
     if (tstart(i) .eq. 5432.0d0) go to 1434
...
1436 if (n13 .eq. 0) n13 = i
     if (d12 .eq. 0.0d0) d12 = sfreq(i)
     if (sfreq(i) .eq. d12) cycle
     n16 = 1
  end do
...
1467 k = k + 1
  ...
  if (iprsup .ge. 6) write (unit = lunit(6), fmt = 1468) k, length(k), n6, n8, n12, d12
1468 format (' k, length(k), n6, n8, n12, d12 =', 5i6, f10.2)
1469 if (k .le. ibr) go to 1446
  if (kswtch .le. 0) go to 1475
  do j = 1, kswtch
     if (adelay(j) .eq. -44444.0d0) go to 1470
     if (tclose(j) .ge. 0.0d0) cycle
     if (adelay(j) .eq.  44444.0d0) cycle
...
1475 if (iprsup .ge. 5) write (unit = lunit(6), fmt = 1476)  n8, (kssfrq(j), j = 1, ntot)
1476 format (' Additional frequency spreading.   n8 =', i5, '      (kssfrq(j), j = 1, ntot)  follows ....', /, (1x, 20i4))
  if (iprsup .ge. 5) write (unit = lunit(6), fmt = 1478) (kpsour(j), j = 1, kconst)
1478 format (' kpsour:', 20i5)
  if (n8 .gt. 0) go to 1441
1480 do i = 1, ntot
     if (kssfrq(i) .eq. 0) kssfrq(i) = -n13
  end do
  if (iprsup .ge. 2) write (unit = lunit(6), fmt = 1482) (kpsour(j), j = 1, kconst)
1482 format (' kpsour:',  20i5)
  if (iprsup .ge. 2) write (unit = lunit(6), fmt = 1484) (kssfrq(j), j = 1, ntot)
1484 format (' Final  (kssfrq(j), j = 1, ntot)  follows ...', /, (1x, 20i4))
  do k = 1, kconst
     if (kpsour(k) .lt. 0) cycle
     n16 = k
     n2 = kpsour(k)
     if (k .eq. kconst) go to 1494
     n7 = k + 1
     do j = n7, kconst
        if (kpsour(j) .ne. n2) cycle
        kpsour(n16) = -j
        n16 = j
     end do
1494 kpsour(n16) = -k
...
9202 if (kill .ne. 223) go to 9204
  ...
  lstat(15) = n18
  lstat(16) = n19
9204 if (iprsup .ge. 1) write (unit = lunit(6), fmt = 9207)
