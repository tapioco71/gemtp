...
1133 format (i2, a6)
  ...
  read (unit = abuff, fmt = 1983) nekfix(nkr), fixbu1(nkr), fixbu2(nkr), fixbu3(nkr), fixbu5(nkr), fixbu8(nkr), fixbu7(nkr), fixbu9(nkr), fixbu4(nkr), fixbu6(nkr)
1983 format (i2, 3a6, 2e16.0, 2e8.0, 2e6.0)
  if (fixbu9(nkr) .eq. 0.0d0) fixbu9(nkr) = fltinf
  if (fixbu4(nkr) .eq. 0.0d0) fixbu4(nkr) = -fltinf
  if (fixbu6(nkr) .eq. 0.0d0) fixbu6(nkr) = fltinf
  write (unit = kunit6, fmt = 1984) fixbu5(nkr), fixbu8(nkr), fixbu7(nkr), fixbu9(nkr)
...
1554 format (/, " (kownt(i), i=1, ntot)   in  'over9' ,  after branch table has been put into steady-state renumbering arrays.", /, (1x, 20i6))
  ...
  i = 0
1631 i = i + 1
  if (adelay(i) .eq. -44444.0d0) go to 1650
  if (tclose(i) .ge. 0.0d0) go to 1640
  if (adelay(i) .eq. 44444.0d0) go to 1640
  go to 1650
