...
c
     ...
     1     integer*4 (i-n)
      ...
      data  text19   /  6hsos     /
      if ( iprsup .ge. 1 )
     1 write (lunit6, 1001)  kill
 1001 format (  25h top of "subr55".  kill =,  i6  )
      if ( isw  .eq.  -3344 )  go to 6740
      if ( kill .le. 200 )  go to 4092
      n1 = kill - 200
      go to (
     1 6201, 6202, 6203, 6204, 6205, 6206, 6207, 6208, 6209, 6210,
     2 6211, 6212, 6213, 6214, 6215, 6216, 6217, 6218, 6219, 6220,
     3 6221, 6222, 6223, 6224, 6225, 6226, 6227, 6228, 6229, 6230,
     4 6231, 6232, 6233, 6234, 6235, 6236, 6237, 6238, 6239, 6240,
     5 6241 ),  n1
      go to 6540
 6201 write (lunit6, 7201)  lstat(16)
 7201 format (5x, 106hthe last-read data card is a request for the furth
     1er (continued) solution of a previously-solved emtp data     ,/,
     2 5x, 107hcase.   the   'restart'   request specifies a permanent f
     3ile in which is stored   /blank/   and   /label/ .           ,/,
     4 5x, 111hbut emtp dimensioning of the present program version is n
     5ot identical to that for the version which created the       ,/,
     6 5x,  97hpermanent file.   specifically, the total length of   /la
     7bel/   for the file-creating program was,  i8,  10h   integer  )
      write (lunit6, 7301)  ltlabl
 7301 format (5x,  48hwords, while the corresponding present figure is,
     1 i8,   45h .    any such discrepancy is illegal.   as a      ,/,
     2 5x, 118hgeneral rule, the user is counseled to use the same progr
     3am version for both operations, thereby guaranteeing success.   )
      go to 6550
 6202 continue
      go to 6550
 6203 write (lunit6, 7203)
 7203 format (5x, 110hthe data case now being solved involves one or mor
     1e type-96 hysteretic inductors.   fine.   however, no       ,/,
     2 5x, 110hsteady-state phasor solution for initial conditions was r
     3equested by the user.   this combination is permitted       ,/,
     4 5x, 106honly if the simulation begins as the continuation of a pr
     5eviously-halted run (with field  'tstart'  of the           ,/,
     6 5x, 116hfloating-point miscellaneous data card punched positive).
     7   the data case under consideration does not satisfy these  ,/,
     8 5x,  43hrestrictions, so solution shall be stopped.        )
      go to 6550
 6204 write (lunit6, 7204)  flstat(14), flstat(15)
 7204 format (5x, 104hthe last-inputted emtp component was a type-96 hys
     1teretic inductor.   columns  27-32  and  33-38  of the      ,/,
     2 5x,  97hbranch card are to be punched with  'i-steady'  and   'ps
     3i-steady' ,   respectively.   but values,  e14.4,  6h   and  ,/,
     4 5x,  e14.4, 106h   were read for these two variables, which repre
     5sents a point in the current-flux plane that lies outside   ,/,
     6 5x, 111hthe user-defined major hysteresis loop.   the emtp does n
     7ot allow such sloppiness (even though the ratio may be      ,/,
     8 5x,  58hcorrect).   define a point within the loop, and try again
     9.   )
      go to 6550
 6205 write (lunit6, 7205)  flstat(16), flstat(17)
 7205 format (5x, 108hthe last-inputted emtp component was a type-96 hys
     1teretic inductor.   columns  39-44  of the branch card are  ,/,
     2 5x,  68hto be punched with a residual (remnant) flux value.   but
     3 a value of,  e14.4,   27h   was read for this, which       ,/,
     4 5x, 115hexceeds (in absolute value) the flux of the user-inputted
     5 major hysteresis loop at zero current.   this latter flux  ,/,
     6 5x,  8hvalue is,  e14.4,   91h .   the result is a current-flux p
     7oint which lies outside the major hysteresis loop, which    ,/,
     8 5x,  87his impossible.   punch a legal residual flux value in col
     9umns  39-44 ,   and try again.    )
      go to 6550
 6206 write (lunit6, 7206)
 7206 format (5x, 114hthe user is trying to combine  'statistics'  or  '
     1systematic'  results using the   'tabulate energization results'
     2 ,/, 5x, 107hfeature.   but not all of the partial results are com
     3patible.   files previously attached and read have the      ,/,
     4 5x,  39hfollowing characteristic parameters ...     )
      write (lunit6, 7306)  ( lstat(i), i=11, 13), bus2
 7306 format (10x,  i5,
     1  45h = ntot    (number of electric network nodes)          ,/,
     2 10x, i5,  39h = nstat   (number of output variables)       ,/,
     3 10x, i5,  31h = kswtch  (number of switches)               ,/,
     4  9x, a6,  42h = bus(ntot)  (name of last network node).    )
      write (lunit6, 7406)  lstat(17)
 7406 format (5x,  58hon the other hand, the most recently attached file
     1, number,  i5,  38h   in order of user specification, has   ,/,
     2 5x,  43hthe following different characteristics ...    )
      write (lunit6, 7306)  ntot, lstat(14), kswtch, bus1
      write (lunit6, 7506)
 7506 format (5x,  99hnever try to combine results which belong to diffe
     1rently-structured problems, as in this data case.     )
      go to 6550
 6207 write (lunit6, 7207)  tmax, tenerg
 7207 format (5x, 104hthis data case has  'statistics'  switches, but it
     1 is highly improbable that any would ever close.   the      ,/,
     2 5x,  50htermination time  'tmax'  of the simulation equals,
     3  e14.4,  42h ,   while all random switch-closing times      ,/,
     4 5x,  6hexceed,  e14.4,   98h   seconds with  3*sigma  probability
     5.   such a waste of computer resources will not be tolerated. ,/,
     6 5x,  96heither increase  'tmax'  beyond this latter figure, or ap
     7propriately decrease the closing times.     )
      go to 6550
 6208 continue
      go  to  6550
 6209 write (lunit6, 7209)  epsiln, lstat(17)
 7209 format (5x,  32hthe jacobian matrix for a newton,
     1             33h solution of zinc-oxide arresters,
     2             31h has been found to be singular.   ,/,
     3        5x,  31hthe tolerance  'epsiln'  equals,
     4     e12.3,  33h ,   while the iteration count is,
     5        i5,   2h .   )
      go to 7412
 6210 write(lunit6,7210)  lstat(14)
 7210 format(5x,101hthe initialization of a saturated synchronous machin
     1e has failed to converge. the machine in question,/, 5x,
     2 24hhad the following number, 2x, i6 )
      go to 6550
 6211 write(lunit6,7211) lstat(14),flstat(13),flstat(14),flstat(15)
 7211 format(5x,58hthe program was inputting data for synchronous machin
     1e no., i8,37ha non-positive set of saturation data, /, 5x, 62hfor
     2one of the axis has been detected. the read in data follow,
     3 25h below this line ........, /, 10x, 3e20.8, /, 5x, 104hin a cas
     4e of an unsaturated s.m. this kill-code is caused by a nonspecifie
     5d value of parameter 'agline'.)
      go to 6550
 6212 write (lunit6, 7212)  maxzno, epstop, flstat(14)
 7212 format (5x,  35ha rigorous solution for one or more,
     1             33h zinc-oxide arresters has failed.,
     2              8h   up to,  i5,
     3             33h   iterations (variable 'maxzno')  ,/,
     4        5x,  33hwere allowed to drive the current,
     5             16h residuals below,   e12.4,
     6             32h   amperes (tolerance 'epstop').   ,/,
     7        5x,   3hbut,   e13.4,
     8             31h   amperes remain for a problem,
     9             34h equation, so the newton iteration,
     9             14h has diverged.     )
 7412 write (lunit6, 7512)  lstat(13), bus1, bus2
 7512 format (5x,  35hby way of component identification,,
     1             10h there are,   i5,
     2             35h   coupled elements which are being,
     3             28h solved simultaneously, with      ,/,
     4        5x,  36hthe first of these (in order of data,
     5             26h input) connecting node  ',  a6,
     6             13h'  to node  ',  a6,
     7             26h' .   the first element is       )
      write (lunit6, 7612)  lstat(15), lstat(14), lstat(16), t
 7612 format (5x,  14hlocated in row,  i5,
     1             34h   of the nonlinear element table,,
     2             34h   while the last is in row number,
     3                     i5,   2h .    ,/,
     4        5x,   9ha rank of,  i5,
     5             34h   exists for  (zthev) ,   and the,
     6             19h simulation time is,  e13.5,  5h sec.  )
      write (lunit6, 7712)
