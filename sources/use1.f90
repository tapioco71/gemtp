...
9003 continue
  ...
  tacs_a = rsto(rptr)
  rptr =rptr - 1
  if (.not. (tacs_a .lt. rmargn)) go to 5031
  tacs_a = zero
  go to 5030
5031 continue
5030 continue
  if (.not. (tacs_b .le. rmargn)) go to 5041
  stpflg = 100
  go to 5040
5041 continue
5040 continue
  p = dmax1 (rmargn, dabs(rmargn * tacs_b))
  if (.not. (tacs_a .gt. tacs_b + p)) go to 5051
  stpflg = 101
  go to 5050
5051 continue
5050 continue
...
5061 continue
5060 continue
  if (.not. (tacs_a .gt. half * tacs_b + p)) go to 5071
  tacs_g = half * tacs_b
  go to 5070
5071 continue
  tacs_g = tacs_a
5070 continue
  tacs_c = rsto(base13 + 8)
  tacs_d = rsto(base3 + 8)
  tacs_e = tacs_c - tacs_d
  if (.not. (tacs_e .le. rmargn)) go to 5081
  stpflg = 102
  stpr1 = tacs_c
  stpr2 = tacs_d
  continue
...
5081 continue
5080 continue
  if (.not. (tacs_e .gt. tacs_b + p)) go to 5091
  tacs_f = tacs_e / dnint (tacs_e / tacs_b + half)
  go to 5090
5091 continue
  tacs_f = tacs_e
5090 continue
  p = dmax1 (rmargn, dabs (rmargn * tacs_g))
  if (.not. (tacs_f .ge. tacs_g - p)) go to 5101
  rsto(base3 + 2) = tacs_g
  rsto(base3 + 3) = tacs_b
  rsto(base3 + 4) = tacs_c
  rsto(base3 + 5) = tacs_e
...
3001 if (.not. (tacs_d + half * tacs_f .lt. tacs_c)) go to 3000
  rsto(base3 + 7) = tacs_d
  ndx = ndx + 1
  tacs_d = rsto(ptr0) + ndx * tacs_f
  if (.not. (tacs_d + half * tacs_f .gt. tacs_c)) go to 5111
  tacs_d = tacs_c
  flg = -1
  go to 5110
5111 continue
5110 continue
  if (.not. (dabs(tacs_d) .le. rmargn)) go to 5301
  tacs_d = zero
  go to 5300
5301 continue
5300 continue
...
3021 if (.not. (env .gt. 0 .and. flg .eq. 0)) go to 3020
  ...
  tacs_a = rsto(rptr)
  rptr = rptr - 1
  if (.not. (tacs_a .gt. rmargn)) go to 5161
  flg = 1
  go to 5160
5161 continue
5160 continue
...
3031 if (.not. (lpflg .gt. 0)) go to 3030
  ...
  tacs_a = rsto(rptr)
  rptr = rptr - 1
  if (.not. (tacs_a .le. rmargn)) go to 5191
  lpflg = 0
  go to 5190
5191 continue
  sptr = sptr - 1
...
5220 continue
  ...
  if (.not. ((r1 .lt. r2 - p  .or. r1 .gt. r2 + p) .and. tacs_a * r3 .le. zero)) go to 5231
  stpflg = 111
  env = isto(sptr)
  sptr = sptr + 1
  stpl1 = isto(env + 4)
...
5230 continue
  tacs_b = r1 - r3
  i = -1
3061 if (.not. (tacs_a * tacs_b .lt. tacs_a * r2 - p)) go to 3060
  i = i + 1
  tacs_b = r1 + i * r3
  rsptr = rsptr - 1
  rsto(rsptr) = tacs_b
