...
c     list-zero "karray" is always 1st, and maybe "over29":
      if ( iprsup  .ge.  1 )
     1 write ( lunit6, 4567 )
 ...
 2456 format (  44h+request for line-constants supporting prog.,
     1          i6   )
      ndim = lphase
      ntri = ndim * (ndim + 1) / 2
      nsqr = ndim * ndim
      ...
      nchain = 51
      go to 99999
   10 call guts44( stg(iofarr), stg(iofxwc), stg(iofxwy), stg(iofyzr),
     1 stg(iofyzi), stg(ioftii), stg(ioftir),stg(ioftvi),
     2 stg(ioftvr), stg(iofer), stg(iofei), stg(iofthe), stg(iofxtr),
     3   stg(iofxti), stg(iofzsu), stg(iofdum), stg(iofdur),stg(ioftix),
     4   stg(iofwor), ndim, ntri, nsqr2 )
      if ( iprsup  .ge.  1 )
     1 write ( lunit6, 4568 )
...
c     subroutine guts44.
c
      subroutine guts44(array, xwc, xwy, yzr, yzi, tii, tir, tvi, tvr,
     1     er, ei, theta2, xtir, xtii, zsurge, dummi, dummr, tixf,
     2     work1, ndim, ntri,nsqr2)
      implicit real*8 (a-h, o-z) ,
     1     integer*4 (i-n)
      include 'blkcom.ftn'
      include 'deck44.ftn'
...
c     fspac needed for conversion of reactance at 1 meter
c     spacing to 1 foot spacing.
c              start of constants definition.
      valu1    =  558 4596.2 d0
...
c              begin calculate constants for carson % bessel.
      bcars(1)=sqrt2/6.
      bcars(2)=unity/16.
 ...
 6603 dcars(i)=bcars(i)*pi/4.
  ...
  811 fked(i-1) = fke(i) * i / 32.
      ...
      if(iprsup.ge.2)write(lunit6,333)(fbe(i),i=1,20),(fbed(i),i=1,20),
     1 (fke(i),i=1,20),(fked(i),i=1,20),(bcars(i),i=1,30),
     2 (dcars(i),i=1,30),(ccars(i),i=1,30)
  333 format(1x,10hdata fbe /  ,/,4(1x,5e25.15,/),/,12h data fbed / ,/
     1 4(1x,5e25.15,/),/,11h data fke / ,/,4(1x,5e25.15,/),/,
     2 12h data fked /  ,/,4(1x,5e25.15,/),/
     3 ,13h data bcars / ,/,6(1x,5e25.15,/),/,13h data dcars / ,/,6(1x,5
     4 e25.15,/),/,13h data ccars / ,/,6(1x,5e25.15,/),/,1x)
      if ( iprsup  .ge.  1 )
     1write (lunit6, 3866)  pi, picon, sqrt2
...
13866 brname(i) = blank
      ...
      muntrn = 0
      mtrnsp = 0
c     segmented, 1, vax e/t can skip translation of rewind:
...
c     optional "branch" card, which species  a6  branch names
      n1 = m + 11
...
54258 if ( bufsem(1) .ne. text6 )  go to 5020
 ...
 4244 format (  45h+blank card terminating line-constants cases.  )
     ...
     1 lunit5 = l5save
      ...
      if ( iprsup .ge. 1   .or.   lastov .eq. 39 )
     1 write (lunit6, 14244)  d1, d1, d1, ipunch
...
c     read input card using cimage
 ...
 4164 if ( kolbeg  .gt.  0 )   go to 8712
      read (unit = abuff(1), fmt = 11) itbic(i), tbtb2(i),
     1     tbr(i),itbtb3(i),tbg(i),tbd(i),
     2     tbx(i), h1, h2, d8, d9, tbtext(i), i3
 ...
 4240 format ( 21h+line-conductor card.,  2e11.3, i6  )
  ...
  706 if (i .le. lphase )   go to 5
    7 kill = 82
      ...
      lstat(19) = 7
      go to 9200
c                                         sorting of conductor data
    6 tbd(i) = 0.0
      if( iprsup .ge. 1 )
     1 write (lunit6, 3625) (itbic(k), itbtb3(k), tbtb2(k), tbr(k),
     2 tbg(k), tbd(k), tbx(k), tby(k), tbtext(k), k=1, i )
 3625 format ( //,  25h unsorted conductor table   ,/,
     1 ( 1x, 2i6, 6e15.5, 5x, a6 ) )
c              '      '      '    convert from si units for calculation.
...
c     segmented, 1, vax e/t can skip translation of rewind:
      ...
      if ( ialter  .ne.  2 )
     1 write (lunit2, 4230)  bufsem
      read (unit = abuff, fmt = 17)
     1     rearth, freq, corr, j1, j2, j3, j4, j5, j6, j7, j8, j9, j10,
     2     j11, j12, iw, dist, j13, j14, j15, j16, isegm, mutual, ik,
     3     ips, j17, imodal, itrnsf, conduc
 17   format(f8.2,f10.2,f10.6,i2,5i1,i2,5i1,i2,f8.3,i2,5i1,3i3,
     1     2i2, e8.2)
 ...
 3168 if ( lastov .eq. 1  .or. imodal .eq. 1 )  go to 217
  ...
  217 jspecl = j1 + j2 + j3 + j4 + j5 + j6 + j7 + j8 + j9
     1        +j10 + j11 + j12
      nfreq = nfreq + 1
      if ( lastov .ne. 1 .and. imodal .eq. 0 .and. nfreq .eq. 2 .and.
...
13723 jprmat (6) = 1
 ...
 3729 kkk = 0
      if ( iprsup  .ge.  1 )
     1 write (lunit6, 7454)  iprint, ldisfr, ialter
 7454 format ( /,  44h new frequency card.  iprint  ldisfr  ialter, /,
     1 20x, 3i8  )
      do 3731  jj = 1, 16
...
c     this check really belongs in "semlyen setup" (overlay 45):
 ...
 7459 iii=0
      ...
      if ( iprsup  .ge.  1 )
     1 write (lunit6, 7457) icheck, ktab, (voltbc(jj), jj=1, 3)
 7457 format ( /, 39h logarithmic frequency.  icheck    ktab,
     1 8x,  7hvolt(1),  8x,  7hvolt(2),  8x,  7hvolt(3),  /,
     2  23x,  2i8,  3e15.6  )
      if ( mfrqpr  .gt.  0 )
     1 write (lunit6, 3004)
      go to 3006
 3005 iii=iii+1
cccc      if ( iii  .ge.  ik )   go to 33316
...
cc    ** identical eigenvalue ? **
 ...
 3006 pkkk = kkk
      ...
      iprint=iprint+1
      if ( iprsup  .ge.  1 )
     1 write (lunit6, 23006)  iprint, kkk, iii, freq, ci1, ck1
23006 format ( /,
     1 63h next logarithmically-spaced frequency.  iprint     kkk     ii
     2i, 11x, 4hfreq, 12x, 3hci1, 12x, 3hck1,  /,
     3 39x, 3i8, 3e15.6  )
 ...
 3007 omega=twopi    *freq
      ...
      write (lunit6, 4)
    4 format ( //,  59h line-conductor table after sorting and initial p
     1rocessing.    ,/, 1x, 5htable, 3x, 5hphase, 3x, 11hskin effect,
     2 4x, 10hresistance, 3x, 28hreactance-data specification, 5x,
     3 8hdiameter, 3x, 10hhorizontal, 3x, 10havg height, 3x,
     4 9hconductor  )
    8 format( 3x, 3hrow, 2x, 6hnumber, 8x,
     5 6hr-type, 4x, 10hr (ohm/mi), 3x, 6hx-type, 6x,
     6 16hx(ohm/mi) or gmr, 5x, 8h(inches), 5x, 8hx (feet),
     7 5x, 8hy (feet), 8x, 4hname   )
      if ( metrik .eq. 0 ) write(lunit6,8)
      if ( metrik .eq. 1 ) write(lunit6,9)
    9 format(3x, 3hrow, 2x, 6hnumber, 8x,
     5 6hr-type, 4x, 10hr (ohm/km), 3x, 6hx-type, 6x,
     6 16hx(ohm/km) or gmr, 5x, 8h(  cm  ), 5x, 8hx (mtrs),
     7 5x, 8hy (mtrs), 8x, 4hname   )
 3008 idist = 1
...
c                                                  formation of p-matrix
   ...
   21 x1=x(j)
      ...
      goto 225
  221 write(lunit6,22) k,ic(j),tb2(j),r(j),itb3(j),gmd(j),d(j),x(j),y(j)
     1  , text(j)
  ...
  222 i=0
   ...
   25 format(//, 46h0following matrices are for earth resistivity=,f8.2,
     121h ohm-m and frequency=,f13.2,23h hz. correction factor=,
     1 f10.6)
...
12345 format(1h ,30x,53h************earth wires will be segmented*******
 ...
 3004 format(1h0,6x,47h-----------------zero sequence-----------------,
     113x,47h---------------positive sequence---------------,/,4x,
     25halpha,8x,4hbeta,8x,1hr,8x,8hl(milli-,4x,8hc(micro-,6x,5halpha,8x
     3,4hbeta,8x,1hr,8x,8hl(milli-,4x,8hc(micro-,4x,9hfrequency,/,
     4 59h neper/mile  radian/mile  ohm/mile  henry/mile) farad/mile)
     5,61h  neper/mile  radian/mile  ohm/mile   henry/mile) farad/mile)
     6 5x,2hhz )
 3020 write (lunt13)  (z(i),i=1,ip)
      if ( iprsup  .ge.  2 )
     1 write (lunit6, 4427)  ( z(i), i=1, ip )
 4427 format ( /, 28h at 4427.  (z(i), i=1, ip) .   ,/,
     1 ( 1x, 8e16.6 ) )
      if ( iprsup  .ge.  1 )
     1 write (lunit6, 4428)  j1, j4, kcirct, j2, j3, j56, kfull
 4428 format ( /, 9h at 4428.  , 7i12 )
c
c                                   reductions and inversions p-matrix
      switch=-unity
      if(j1.gt.0) go to 36
   31 if(j4.gt.0) go to 37
   32 if(kcirct.eq.0) go to 80
      if(j2+j3+j56  .eq.0) go to 80
c                          begin elimination of earth wires and bundling
   49 k=kcirct
   50 k=k+1
...
cccc      write (*,*) ' after redu44 on c.  p(1:6) =',
cccc     1                                ( p(i), i=1, 6 )
      if( kill .gt. 0 )  go to 9200
c                            end elimination of earth wires and bundling
cccc      write (*,*) ' branch to 38?  j2, j3 =',  j2, j3
      if(j2+j3.gt.0) go to 38
   33 if(j56.gt.0) go to 39
c
c                                         formation of z-matrix
   80 switch=+unity
...
c                    begin carson series for mutual impedance if z.le.5.
      rm = twopi / 16.
      zl=alogz(z1)
 ...
 1115 if ( error  .gt.  onehaf )   go to 126
      error = unity
      go to 1005
c                      end carson series for mutual impedance if z.le.5.
c
c                    begin carson series for mutual impedance if z.gt.5.
  151 s=s*z1
      sn=dx/s
      cs=h2/s
      s2=sn*cs*2.0
      ...
      cs=cs*c2-sn*s2
 1006 sn=h1
c                      end carson series for mutual impedance if z.gt.5.
  126 p(ip)=rm*f2
      z(ip)=xm*f2
      if ( iprsup  .ge.  4 )
...
c                    begin carson series for self impedance if z.le.5.0
      r1 = r1 + twopi/16.
      zl=alogz(z1)
 ...
 2115 if ( error  .gt.  onehaf )   go to 127
      error = unity
      go to 2005
c                      end carson series for self impedance if z.le.5.0
c
c                    begin carson series for self impedance if z.gt.5.0
  152 cs=unity/z1
      c2=cs*cs
      r1=r1-c2
      do 2006 i5=1,7,2
      ...
      xs=xs+dcars(i5)*cs
 2006 cs=cs*c2
c                      end carson series for self impedance if z.gt.5.0
  127 r1=r1*f2
  128 z(ip)=xs*f2
      p(ip)=r1
      if ( iprsup  .ge.  4 )
     1 write (lunit6, 4440)  i, k, kfull, kcirct, i5, ip, p(ip), z(ip)
 4440 format ( 9h at 4440   , 6i10, 2e16.6  )
  ...
  150 xs=g1/f2
      ...
      go to 122
c
c                                  routines for z-printing and inversion
c     impedance matrix for physical conductors is in p(1),...p(ip) (real
c     part) and z(1)....z(ip) (imaginary part).
  200 if ( j7  .gt.  0 )
     1 call output ( metrik, p(1), z(1), unity, kfull, ll6, ll1 )
      if ( iprsup  .ge.  1 )
     1 write (lunit6, 4434)  j7, j10, j56, kcirct, j8, mutual, j11, j12,
...
c     impedance matrix for equivalent phase conductors is in p(1),...
c     p(kp) (real part) and z(1),...z(kp) (imaginary part).
      if ( j8out  .gt.  0 )
     1 call output ( metrik, p(1), z(1), unity, kcirct, ll6, ll2 )
      if ( ipunch .ne. 44 )  go to 8202
      call punpie ( kcirct )
 8202 if ( iprsup  .ge.  1 )
     1  write (lunit6, 3202) imodal, dist
 3202 format (  15h imodal, dist =,  i5, e17.5  )
      if ( lastov .ne. 39 )   go to 3203
      if ( nfreq   .ne.  1  )   go to 3203
c     "marti setup" assumes distance in kilometers is on 9:
      distkm = dist / fmipkm
      write (lunit9) imodal, metrik, distkm, mspedb, itrnsf
      if (iprsup .gt. 1)
     1  write (lunit6, *) ' mspedb written on lunit9 =', mspedb
 3203 if ( imodal .le. 0 )  go to 2202
      if( lastov .eq. 1  .and.  itrnsf .ne. -1  .and.
 ...
 1202 if ( nfreq .eq. 3 )  ntol = ntol + 1
      ...
      call modal(array, xwc, xwy, yzr, yzi, tii, tir, tvi, tvr, er, ei,
     1     theta2, xtir, xtii, zsurge, dummi, dummr, tixf, work1, freq,
     2     kcirct, iw, dist, metrik, fmipkm, ndim, ntri, nsqr2, itrnsf,
     3     kfull, mrr, nrp, ntol, conduc)
 ...
 2202 if (lastov.eq.39 .or. lastov.eq.45 .or. imodal .ne.0) go to 8777
      ...
      rewind lunit1
      if ( ipunch  .eq.  0 )
     1 write (lunit7, 8769)  tclock, date1
 8769 format (    42hc   punched card output of transposed line,
     1            15h which began at,  2x,  2a4,  2x,  2a4  )
      if ( ipunch .ne. 0 )  go to 8770
      if ( imodal .eq. 0 .and. mspedb .eq. 0) write (lunit7, 8780) freq
 8780 format (56hc   ***** transposed k.c. lee line segment calculated a
     1t, 2x, e10.3, 10h hz. *****   )
      if ( imodal .eq. 0 .and. mspedb .eq. 1) write (lunit7, 8785) freq
 8785 format (59hc **** special double circuit transposed line calculate
     1d at, 2x, e10.3, 9h hz. ****         )
 8770 rewind lunit2
      n5 = 0
c     if ( jdatcs .gt. 0 )  go to 8777
      do 8774  n12=1, 9999
      read (lunit2, 8771)  ( texta6(i), i=1, 14 )
 8771 format ( 13a6, a2 )
      if ( ipunch  .eq.  0 )
     1 write (lunit7, 8772)  ( texta6(i), i=1, 13 )
 8772 format ( 2hc ,  13a6 )
      if (idebug.eq.0) go to 8100
      write (lunit1, 8773)  ( texta6(i), i=1, 14 )
 8773 format ( 3h c ,  13a6,  a2 )
      kount = kount+1
 8100 if ( texta6(1) .eq. blank     .and.
     1     texta6(2) .eq. blank )   n5 = n5 + 1
 ...
 8777 if ( ialter  .le.  0 )   go to 3734
     ...
     + )
 7423 format ( /,  39h output of  (r)  and  (l)  for semlyen,    ,
     1 i6,   13h  cells each.,  /,  ( 1x,  8e16.4 ) )
 3734 if ( mutual  .gt.  0 )
     1 call outspc(p(1), z(1),kcirct,metrik,fmipkm)
      j56 = j11+j12+idist
...
c     inverted impedance matrix for equivalent phase conductors is in
c     p(1),...p(kp) (real part) and z(1),...z(kp) (imaginary part).
      if ( j11  .gt.  0 )
     1 call output ( metrik, p(1), z(1), unity, kcirct, ll5, ll2 )
...
c     inverted impedance matrix for symmetrical components of equivalent
c     phase conductors is in p(1),...p(kk*(kk+1)/2) (real part) and z(1)
c     ,...z(kk*(kk+1)/2) (imaginary part). definition of kk same as for
c     inverse susceptance matrix
...
c     inverted impedance matrix for physical conductors is in p(1),...
c     p(ip) (real part) and z(1),...z(ip) (imaginary part).
...
c     segmented, 1, vax e/t can skip translation of rewind:
      rewind lunt13
  254 go to 45454
c  254 if ( lastov .eq. 39  .or.
c     1          kcirct .gt. 3 ) go to 45454
  256 call symm ( p(1), z(1), unity, kcirct, kk )
c     impedance matrix for symmetrical components of equivalent phase
c     conductors is in p(1),...p(kk*(kk+1)/2) (real part) and z(1),...
c     z(kk*(kk+1)/2) (imaginary part). definition of kk same as for
c     inverse susceptance matrix.
      if ( j9out  .gt.  0 )
     1 call output ( metrik, p(1), z(1), unity, kk, ll6, ll3 )
...
cccc      write (*,*) ' ready for z avg.  p(1:6) =',
cccc     1                              ( p(i), i=1, 6 )
cccc      write (*,*) ' ready for z avg.  z(1:6) =',
cccc     1                              ( z(i), i=1, 6 )
cccc      write (*,*) ' imodal, nfreq =',  imodal, nfreq
c      if (imodal .ne. 0)    go to 2254
 ...
 6256 if ( nfreq .eq. 1 )  go to 9998
      ...
      go to 3006
c     if (lastov .eq. 39  .and.  nfreq  .eq. 1)    go to 9998
cccc  dor = p(1)
cccc  doi = z(1)
cccc  d1r = p(ll5)
cccc  d1i = z(ll5)
 ...
 5060 continue
      if (iprsup .ge. 1 )
     1  write (lunit6, 5065) freq, kcirct, (zsurge(i),theta2(i),
     2                       er(i), ei(i), i = 1,3 )
 5065 format ('  at 5065, freq,kcirct,zsurge,theta2,er,ei are: ',/,
     1 e10.3,i3,12e9.2 )
      go to 9998
 ...
 5081 continue
      if (iprsup .ge. 1 )
     1 write (lunit6,*) ' zsurge, er = ',(zsurge(i),er(i),i=1,3)
      do 5088  i = 1, kcirct
      if ( i .gt. 1 )  go to 5082
      write (lunit7,140) i, brname(2*i-1), brname(2*i),
     1      xtir(i), zsurge(i), er(i), dist
      go to 5088
 5082 if ( i .gt. 2 )  go to 5084
      write (lunit7,140) i, brname(2*i-1), brname(2*i),
     1     xtir(3), zsurge(3), er(3), dist
      go to 5088
 5084 if ( i .gt. 3 )  go to 5086
      write (lunit7,140) i, brname(2*i-1), brname(2*i),
     1     xtir(2), zsurge(2), er(2), dist
      go to 5088
 5086 if ( i .le. 9 )
 ...
 5059 xtir(1) = p(1)
      xtii(1) = z(1)
      go to 480
cccc      write (*,*) ' enter do 478 loop.   kcirct =',  kcirct
 5090 do 478  i=1, kcirct
      rdiag = rdiag + p(n55)
      xdiag = xdiag + z(n55)
      do 479  jp=n55-i+1, n55-1
      roff = roff + p(jp)
  479 xoff = xoff + z(jp)
  478 n55 = n55 + i + 1
      rdiag = rdiag / kcirct
...
cccc      write (*,*) ' average of diagonals,  rdiag, xdiag =',  rdiag,
cccc      write (*,*) ' average of off-diag,  roff, xoff =',  roff, xoff
     ...
     1   go to 480
      ...
      write (lunit6,*) ' gmode,xtir, xtii, , dummi =',
     1  gmode, (xtir(i),xtii(i),dummi(i), i = 1, 2)
 ...
 5094 continue
      if (iprsup .ge. 1 )
     1  write (lunit6, 5095) freq, kcirct, (zsurge(i),theta2(i),
     2                       er(i), ei(i), i = 1,2 )
 5095 format ('  at 5095, freq,kcirct,zsurge,theta2,er,ei are: ',/,
     1 e10.3,i3,8e12.3 )
      go to 9998
 2254 rzero1 = rzero
      rpos1 = rpos
...
13012 format (19h at 13012, ipunch =, i10)
 ...
 3011 format(5e15.5)
      go to 13039
13011 if ( mfrqpr  .gt.  0 )
     1 write (lunit6, 3012)  alphao, beto, rzero, xzero,
     2  yzero, alpha1, bet1, rpos, xpos, ypos, freq
...
13026 write (lunit9)  rzero, xzero, freq
      if (iprsup .ge. 1)
     1 write (lunit6, 23024)  rzero, xzero, freq
...
13039 if (ik .gt. 0)   go to 3006
 3010 vol=omega/beto
      ...
      write(lunit6,9991)zso,zsodrg,alphao,vol,waveo,rzero,xzero1,yzero1,
     1                  zs1,zs1drg,alpha1,v1l,wave1,rpos,xpos1,ypos1
 9991 format(9h0sequence,6x,15hsurge impedance,7x,76hattenuation   veloc
     1ity    wavelength   resistance    reactance   susceptance/9x,
     2102hmagnitude(ohm) angle(degr.)   db/mile      miles/s       miles
     3      ohm/mile     ohm/mile     mho/mile/9h   zero  ,8e13.5/
     4 9h positive,8e13.5)
      go to 9998
 9992 alphao = alphao * fmipkm
      ...
      ypos1 = ypos1 * fmipkm
      write(lunit6,9993)zso,zsodrg,alphao,vol,waveo,rzero1,xzero1,
     1           yzero1,zs1,zs1drg,alpha1,v1l,wave1,rpos1,xpos1,ypos1
 9993 format(9h0sequence,6x,15hsurge impedance,7x,76hattenuation   veloc
     1ity    wavelength   resistance    reactance   susceptance/9x,
     2102hmagnitude(ohm) angle(degr.)    db/km         km/s          km
     3       ohm/km       ohm/km       mho/km /9h   zero  ,8e13.5/
     4 9h positive,8e13.5 )
...
c     segmented, 1, vax e/t can skip translation of rewind:
      rewind lunt13
      go to 203
c                          end of routines for z-printing and inversions
c
c                                routines for p-printing and inversions
   36 i2=1
c     inverse susceptance or capacitance matrix for physical conductors
c     is in p(1),...p(ip).
      if (iw.eq.0) i2 = i2+1
   ...
   37 i2=3
      if ( iprsup  .ge.  1 )
     1write (lunit6, 4444)  iw, ip, ll0, ll1
 4444 format ( /,  28h at 4444.  iw, ip, ll0, ll1=, 4i10 )
  ...
  371 z(i)=-p(i)
      ...
      if( kill .gt. 0 )  go to 9200
c     susceptance or capacitance matrix for physical conductors is in
c     z(1),...z(ip).
      call output ( metrik, z(1), p(1), switch, kfull, i2, ll1 )
      go to 32
   38 i2=1
c     inverse susceptance or capacitance matrix for equivalent phase
c     conductors is in p(1),...p(kp)
c     to store wc(inv) or c(inv) for modal analysis
      if ( iprsup .ge. 1 )
     1  write (*,*) ' at s.n. 38,  p(1) =',  p(1)
      if ( imodal .le. 0 )  go to 403
      do 404  i = 1, kp
  404 xwc(i) = p(i)
  403 if (iw.eq.0) i2=i2+1
      if ( j2out  .gt.  0 )
     1 call output ( metrik, p(1), z(1), switch, kcirct, i2, ll2 )
      if ( iprsup  .ge.  4 )
     1 write (lunit6, 2654)  j3, kcirct, j56, kp, iw, i2, j2,
     2                       ( p(i), i=1, kp )
 2654 format ( /,  36h before 'c'-matrix call to  'symm' .,
     1 32h      j3  kcirct     j56      kp,
     2 24h      iw      i2      j2  ,/,  36x,  7i8  ,/,
     3  29h (p(i), i=1, kp)  follow ....  ,/,  ( 1x,  8e16.7 )  )
...
c     segmented, 1, vax e/t can skip translation of rewind:
   ...
   40 call symm ( p(1), z(1), switch, kcirct, kk )
      i2=1
      if (iw.eq.0) i2=i2+1
c     inverse susceptance or capacitance matrix for symmetrical compo-
c     nents of equivalent phase conductors (grouped as 3-phase circuits
c     in order 1-2-3, 4-5-6 etc.) is in p(1),...p(kk*(kk+1)/2) for real
c     part and z(1),...z(kk*(kk+1)/2) for imaginary part with kk=highest
c     number of equivalent conductor being a multiple of 3.
...
c     is in p(1),...p(kp).
 ...
 5130 continue
      ...
      coff = coff / 6
      if ( iprsup .ge. 1 )
     1  write (lunit6,*) ' cdiag, coff are ', cdiag, coff
      n55 = 6
      do 5150  jp = 4, 6
      j = n55 + 1
      do 5140  i = j, j+2
 ...
 5150 continue
      ...
      dummi(3) = cdiag - coff
      if ( iprsup .ge. 1)
     1  write (lunit6,*) ' at 5150, dummi =', (dummi(i),i= 1,3)
 ...
 5190 if ( kcirct .lt. 2 )  go to 1379
      do 378  i=1, kcirct
      cdiag = cdiag + p(n55)
      do 379  jp=n55-i+1, n55-1
  379 coff = coff + p(jp)
  378 n55 = n55 + i + 1
      cdiag = cdiag / kcirct
      coff = coff / ( kcirct * ( kcirct - 1 ) / 2 )
c     write (*,*) ' average of diagonals,  cdiag =',  cdiag
c     write (*,*) ' average of off-diag,   coff =',   coff
 ...
 3741 if ( idist  .eq.  0 ) go to 393
  ...
  393 if(j6.eq.0 .or. kcirct.lt.2) go to 80
      ...
      i2=3
      if(iw.eq.0) i2=i2+1
c     susceptance or capacitance matrix for symmetrical components of
c     equivalent phase conductors is in p(1),...p(kk*(kk+1)/2) for real
c     part and z(1),...z(kk*(kk+1)/2) for imaginary part. definition of
c     kk same as for inverse susceptance matrix
      if ( j6out  .gt.  0 )
     1 call output ( metrik, p(1), z(1), unity, kk, i2, ll3 )
...
98765 format (15h czero on unit9, e15.6)
  ...
  399 yzero = p(1)
      ...
      ypos=ypos*omega
      go to 80
c                          end of routines for p-printing and inversions
c
c                                        changes in conductor data
  500 icount=0
      k=0
...
c     read input card using cimage
 504  call cimage
      ...
      if ( ialter  .ne.  2 )
     1     write (lunit2, 4230)  bufsem
      read (unit = abuff, fmt = 11)
     1     i1, y1, r1, i2, g1, d1, x1, h1, h2, d8, d9, bus1, i3
 ...
 1505 i = i +1
  ...
  507 i = kfull + 1
      go to 6
c
c                            extending the matrices for specified length
  ...
  602 format(1h+,41x,58hcannot be calculated with number of equivalent c
     1onductors=,i3)
      go to 16
  603 ip=0
      d1 = fltinf
  ...
  606 i2=i2*2
      write(lunit6,607)
  607 format(1h+,41x,60hcannot be calculated with number of necessary se
     1ctions=2**33)
      go to 16
  608 x1=i2
      i1=isec-1
  ...
  609 format (38h computed by connecting in cascade 2**,i2,18h equal sec
     1tions of, e13.6, 12h miles each.)
      go to 615
  613 deltam = deltad / fmipkm
      write( lunit6,614 ) i1, deltam
  614 format(1h+,41x,36hcomputed by connecting in series 2**,i2,18h equa
     1l sections of,f9.4,9h km each. )
  615 continue
      f1=unity/deltad
      f2=deltad* onehaf
      ...
      gd(i)=r1+r1
  621 bd(i)=x1+d1
c                                  begin of loop for connecting sections
      icount=0
  622 icount=icount+1
...
c                                       expanding matrix
  ...
  626 call cxred2( p(1), z(1), workr1(1), workr2(1), kcir2, kcirct )
      go to 622
c                                       output of matrices for length
  650 do 651 i=1,kp
      gd(i)=gd(i)-p(i)
      bd(i)=bd(i)-z(i)
      p(i) = (p(i) - gd(i)) * 2.
...
c     transfer admittance matrix for equivalent cond. is in gd+j*bd.
      call output ( ll0, gd(1), bd(1), unity, kcirct, ll7, ll2 )
c     shunt admittance matrix for equivalent cond. is in p+j*z.
      call output ( ll0,  p(1),  z(1), unity, kcirct, ll8, ll2 )
  653 if(j14.eq.0) go to 654
      call symm ( gd(1), bd(1), unity, kcirct, kk )
c     transfer admittance matrix for symmetrical comp. is in gd+j*bd.
      call output ( ll0, gd(1), bd(1), unity, kk, ll7, ll3 )
      call symm ( p(1), z(1), unity, kcirct, kk )
c     shunt admittance matrix for symmetrical comp. is in p+j*z.
...
c     transfer impedance matrix for equivalent conductors is in gd+j*bd.
      call output ( ll0, gd(1), bd(1), unity, kcirct, ll9, ll2 )
c     shunt impedance matrix for equivalent conductors is in p+j*z.
      call output ( ll0,  p(1),  z(1), unity, kcirct, ll10, ll2 )
  656 if(j16.eq.0) go to 16
      call symm ( gd(1), bd(1), unity, kcirct, kk )
c     transfer impedance matrix for symmetrical comp. is in gd+j*bd.
      call output ( ll0, gd(1), bd(1), unity, kk, ll9, ll3 )
      call symm ( p(1), z(1), unity, kcirct, kk )
c     shunt impedance matrix for symmetrical comp. is in p+j*z.
 ...
 1201 format ( 11h$vintage, 1)
      ...
      yd(kk) = yd(kk) / tenm6
      if ( kk .eq. j )
     1  write (lunit7, 5201) i, brname(2*i-1), brname(2*i), p(kk),
     2                       z(kk), yd(kk)
 5201 format ( i2, 2a6, 12x, 3e16.5)
      if ( kk .ne. j)
     1  write (lunit7, 6201) p(kk), z(kk), yd(kk)
...
c     subroutine modal.
c
      subroutine modal(array, xwc, xwy, yzr, yzi, tii, tir, tvi, tvr,
     1     er, ei, theta2, xtir, xtii, zsurge, dummi, dummr, tixf,
     2     work1, freq, m, iw, dist, metrik, fmipkm, ndim, ntri, nsqr2,
     3     itrnsf, kfull, mrr, nrp, ntol, conduc)
      implicit real*8 (a-h, o-z) ,
     1     integer*4 (i-n)
      include 'blkcom.ftn'
      include 'labl44.ftn'
  ...
  100 if ( iprsup .ge. 1 )
     1 write (lunit6,*) ' top of modal.   ndim =',  ndim
      ll0 = 0
      mm=m*(m+1)/2
      c=unity
      if (iprsup .ge. 1 )
     1 write (lunit6, 300) m, mm, (p(i),z(i), i=1, mm)
  300 format ( 34h at the top of modal, m and mm are, 2i10,
     1  /,  28h (p(i),  z (i), i=1, mm) are,/, (1x, 8e15.6) )
      d13 = twopi * freq
      if ( iw .eq. 1 ) c = c / d13
      do 27 i=1,mm
      xwc(i) = xwc(i) * c
...
c    const. [t], marti setup will bypass the exact diagonalization
c     at each looping frequency
      if (nfreq .ne. 1  .and.  lastov .eq. 39
     1                  .and. itrnsf .ne. 1) go to 2520
...
c ***** create y * z matrix from array from lcp main
      k=0
    3 k=k+1
      do 50 j=1,m
c     n=max0z(j,k)
      ...
      er(j)=p(n)
   50 ei(j)=z(n)
c **** the k-th column of r+xwl is stored
      i=0
    2 i=i+1
      cr=0.0
      cl=0.0
...
c **** the element of row i and column k of yz is obtained
      yzr(i,k)=cl
      yzi(i,k)=cr
      if (iprsup .ge. 1 )
     1 write (lunit6, 350) i,k,yzr(i,k),yzi(i,k)
  350 format ( 46h       i       k       yzr(i,k)       yzi(i,k)
     1  ,/,  2i8,  2e15.6 )
      if(i.lt.m)go to 2
c **** yz is obtained in order of (1,1),(2,1),...,(1,2),(2,2),columnwise
      if(k.lt.m)go to 3
c *** find eigenvalues + eigenvectors ***
      if ( kexact .eq. 88333 )  go to 6999
      if ( itrnsf .eq. -1 )  go to 459
      if ( lastov .eq. 1 .or. itrnsf .ne. 1)  go to 7011
c $$$$  change to zy-matrix i.e. transformation matrix is tv   $$$$
 6999 do 7001 i=1,m
      do 7001 j=1,m
      tempr(i,j)=yzr(j,i)
      tempi(i,j)=yzi(j,i)
 7001 continue
      ...
      yzi(i,j) = tempi(i,j)
 7008 continue
      call dceign(yzr,yzi,tvi,tvr,er,ei,m,m,ierror,ll1,ll0,lunit6
     1, iprsup,ndim)
c      write(*,*)(er(i), i=1,m)
c      write(*,*)(ei(i), i=1,m)
      go to 7012
 7011 call dceign(yzr,yzi,tii,tir,er,ei,m,m,ierror,ll1,ll0,lunit6
     1, iprsup,ndim)
c      write(*,*)(er(i), i=1,m)
c      write(*,*)(ei(i), i=1,m)
      go to 7072
...
c $$$$  put data in ping vector  $$$$
 ...
 5605 continue
c $$$$  begin the process of eigenvectors  $$$$
      do 5613 i=1,m
      ee1(i)=p(i)
      ee2(i)=z(i)
      do 5613 j=1,m
...
c $$$$ use tv  $$$$
 ...
 5612 continue
c $$$$ new code to get ti from tv-t $$$$
 5620 ll=1
      do 7002 i=1,m
      do 7003 j=1,m
      tixf(ll)=tvr(i,j)
 ...
 7002 continue
      ...
      ll=1
 7766 if (iprsup .ge. 3)
     1 write(lunit6,*) ' before do 7004.  m, ndim, freq =',
     1                               m, ndim, freq
      do 7004 j=1,m
      do 7005 i=1,m
      tir(i,j)=work1(ll)
...
c
 ...
 5812 do 5614 j=1,m
      ...
      da=pp2(kmax(j),j)
      if (iprsup .ge. 3)
     1  write(*,*) 'in do loop 5614, j=, kmax(j)=,dv=,da=',
     1                             j, kmax(j), dv, da
      do 5624 i=1,m
      if (iprsup .ge. 3)
     1 write(*,*) 'in do loop 5624, i, j, pp1(i,j), pp2(i,j)',
     1                              i, j, pp1(i,j), pp2(i,j)
      pp1(i,j)=pp1(i,j)*dv
      pp2(i,j)=pp2(i,j)-da
c     *** to keep angles within principal value region
      ...
      if (iprsup .ge. 3)
     1 write(*,*) 'just before 5624, i, j, pp1(i,j), pp2(i,j)',
     1                              i, j, pp1(i,j), pp2(i,j)
 5624 continue
 5614 continue
cc    ** fix the error of using  ti as tv  **
      do 6001 i=1,m
      do 6001 j=1,m
 ...
 6001 continue
 7120 if ( iprsup .ge. 1 )
     1 write (lunit6, 400) freq, ((tir(i,j),tii(i,j),j=1,m),i=1,m)
  400 format (' at ', e12.5, 'hz, eigenvectors tir and tii after scaling
     1 are ' ,/,   (1x, 8e15.6) )
 ...
 459  do  462  j = 1, m
         do  462  i = 1, m, 3
            ij = i + 2
            call cimage
            read (unit = abuff(1), fmt = 469)
     1           (tir(j, ibk), tii(j, ibk), ibk = i, ij)
 469        format( 6e13.0 )
 462  continue
c     c   $$$$ skip the rotation and normalization  for freq-dep [t] ??
 463  if( lastov.eq.39 .and. itrnsf.eq. 1  .or. kexact .eq. 88333)
     1     go to 2500
      if ( itrnsf .eq. -9  .or.  itrnsf .eq. -1 )  go to 464
      if ( iprsup .lt. 4 ) go to 88
      write (lunit6, 1462)
 1462 format( 65h current transformation matrix before zeroing the imagi
     1nary part: )
      write (lunit6, 39)
      do 1472  k = 1, m
...
c $$$$  calculation of tv from ti  $$$$
 ...
 6005 continue
 6004 continue
cc    ** now, we have both ti and tv **
      if ( iprsup .ge. 1 )
     1 write (lunit6, 403)  ((tvr(i,j),tvi(i,j),j=1,m),i=1,m)
 ...
 8769 format (    40hc    punched output of k. c. lee's line ,
     1            15h which began at,  2x,  2a4,  2x,  2a4  )
      write (lunit7, 8770)  freq
 8770 format ( 'c  ***** untransposed k.c. lee line segment calculated a
     1t', 2x, e10.3,  ' hz. *****')
      rewind lunit2
      n5 = kfull+nfreq+1
      do 8774  n12=1, n5
...
c *** xwy  is j(wc) ******
 ...
 3151 continue
   ...
   43 g=g+xa*c-xb*d1
      if ( iprsup .ge. 1 )
     1 write (lunit6, *) ' modal admittance ym for mode ', i, g, b
   44 format(//,106h mode   resistance  reactance  susceptance
     1   surge impedance(ohm)          velocity  attenuation)
   65 format(104h          ohm/mile    ohm/mile   s/mile        real
     1     imag       lossless     mile/sec  neper/mile)
 7031 format(102h          ohm/km      ohm/km     s/km         real
     1     imag      lossless     km/sec     neper/km)
   33 format(i5,3x,9e12.5)
c to get rotation for  y=0+xwc
      ya = atan2z(b,g)
      theta=pi*onehaf- ya
      theta2(i)=-theta*onehaf
      dumm1=sqrtz(g*g+b*b)
      continue
c **** to get zmodal, zsurge, vmode from ymodal & eigenvalues ****
...
cc  ** new coding **
 ...
 1232 ycharm = dumm1 / gamma
      ychara = ya - ea
c  ***********************************
c     comment the following two lines if ych is to be fitted
c      ycharm = 1./ycharm    !  zch = 1 / ych
c      ychara = - ychara
c   ********************************
      alpha = gamma * cosz(ea)
      beta = gamma * sinz(ea)
...
c     n = max0z(j,l)
 ...
 2950 zsurge(i)=sqrtz(b/dumm1)
  ...
  137 write(lunit6,33)i,g,b,dumm1,zcharr,zchari,zsurge(i),vmode,cc
      distm = -dist
      if ( m  .lt.  10     .and.     lastov  .eq.  1 )
     1 write (lunit7,140) i, brname(2*i-1), brname(2*i),
     1             g,zsurge(i),vmode,distm,m
  ...
  141 if ( m .eq. 10 )  text1 = text2
      ...
      if ( i .le. 9 )
     1write (lunit7,143) i, brname(2*i-1), brname(2*i),
     1             g,zsurge(i),vmode,distm,text1
  143 format (1h-,i1,2a6,12x,4e12.5,2h 1,2x,a1)
      if ( i .gt. 9 )
     1write (lunit7,145) i, brname(2*i-1), brname(2*i),
     1             g,zsurge(i),vmode,distm,text1
  145 format (i2,2a6,12x,4e12.5,2h 1,2x,a1)
      go to 40
  155 write(lunit9) d13,ycharm,ychara,alpha,beta
      if ( iprsup .ge. 1 )
     1  write (lunit6, 158) d13, i, ycharm,ychara,alpha,beta
  158 format (' at ', e12.5,' hz,ycharm,ychara,alpha,beta for
     1mode',  i5, ' are', 5x, 4e12.5)
  ...
  240 format (11h$vintage, 0)
      ...
      tii(j,i)=cc*d1+dd*c
   37 continue
c ***** end of rotation of ti matrix for k.c.lee's const.param.*******
   39 format(//,33h      real components, row by row)
  166 write(lunit6,66)
   66 format(////,69h eigenvector matrix ti for current transformation i
     1(phase)=ti*i(mode))
      write(lunit6,39)
    9 format(2x,6e12.5)
      do 57 k=1,m
   ...
   60 format(//,37h     imaginary components, row by row)
      do 61 k=1,m
   61 write(lunit6,9)(tii(k,j),j=1,m)
      do 1204  i =1, m
...
c to get zsurge matrix in phase domain
   75 do 80 i=1,m
      c = 1./zsurge(i)
      do 80 j=1,m
 ...
 9900 return
      ...
      implicit real*8 (a-h, o-z) ,
     1      integer*4 (i-n)
c   this subroutine performs inversion of a complex matrix
      include  'blkcom.ftn'
      include  'labl44.ftn'
  ...
  170 k = k + nph2
      ...
      subroutine dceign(ar,ai,vi,vr,er,ei,n,nm,ierr,nv,nb,lunit6
     1,iprsup,ndim)
      implicit real*8 (a-h, o-z) ,
     1      integer*4 (i-n)
      dimension ar(ndim,ndim),ai(ndim,ndim),er(ndim),ei(ndim)
     1,vi(ndim,ndim),vr(ndim,ndim)
      dimension scale(20),int(20),iord(20)
      if ( iprsup .ge. 1 )
     1 write (*,*) ' top of dceign.  input matrix ar ....'
      do 2222 i=1,n
      if ( iprsup .ge. 1 )
     1 write(*,*) (ar(i,j), j=1,n)
 2222 continue
      if ( iprsup .ge. 1 )
     1 write (*,*) '                 input matrix ai ....'
      do 2223 i=1,n
      if ( iprsup .ge. 1 )
     1 write(*,*) (ai(i,j), j=1,n)
 2223 continue
      if ( n .gt. nm ) go to 90
      low=1
      nupp=n
...
c  do transformation to upper hessenberg form
1     continue
      if (iprsup .ge. 3 )
     1  write (lunit6, 300) nm,n,low,nupp,nb,nv,((ar(i,j),ai(i,j),j=1,
     2                      nm),i=1,nm)
  300 format ( 49h before call comhes, nm, n, low, nupp, nb, nv are,
     1  6i5,/, 38h ((ar(i,j),ai(i,j),j=1,nm),i=1,nm) are,/, (1x,8e15.6))
      call comhes(nm,n,low,nupp,ar,ai,int,lunit6,iprsup,ndim,iord)
      if(nv.eq.0) go to 12
c  calculate values and vectors
      if ( iprsup .ge. 3 )
     1  write (lunit6, 350) ((ar(i,j),ai(i,j),j=1,nm),i=1,nm)
  350 format ( 43h after call comhes, ar(i,j) and ai(i,j) are,/,
     1     (1x, 8e15.6) )
      call comlr2(nm,n,low,nupp,int,ar,ai,vi,vr,er,ei,ierr,ndim
     1,iord)
      if (iprsup .lt. 3 ) go to 410
      write (lunit6,400)  (er(i),ei(i),i=1,nm)
  400 format ( 49h after call comlr2 in dceign, the eigenvalues are,/,
     1     ( 1x, 8e15.6), / )
      write (lunit6, 405)  ((vr(i,j),vi(i,j),j=1,nm),i=1,nm)
  405 format ( 21h and eigenvectors are,/ (1x,8e15.6) )
  410 if(ierr.ne.0) go to 5
c  transform vectors to vectors of original matrix
      if(nb.eq.0) return
      call cbabk2(nm,n,low,nupp,scale,n,vi,vr,ndim)
      return
c  calculate values only
12    continue
...
5     write(lunit6,6) ierr
    6 format ( 31h ***warning-dceign:  eigenvalue,  i4,
     1       18h did not converge.)
      return
90    ierr=-1
      write(lunit6,91) n,nm
   91 format(52h **error-dceign:  order of matrix greater than first,
     1 20h dimension of matrix,/,18x,6horder=,i5,
     2 17h first dimension=, i5)
...
c
c     &&&&&&&&&&&&&&&   temporary patch for f4v vax compiler:
      radix = 16.0d0
c     data statement just above is disabled (error for f4v). &&&
c     &&&&&&&&&&&&&&&&&&&&&&&&&&&&  end temporary patch.  23 jan
      b2 = radix * radix
      k = 1
      l = n
      go to 100
...
c                igh+1 step 1 until n do -- ::::::::::
  ...
  200 return
      ...
      subroutine comhes(nm,n,low,igh,ar,ai,int,lunit6,iprsup,ndim
     1,iord)
      implicit real*8 (a-h, o-z) ,
     1      integer*4 (i-n)
...
c     procedure cdiv by complex division using subroutine cmplxz.
c
c     questions and comments should be directed to b. s. garbow,
c     applied mathematics division, argonne national laboratory
c
c     ------------------------------------------------------------------
c
cccc      flzero = 1.d-12       !replaced by epscmh }
      epscmh = 1.e-15
      do 121 i=1, 20
      iord(i) = i
  121 continue
      ...
      kp1 = low + 1
      if (iprsup .ge. 3 )
     1  write (lunit6, 20 )  igh, low, la, kp1
   20 format ( 47h at the top of comhes, igh, low, la and kp1 are, 4i10)
      ...
      do 180 m = kp1, la
         ...
         i = m
         do 100 j = m, igh
cccc            if (absz(ar(j,mm1)) + absz(ai(j,mm1))
cccc     1         .le. absz(xr) + absz(xi)) go to 100
            if ((ar(j,mm1)**2) + (ai(j,mm1)**2)
     1         .le. (xr*xr) + (xi*xi)) go to 100
            xr = ar(j,mm1)
            xi = ai(j,mm1)
            i = j
  100    continue
...
c     :::::::::: interchange rows and columns of ar and ai ::::::::::
  ...
  120    continue
c     :::::::::: end interchange ::::::::::
c  130    if (xr .lt. d13 .and. xi .lt. d13)  go to 180
cc  130    if (absz(xr) .lt. d13 .and. absz(xi) .lt. d13)  go to 180
  130    if (xr .eq. 0.0 .and. xi .eq. 0.0)  go to 180
         mp1 = m + 1
          do 160 i = mp1, igh
            yr = ar(i,mm1)
            yi = ai(i,mm1)
c      if (yr .lt. d13 .and. yi .lt. d13) go to 160
cc    if (absz(yr) .lt. d13 .and. absz(yi) .lt. d13) go to 160
  ...
  160    continue
      if ( iprsup .ge. 3 )
     1 write (lunit6, 170) m, igh, int(m),(ar(j,m),ai(j,m),j=1,igh)
  170 format ( 29h m, igh and int(m) at 170 are, 3i10,/,
     1  31h (ar(j,m),ai(j,m),j=1,igh), are,/, (1x,8e15.6) )
  180 continue
  200 return
      end
...
c                epmach = 16.0d0**(-13) for long form arithmetic
c                on s360 ::::::::::
c     data epmach/z3410000000000000/
c     epmach will be set to equal to the system dependent floating
c     point zero variable  "flzero"
      epmach = flzero
      ierr = 0
...
c                for l=ien step -1 until low d0 -- ::::::::::
  240 do 260 ll = low, ien
         l = ien + low - ll
         if (l .eq. low) go to 300
         if (absz(hr(l,l-1)) + absz(hi(l,l-1)) .le.
     1      epmach * (absz(hr(l-1,l-1)) + absz(hi(l-1,l-1))
     1             + absz(hr(l,l)) + absz(hi(l,l)))) go to 300
  260 continue
c     :::::::::: form shift ::::::::::
...
c     :::::::::: for m=ien-1 step -1 until l do -- ::::::::::
      do 380 mm = l, ienm1
         m = ienm1 + l - mm
         if (m .eq. l) go to 420
         yi = yr
...
c     :::::::::: last card of comlr ::::::::::
      ...
      subroutine comlr2(nm,n,low,igh,int,hr,hi,zi,zr,wr,wi,ierr,ndim
     1,iord)
      implicit real*8 (a-h, o-z) ,
     1      integer*4 (i-n)
      include  'blkcom.ftn'
...
c
c        the upper hessenberg portions of hr and hi have been
c          destroyed, but the location hr(1,1) contains the fnorm
...
c                on s360 ::::::::::
c     data epmach/z3410000000000000/
c     like in subroutine comlr, epmach is set to be flzero
cccc      epmach = 1.0e-50      ! now in choice.dat }
      epmach = 1.e-32
      itsmax = 300
      ierr = 0
...
c     :::::::::: for i=igh-1 step -1 until low+1 do -- ::::::::::
  ...
  160 continue
      if ( iprsup .ge. 3 )
     1 write (lunit6, 170) ((zr(i,j),zi(i,j),j=1,igh),i=1,igh)
  170 format ( 69h after do 160 loop in comlr2, ((zr(i,j),zi(i,j),j=1,ig
     1h),i=1,igh) are,/, (1x,8e15.6) )
c     :::::::::: store roots isolated by cbal ::::::::::
  180 do 200 i = 1, n
         if (i .ge. low .and. i .le. igh) go to 200
         wr(i) = hr(i,i)
...
c     :::::::::: search for next eigenvalue ::::::::::
  220 if (ien .lt. low) go to 680
      ...
      ienm1 = ien - 1
c     :::::::::: look for single small sub-diagonal element
c                for l=ien step -1 until low do -- ::::::::::
  240 do 260 ll = low, ien
         l = ien + low - ll
         if (l .eq. low) go to 300
         if (absz(hr(l,l-1)) + absz(hi(l,l-1)) .le.
...
cccc      if ((its/10)*10 .eq. its ) go to 320
ccccc     if (its .eq. 10 .or. its .eq. 20) go to 320
      sr = hr(ien,ien)
      si = hi(ien,ien)
      xr = hr(ienm1,ien) * hr(ien,ienm1) - hi(ienm1,ien) * hi(ien,ienm1)
      xi = hr(ienm1,ien) * hi(ien,ienm1) + hi(ienm1,ien) * hr(ien,ienm1)
      if ( iprsup .ge. 3)
     1 write (lunit6, 305) sr, si, xr, xi
  ...
  310 d1 = (yr+zzr) **2 + (yi+zzi) **2
      ...
      d3 = ( xi*(yr+zzr) - xr*(yi+zzi) ) / d1
      if ( iprsup .ge. 3 )
     1 write (lunit6, 315) yr, yi, zzr, zzi, d2,d3
  315 format ( 38h yr, yi, zzr, zzi and d2,d3 at 315 are, 6e15.6)
      sr = sr - d2
      si = si - d3
      go to 340
c     :::::::::: form exceptional shift ::::::::::
cccc  320 sr = absz(hr(ien,ienm1)) + absz(hr(ienm1,ien-2))
cccc      si = absz(hi(ien,ienm1)) + absz(hi(ienm1,ien-2))
  340 do 360 i = low, ien
         hr(i,i) = hr(i,i) - sr
         hi(i,i) = hi(i,i) - si
      if ( iprsup .ge. 3 )
     1 write (lunit6, 350) i, sr, si, hr(i,i),hi(i,i)
  350 format ( 43h i, sr, si, hr(i,i), and hi(i,i) at 350 are,/,
     1        10x, i8, 4e15.6)
  360 continue
      tr = tr + sr
      ti = ti + si
...
c     :::::::::: for m=ien-1 step -1 until l do -- ::::::::::
      do 380 mm = l, ienm1
         ...
         xr = absz(hr(m-1,m-1)) + absz(hi(m-1,m-1))
      if ( iprsup .ge. 3 )
     1 write (lunit6, 375) mm, yr, zzr, yi, xr, xi, epmach
  375 format ( 47h at 375, mm, yr, zzr, yi, xr, xi and epmach are,/,
     1         10x, i8, 6e15.6)
         if (yr .le. epmach * zzr / yi * (zzr + xr + xi)) go to 420
  380 continue
c     :::::::::: triangular decomposition h=l*r ::::::::::
...
c     iord(im1) = it
  ...
  500    continue
      if ( iprsup .ge. 3 )
     1 write (lunit6, 510) i,n, (hr(i,j), hi(i,j), j=1,n)
  510 format ( 42h at 510, (hr(i,j), hi(i,j), j=1,n) for i =, i8,
     1   10 h, and n = , i8,  2x, 3hare,/, (1x, 8e15.6) )
  520 continue
c     :::::::::: composition r*l=h ::::::::::
      do 640 j = mp1, ien
         xr = hr(j,j-1)
...
c     :::::::::: accumulate transformations ::::::::::
  ...
  620    continue
       if ( iprsup .ge. 3 )
     1 write ( lunit6, 630) j, low, igh, (hr(i,j),hi(i,j),i=1,j)
     2, (zr(i,j),zi(i,j),i=low,igh)
  630 format ( 26h j, low and igh at 630 are, 3i8, 60h (hr(i,j),hi(i,j),
     1i=1,j) and (zr(i,j),zi(i,j),i=low,igh) are, /, (1x,8e15.6) )
  640 continue
      go to 240
c     :::::::::: a root found ::::::::::
  660 hr(ien,ien) = hr(ien,ien) + tr
      ...
      ien = ienm1
      if ( iprsup .ge. 3 )
     1 write (lunit6, 670) ien,hr(ien,ien),hi(ien,ien),wr(ien),wi(ien)
  670 format ( 67h     ien    hr(ien,ien)    hi(ien,ien)       wr(ien)
     1      wi(ien),/, i8, 4e15.6)
      go to 220
  680 continue
      do 671 id=low,igh
...
c     :::::::::: for ien=n step -1 until 2 do -- ::::::::::
      do 800 nn = 2, n
         ien = n + 2 - nn
         xr = wr(ien)
         xi = wi(ien)
         ienm1 = ien - 1
c     :::::::::: for i=ien-1 step -1 until 1 do -- ::::::::::
         do 780 ii = 1, ienm1
            i = ien - ii
            zzr = hr(i,ien)
            zzi = hi(i,ien)
  ...
  760       yr = xr - wr(i)
      ...
      d3 = ( yr*zzi - yi*zzr ) / d1
            ...
            hi(i,ien) = d3
      if ( iprsup .ge. 3 )
     1 write (lunit6, 770)  nn,n,ien,ii,i,ienm1,zzr,zzi,yr,yi,d2,d3
     2, hr(i,ien),hi(i,ien)
  770 format ( 87h nn, n, ien, ii, i, ienm1, zzr, zzi, yr, yi, d2, d3, h
     1r(i,ien) and hi(i,ien) at 770 are,/, 1x, 6i5, 8e12.5)
  780    continue
  800 continue
...
c        m = min0z(j-1,igh)
  ...
  860       continue
            ...
            zi(i,j) = zzi
      if (iprsup .ge. 3 )
     1 write (lunit6, 870) jj, j, m, zr(i,j),zi(i,j)
  870 format ( 62h at the end of do 880 loop, jj, j,  m, zr(i,j) and zi(
     1i,j) are,5x,3i8,2e15.6)
  880 continue
      go to 1001
c     :::::::::: set error -- no convergence to an
c                eigenvalue after 30 iterations ::::::::::
 1000 ierr = ien
 1001 continue
cccc      do 1002 i=low, igh
cccc      wr(i) = umr(i)
cccc      wi(i) = umi(i)
 1002 continue
cc     ** sorting the eigenvalues with magnitude **
cc        also reordering the eigenvector
      do 1003 i=1,20
      lseq(i)=i
 1003 continue
      do 1004 i=1,n
...
c     :::::::::: last card of comlr2 ::::::::::
      ...
      implicit real*8 (a-h, o-z) ,
     1      integer*4 (i-n)
c)    elimination of variables m+1,...n in symmetric complex matrix with
c)    a=real part, c=imaginary part. a and c are
c)    stored as triangle (1 element for 1.column,2 for 2.column etc.).
c)    result is reduced matrix in columns 1,...m in case of reduction
c)    (m unequal 0) or negative inverse matrix in columns 1,...n in case
c)    of inversion (m=0).
...
c                                   end k-loop
  ...
  681 ai(i,k) = f2
   ...
   70 k=k+1
      ...
      if(ki.lt.j) go to 71
      if(i.gt.k) go to 73
   71 p(m) =(ar(i,1)+fr(k)*(ar(i,2)+ar(i,3))+fi(k)*(ai(i,3)-ai(i,2)) )
     1 / 3.0
   72 z(m) =(ai(i,1)+fr(k)*(ai(i,2)+ai(i,3))+fi(k)*(ar(i,2)-ar(i,3)) )
     1 / 3.0
   ...
   76 if(k.eq.3) go to 65
      ...
      go to 67
   79 kk=kk-3
c                                   end of symmetrical components matrix
      return
  ...
  320 format (27h e, f,  e2f2, and s2 at 320, 4e16.6)
      ...
      xf = (g*e+h*f)*s2
      go to 9900
c      calculation of kelvin-functions for argument.le.8.
  100 z = x2
      ber = unity
      bei = 0.0
      berd = 0.0
  ...
  104 go to (6 , 4 ),iback
c      calculations of kelvin-functions for argument.gt.8.
  200 x2 = 8.0  /x
      z = x2
      ber = 0.0
      bei = - valu11
 ...
 9900 continue
      ...
      implicit real*8 (a-h, o-z) ,
     1      integer*4 (i-n)
c     dummy imitation of ontario hydro univac module.
      n1 = -7654
      return
      end
...
c
      ...
      implicit real*8 (a-h, o-z) ,
     1     integer*4 (i-n)
c)    Elimination of variables m+1,...n in symmetric matrix a. A is
c)    stored as triangle (1 element for 1.column,2 for 2.column etc.).
c)    Result is reduced matrix in columns 1,...m in case of reduction
c)    (m unequal 0) or negative inverse matrix in columns 1,...n in case
c)    of inversion (m=0).
