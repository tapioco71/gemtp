...
int main(int argc, char *argv[])
{
  ...
  char cblock[ 196 ][ 7 ] =
    {
     "x     ", "ykm   ", "km    ", "xk    ", "xm    ", "weight", "iwtent", "con1  ",
     "iskip ", "zinf  ", "eta   ", "nhist ", "stailm", "stailk", "xmax  ", "koutvp",
     "bnrg  ", "sconst", "cnvhst", "sfd   ", "qfd   ", "semaux", "ibsout", "bvalue",
     "sptacs", "kswtyp", "modswt", "begsw ", "lastsw", "kentnb", "nbhdsw", "topen ",
     "crit  ", "kdepsw", "tdns  ", "isourc", "energy", "iardub", "ardube", "nonlad",
     "nonle ", "vnonl ", "curr  ", "anonl ", "vecnl1", "vecnl2", "namenl", "vzero ",
     "ilast ", "nltype", "kupl  ", "nlsub ", "xoptbr", "coptbr", "cursub", "cchar ",
     "vchar ", "gslope", "ktrans", "kk    ", "emtpc ", "tr    ", "tx    ", "r     ",
     "nr    ", "length", "cik   ", "ci    ", "ck    ", "ismout", "elp   ", "cu    ",
     "shp   ", "histq ", "ismdat", "texvec", "ibrnch", "jbrnch", "tstop ", "nonlk ",
     "nonlm ", "spum  ", "kks   ", "kknonl", "znonl ", "znonlb", "znonlc", "finit ",
     "ksub  ", "msub  ", "isubeg", "litype", "imodel", "kbus  ", "mbus  ", "kodebr",
     "cki   ", "ckkjm ", "indhst", "kodsem", "namebr", "iform ", "node  ", "crest ",
     "time1 ", "time2 ", "tstart", "sfreq ", "kmswit", "nextsw", "rmfd  ", "cikfd ",
     "imfd  ", "tclose", "adelay", "kpos  ", "namesw", "emtpe ", "emtpf ", "kssfrq",
     "kode  ", "kpsour", "volti ", "voltk ", "volt  ", "bus   ", "karray", "tp    ",
     "norder", "index ", "diag  ", "diab  ", "solr  ", "soli  ", "ich1  ", "bnd   ",
     "iloc  ", "gnd   ", "karray", "xdat  ", "ydat  ", "aphdat", "jndex ", "diagg ",
     "diabb ", "solrsv", "solisv", "gndd  ", "bndd  ", "nekfix", "fxtem1", "fxtem2",
     "fxtem3", "fxtem4", "fxtem5", "fxtem6", "fixbu1", "fixbu2", "fixbu3", "fixbu4",
     "fixbu5", "fixbu6", "fixbu7", "fixbu8", "fixbu9", "fixb10", "fixb11", "kndex ",
     "karray", "hp    ", "z     ", "ic    ", "r     ", "emtpd ", "gmd   ", "x     ",
     "y     ", "tb2   ", "itb3  ", "workr1", "workr2", "text  ", "gd    ", "bd    ",
     "yd    ", "itbic ", "tbr   ", "tbd   ", "tbg   ", "tbx   ", "tby   ", "tbtb2 ",
     "itbtb3", "tbtext", "karray", "karray"
    };
  char letters[ 6 ][ 2 ] =
    {
     "i", "j", "k", "l", "m", "n"
  ...
  char types[ 4 ][ 11 ] =
    {
     "real(8)", "complex(8)", "real(8)", "integer(4)"
    };
  FILE *pFile = NULL;

  for (i = 0; i < 127; i++)
    snprintf(cblser[ i ], 7, "c0b%03d", (unsigned int) i);
...
/*
  ...
  data  filename     / 14havvelenato.for /
  ...
  mulvar(3)  =  2
  mulvar(4)  =  1
  open (unit = lunit(2), status = 'replace', file = filename, iostat = ios, form = 'formatted')
  if (ios .eq. 0) then
     indexm = 0
     mextra = 0
...
8156 format (6x, 'return', 68x)
     ...
     nrec3 = nrec3 + 2
8158 format (i3, '00 if (nchain .gt.', i4, ') go to', i3, '00', 38x)
     do ii = 1, 12
        i = 126  +  ii
        n3 = ncbarr(i)
...
4769    format (6x, 'lsize(', i2, ') =', i7, 56x)
        nrec3 = nrec3 + 2
4602    continue
        nonneg = lstnew(n3)
        if ( nonneg  .le.  0 )   nonneg = 1
...
7274 format (6x, 'call subr', i2, 63x)
     write (lunit(2), 7297)
     write (lunit(2), 7298)
     indexm = indexm + 1
     mextra = 5000
...
9900 continue
     ...
     write (lunit(6), 1) filename, ios
  end if
1    format ('Error opening file ', a16, ' code ', i6, '. Exiting.', //)
9999 stop
end program vardim
*/
